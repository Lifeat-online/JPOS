...
describe('demo seed data', () => {
    ...
    it('clears seeded demo rows and stale demo package denials', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("DELETE FROM sales WHERE tenant_id = $1 AND id LIKE 'demo_%'"), ['tenant_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("DELETE FROM cash_sessions WHERE tenant_id = $1 AND id LIKE 'demo_%'"), ['tenant_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("DELETE FROM customers WHERE tenant_id = $1 AND id LIKE 'demo_cust_%'"), ['tenant_1']);
