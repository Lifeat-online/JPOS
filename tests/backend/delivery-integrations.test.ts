...
import { ingestDeliveryOrder, listDeliveryOrders, normalizeDeliveryProvider, normalizeDeliveryStatus, updateDeliveryOrderStatus, } from '../../server/deliveryIntegrations.js';
...
describe('delivery integrations', () => {
    ...
    it('ingests a new Uber Eats order with normalized line items and audit evidence', async () => {
        (dbModule.query as any).mockImplementation((sql: string) => {
            if (sql.includes('SELECT id FROM delivery_orders'))
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO delivery_orders'), expect.arrayContaining(['tenant_1', 'uber_eats', 'UE-100', 'new', 'Sam']));
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO delivery_order_items'), expect.arrayContaining(['tenant_1', expect.any(String), 'item_1', null, 'Burger', 2, 74]));
    ...
    it('updates an existing Mr D order idempotently and replaces line items', async () => {
        (dbModule.query as any).mockImplementation((sql: string) => {
            if (sql.includes('SELECT id FROM delivery_orders'))
                return Promise.resolve([{ id: 'do_existing' }]);
            if (sql.includes('FROM delivery_orders') && sql.includes('SELECT')) {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE delivery_orders'), expect.arrayContaining(['accepted', null, null, null, 50, 0, 0, 0, 50]));
        expect(dbModule.query).toHaveBeenCalledWith("DELETE FROM delivery_order_items WHERE tenant_id = $1 AND delivery_order_id = $2", ['tenant_1', 'do_existing']);
        expect(recordAuditEventSafe).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('lists delivery orders with hydrated item rows', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining("provider = $1"), ['tenant_1', 'mr_d', 'ready']);
    ...
    it('updates order status and audits the transition', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE delivery_orders'), ['ready', 'ready', 'tenant_1', 'do_1']);
        expect(recordAuditEventSafe).toHaveBeenCalledWith(expect.objectContaining({
