...
describe('audit helpers', () => {
    it('records JSON audit event details', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'sale.created', 'sale', 'sale_1']));
    ...
    it('applies product stock deltas and records a movement row', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Bread', -3, 10, 7, 'sale']));
