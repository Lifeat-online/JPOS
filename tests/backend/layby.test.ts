...
describe('layby workflow', () => {
    ...
    it('creates a lay-by, reserves stock, records deposit payment, and audits it', async () => {
        const conn = makeConn(async (sql: string) => {
            ...
            if (sql.includes('FROM products'))
                return [[{ id: 'prod_1', name: 'Shoes', stock: 5 }]];
            return [[]];
        });
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE products SET stock = $1"), [3, 'tenant_1', 'prod_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Shoes', -2, 5, 3, 'layby_reserve', 'transfer']));
    ...
    it('completes a paid-up lay-by by creating a layby_final sale without a second stock deduction', async () => {
        const conn = makeConn(async (sql: string) => {
            ...
            if (sql.includes('FROM layby_items'))
                return [[{ id: 'layitem_1', productId: 'prod_1', productName: 'Shoes', price: 200, quantity: 1, reservedQuantity: 1 }]];
            if (sql.includes('FROM layby_payments'))
                return [[{ id: 'laypay_1', method: 'cash', amount: 200, tenderedAmount: 200, changeAmount: 0 }]];
            return [[]];
        });
        (dbModule.getConnection as any).mockResolvedValue(conn);
    ...
    it('cancels an active lay-by, releases reserved stock, and records cash refunds', async () => {
        const conn = makeConn(async (sql: string) => {
            ...
            if (sql.includes('FROM layby_items'))
                return [[{ id: 'layitem_1', productId: 'prod_1', productName: 'Shoes', price: 200, quantity: 2, reservedQuantity: 2 }]];
            if (sql.includes('FROM layby_payments'))
                return [[{ id: 'laypay_1', method: 'cash', amount: 100, tenderedAmount: 100, changeAmount: 0 }]];
            if (sql.includes('FROM products'))
                return [[{ id: 'prod_1', name: 'Shoes', stock: 3 }]];
            return [[]];
        });
        (dbModule.getConnection as any).mockResolvedValue(conn);
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE products SET stock = $1"), [5, 'tenant_1', 'prod_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Shoes', 2, 3, 5, 'layby_release', 'transfer']));
