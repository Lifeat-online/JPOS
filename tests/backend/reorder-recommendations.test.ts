...
} from '../../server/reorderRecommendations.js';
...
describe('reorder recommendations', () => {
  ...
  it('creates auditable reorder notification rules by location', async () => {
    (dbModule.query as any).mockImplementation((sql: string) => {
      if (sql.includes('INSERT INTO reorder_notification_rules')) return Promise.resolve({});
    ...
    expect(dbModule.query).toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO reorder_notification_rules'),
      expect.arrayContaining(['tenant_1', 'Branch cooler watch', 'active', 'branch_1', 'days_cover', 'high', 21, 'vendor_1'])
    );
  ...
  it('runs location-specific reorder rules into purchase-order suggestions', async () => {
    ...
    (dbModule.query as any).mockImplementation((sql: string, params: any[] = []) => {
      ...
      if (sql.includes('FROM products p')) {
        expect(sql).toContain('COALESCE(pls.location_id, ?) = ?');
    ...
    expect(dbModule.query).toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO reorder_recommendations'),
      expect.arrayContaining(['vendor_1', 'branch_1', 'reorder_rule'])
    );
    expect(dbModule.query).toHaveBeenCalledWith(
      expect.stringContaining('last_result = ?'),
      expect.arrayContaining(['tenant_1', 'rule_1'])
    );
    expect(auditModule.recordAuditEvent).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
