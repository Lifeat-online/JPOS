...
describe('staff scheduling and attendance', () => {
    ...
    it('creates audited draft shifts using staff pay/profile data', async () => {
        (dbModule.query as any).mockImplementation((sql: string) => {
            if (isStaffProfileQuery(sql) && sql.includes('LIMIT 1'))
                return Promise.resolve([staffRow]);
            if (sql.includes('INSERT INTO staff_shifts'))
                return Promise.resolve([]);
            if (sql.includes('FROM staff_shifts') && sql.includes("WHERE tenant_id = $1 AND id = $2"))
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO staff_shifts'), expect.arrayContaining(['tenant_1', 'staff_1', 'Jess Cashier', 'cashier', '2026-06-05', '2026-06-05 08:00:00', '2026-06-05 17:00:00', 'draft']));
    ...
    it('publishes roster ranges and returns refreshed shifts', async () => {
        (dbModule.query as any).mockImplementation((sql: string) => {
            if (sql.includes('UPDATE staff_shifts'))
                return Promise.resolve([]);
            if (sql.includes('FROM staff_shifts'))
                return Promise.resolve([{ ...shiftRow, status: 'published' }]);
            return Promise.resolve([]);
        });
        const result = await publishRoster('tenant_1', '2026-06-05', '2026-06-07', { staffId: 'mgr_1', staffName: 'Manager' });
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining("status = 'published'"), ['mgr_1', 'Manager', 'tenant_1', '2026-06-05', '2026-06-07']);
    ...
    it('clock-in links the scheduled shift and stores the scheduled minutes', async () => {
        ...
        (dbModule.query as any).mockImplementation((sql: string) => {
            ...
            if (sql.includes('FROM staff_shifts') && sql.includes("status IN ('draft','published')"))
                return Promise.resolve([{ ...shiftRow, status: 'published' }]);
            if (sql.includes('INSERT INTO staff_attendance'))
                return Promise.resolve([]);
            return Promise.resolve([]);
        });
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO staff_attendance'), expect.arrayContaining(['tenant_1', 'staff_1', 'Jess Cashier', 'shift_1', '2026-06-05 08:00:00', 510]));
    ...
    it('adds break minutes when ending an active break', async () => {
        (dbModule.query as any).mockImplementation((sql: string) => {
            ...
            if (sql.includes('UPDATE staff_attendance'))
                return Promise.resolve([]);
            return Promise.resolve([]);
        });
        await endBreak('tenant_1', 'staff_1', '2026-06-05T10:30:00');
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining("break_minutes = COALESCE(break_minutes, 0) + $1"), [30, 'tenant_1', 'att_1']);
        expect(recordAuditEventSafe).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('clock-out calculates worked minutes, overtime, and hourly payroll amount', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining("status = 'closed'"), ['2026-06-05 18:00:00', 30, 570, 480, 90, 1025, null, 'tenant_1', 'att_1']);
