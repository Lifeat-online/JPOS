...
describe('db-crud', () => {
    ...
    it('creates a sale without treating transaction row tuples as recipe rows', async () => {
        const conn = {
            ...
            query: vi.fn().mockImplementation((sql: string) => {
                if (sql.includes('SELECT bulk_item_id'))
        ...
        expect(conn.query).not.toHaveBeenCalledWith(expect.stringContaining("UPDATE bulk_items SET stock = stock - $1"), expect.anything());
    });
    it('validates and records promotion redemptions when creating a completed sale', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('SELECT * FROM promotions'), expect.arrayContaining(['tenant_1', 'SAVE10']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO sales'), expect.arrayContaining(['promo_1', 'SAVE10', 10]));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO promotion_redemptions'), expect.arrayContaining(['tenant_1', 'promo_1', 'SAVE10', expect.any(String), 'cust_1', 'staff_1', 10]));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE promotions SET redemption_count = redemption_count + 1'), ['tenant_1', 'promo_1']);
    });
    it('does not stamp ordered_at for held workstation items', async () => {
        ...
        expect(String(itemInsert?.[0]).replace(/\s+/g, ' ')).toContain("$1, $2, $3, $4, $5, $6, $7, $8, NULL, NOW(), NOW())");
    });
    it('stamps ordered_at with server time when sending workstation items', async () => {
        ...
        const itemInsert = conn.query.mock.calls.find(([sql]: any[]) => String(sql).includes('INSERT INTO sale_items'));
        expect(String(itemInsert?.[0]).replace(/\s+/g, ' ')).toContain("$1, $2, $3, $4, $5, $6, $7, $8, CURRENT_TIMESTAMP, NOW(), NOW())");
    });
    it('updates a sale and replaces its items', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("SELECT * FROM sale_items WHERE sale_id = $1"), ['sale_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("DELETE FROM sale_items WHERE sale_id = $1"), ['sale_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO sale_items'), expect.arrayContaining(['sale_1', 'prod_1', 'Burger', 25, 1, 'pending']));
    ...
    it('preserves existing workstation timestamps and stamps newly re-added items on resend', async () => {
        ...
        expect(String(itemInserts[0][0]).replace(/\s+/g, ' ')).toContain("$1, $1, NULL, NULL, $9, NOW(), NOW())");
        ...
        expect(String(itemInserts[1][0]).replace(/\s+/g, ' ')).toContain("CURRENT_TIMESTAMP, NULL, NULL, NULL, $9, NOW(), NOW())");
    ...
    it.each([
        ...
        expect(sql).not.toContain("ordered_at = $1");
    ...
    it('records stock movements and audit events when a sale completes', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Bread', -2, 10, 8, 'sale']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'sale.created', 'sale']));
    });
    it('depletes stock batches in FEFO order when a sale completes', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('FROM stock_batches'), ['tenant_1', 'prod_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE stock_batches'), [0, 'depleted', 'tenant_1', 'batch_old']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE stock_batches'), [3, 'active', 'tenant_1', 'batch_new']);
    });
    it('receives purchase orders by booking audited stock movements once', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE products SET stock = $1"), [10, 'tenant_1', 'prod_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Coffee Beans', 6, 4, 10, 'purchase_order_receiving', 'receiving']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE purchase_orders'), expect.arrayContaining(['received', 'paid', expect.any(String), 'INV-100', '2026-06-01', 'mgr_1', 'Manager', 'Manager counted delivery', 252, 'tenant_1', 'po_1']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'purchase_order.received', 'purchase_order', 'po_1']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_batches'), expect.arrayContaining(['tenant_1', 'prod_1', 'Coffee Beans', 'po_1', 'vendor_1', 'INV-100', '2026-06-01', 'LOT-7', 6, 6, 42, '2026-07-15']));
        expect(conn.commit).toHaveBeenCalled();
        expect(conn.rollback).not.toHaveBeenCalled();
    ...
    it('rejects duplicate purchase-order receiving before stock is booked again', async () => {
        ...
        expect(conn.query).not.toHaveBeenCalledWith(expect.stringContaining("UPDATE products SET stock = $1"), expect.anything());
    ...
    it('includes offline_event_id and sync_source in the INSERT when provided', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO sales'), expect.arrayContaining(['offline_sale_123_test', 'offline']));
        // offline.sale_synced audit event should be recorded
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'offline.sale_synced', 'sale']));
    });
    it('persists recipe yield, waste, and substitution metadata', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('yield_quantity'), ['prod_1', 'bulk_1', 2, 4, 10, 'milk', 1, 1]);
        expect(conn.commit).toHaveBeenCalled();
    });
    ...
    it('deducts recipe stock with yield waste and ranked substitutions', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE bulk_items SET stock = stock - $1"), [1.1, 'bulk_primary']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE bulk_items SET stock = stock - $1"), [2.4, 'bulk_almond']);
        expect(conn.query).not.toHaveBeenCalledWith(expect.stringContaining("UPDATE bulk_items SET stock = stock - $1"), expect.arrayContaining(['bulk_oat']));
    });
    ...
    it('records manager-review conflicts when an offline sync would oversell stock', async () => {
        const conn = {
            ...
            query: vi.fn().mockImplementation((sql: string) => {
                if (sql.includes('SELECT id, name, stock'))
                    return Promise.resolve([[{ id: 'prod_1', name: 'Tea', stock: 1 }]]);
                if (sql.includes('SELECT bulk_item_id'))
                    return Promise.resolve([[]]);
                return Promise.resolve([[]]);
            }),
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'offline.sync_conflict', 'sale']));
        const conflictAudit = conn.query.mock.calls.find(([sql, params]: any[]) => (String(sql).includes('INSERT INTO audit_events') && params?.[2] === 'offline.sync_conflict'));
    ...
    it('deducts customer wallet inside completed wallet checkout transactions', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE customers'), [60, 'tenant_1', 'cust_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'customer_wallet.sale_payment', 'customer_wallet', 'cust_1']));
        expect(conn.commit).toHaveBeenCalled();
    });
    it('reserves staff wallet balance when creating payout requests', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE staff'), [55, 'tenant_1', 'staff_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO payout_requests'), expect.arrayContaining(['tenant_1', 'staff_1', 'Jess', 35, 'pending']));
    ...
    describe('transaction-safe checkout side effects', () => {
        ...
        it('records BNPL refund references and marks the original payment refunded', async () => {
            ...
            conn.query.mockImplementation((sql: string) => {
                if (sql.includes('FROM sales') && sql.includes("WHERE tenant_id = $1 AND id = $2")) {
                    return Promise.resolve([[{
                                id: 'sale_bnpl_1',
                                tenant_id: 'tenant_1',
                                customer_id: 'cust_1',
                ...
                if (sql.includes('INNER JOIN sale_items'))
                    return Promise.resolve([[]]);
                return Promise.resolve([[]]);
            });
            (dbModule.getConnection as any).mockResolvedValue(conn);
            ...
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO sale_payments'), expect.arrayContaining(['bnpl', -100, -100, 'mobicred', 'MOBI-REFUND-1', 'refunded']));
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE sale_payments'), expect.arrayContaining(['refunded', 'Refund approved in Mobicred portal', 'sale_bnpl_1']));
            expect(conn.commit).toHaveBeenCalled();
        });
        it('updates sale payment provider reconciliation status with an audit event', async () => {
            ...
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE sale_payments'), expect.arrayContaining(['payjustnow', 'PJN-Portal', 'PJN-SETTLED-1', 'AUTH-SETTLED-1', 'settled', 'Settled in provider portal', 'sale_bnpl_1', 'pay_bnpl_1']));
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'payment.provider_reconciled', 'sale_payment']));
            expect(conn.commit).toHaveBeenCalled();
        });
        it('rejects sensitive provider reconciliation evidence before updating payment rows', async () => {
            ...
            expect(conn.query).not.toHaveBeenCalledWith(expect.stringContaining('UPDATE sale_payments'), expect.anything());
            expect(conn.rollback).toHaveBeenCalled();
            expect(conn.commit).not.toHaveBeenCalled();
        });
        it('applies loyalty points, cash session, staff metrics and account balance atomically within the sale transaction', async () => {
            ...
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE customers'), expect.arrayContaining([100, 'tenant_1', 'cust_1']));
            // Verify cash session expected_cash update within the transaction
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE cash_sessions'), expect.arrayContaining([50, 'cs_1', 'tenant_1']));
            // Verify cash movement recorded within the transaction
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO cash_movements'), expect.arrayContaining(['tenant_1', 'cs_1', 'cash_sale', 'in', 50, expect.stringContaining('sale_')]));
            // Verify staff metrics update within the transaction
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE staff'), expect.arrayContaining([expect.stringContaining('orders'), 'tenant_1', 'staff_1']));
        ...
        it('calculates earned loyalty points from active tiers before updating the customer balance', async () => {
            ...
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE customers SET loyalty_points = $1"), [90, 'tenant_1', 'cust_1']);
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('SELECT * FROM loyalty_tiers'), ['tenant_1']);
            expect(conn.commit).toHaveBeenCalled();
            expect(conn.rollback).not.toHaveBeenCalled();
        });
        it('applies tips delta for card payments', async () => {
            ...
            await createSale('tenant_1', sale as any);
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE cash_sessions'), expect.arrayContaining([10, 'cs_1', 'tenant_1']));
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO cash_movements'), expect.arrayContaining(['tenant_1', 'cs_1', 'tip', 'neutral', 10]));
            expect(conn.commit).toHaveBeenCalled();
        });
        it('applies account balance delta for account payments', async () => {
            ...
            await createSale('tenant_1', sale as any);
            expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('GREATEST'), expect.arrayContaining([50, 'tenant_1', 'cust_1']));
            expect(conn.commit).toHaveBeenCalled();
        });
    ...
    it('reserves customer wallet balance when creating customer payout requests', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE customers'), [40, 'tenant_1', 'cust_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO customer_payout_requests'), expect.arrayContaining(['tenant_1', 'cust_1', 'Lebo', 'lebo@example.test', 30, 'pending']));
