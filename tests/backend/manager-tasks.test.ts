...
describe('manager task queue', () => {
    ...
    it('syncs operational signals into active manager tasks', async () => {
        (dbModule.query as any).mockImplementation((sql: string) => {
            ...
            if (sql.includes('INSERT INTO manager_tasks'))
                return Promise.resolve({});
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO manager_tasks'), expect.arrayContaining(['tenant_1', 'cash_variance']));
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO manager_tasks'), expect.arrayContaining(['tenant_1', 'ai_recommendation']));
    ...
    it('approves reorder recommendation tasks into purchase orders', async () => {
        ...
        (dbModule.query as any).mockImplementation((sql: string) => {
            ...
            if (sql.includes('UPDATE manager_tasks'))
                return Promise.resolve({});
    ...
    it('syncs stocktake exceptions and offline sync issues into manager tasks', async () => {
        ...
        const result = await getManagerTaskQueue('tenant_1');
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO manager_tasks'), expect.arrayContaining(['tenant_1', 'stock_variance']));
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO manager_tasks'), expect.arrayContaining(['tenant_1', 'offline_sync']));
    ...
    it('requires a manager note and audits task decisions', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE cash_sessions'), ['reviewed', 'mgr_1', 'Checked till slip and accepted variance.', 'tenant_1', 'cash_1']);
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'manager_task.approved', 'manager_task', 'task_1']));
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO manager_overrides'), expect.arrayContaining(['tenant_1', 'manager_task', 'manager_task', 'task_1', 'approve', 'approved', 'Checked till slip and accepted variance.']));
    ...
    it('requires a manager override reason before dismissing a task', async () => {
        ...
        expect(dbModule.query).not.toHaveBeenCalledWith(expect.stringContaining('UPDATE manager_tasks'), expect.anything());
    });
    it('creates cashier refund approval requests for the manager queue', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO manager_tasks'), expect.arrayContaining(['tenant_1', 'refund_request']));
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'manager_task.requested.refund', 'manager_task']));
    ...
    it('executes approved refund requests from the manager action center', async () => {
        ...
        expect(crudModule.processSaleRefund).toHaveBeenCalledWith('tenant_1', 'sale_1', expect.objectContaining({
            reason: 'Wrong item',
            staffId: 'mgr_1',
            staffName: 'Manager',
        }));
    ...
    it('creates cashier stock adjustment approval requests for the manager queue', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO manager_tasks'), expect.arrayContaining(['tenant_1', 'Decrease stock for Milk']));
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'manager_task.requested.stock_adjustment', 'manager_task']));
    ...
    it('executes approved stock adjustment requests through the stock ledger', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE products SET stock = $1"), [7, 'tenant_1', 'prod_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Milk', 2, 5, 7, 'manual_adjustment']));
    ...
    it('approves stocktake variance tasks through the stocktake ledger', async () => {
        ...
        expect(stockTakeModule.approveStockTakeSession).toHaveBeenCalledWith('tenant_1', 'session_1', { staffId: 'mgr_1', staffName: 'Manager', role: 'manager' });
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'manager_task.approved', 'manager_task', 'task_stocktake_1']));
        expect(result).toMatchObject({
            id: 'task_stocktake_1',
            status: 'approved',
