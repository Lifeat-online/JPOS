...
import { buildHardwareCommand, createHardwareDevice, queueCashDrawerPulseForNoSale, queueKitchenPrintJobsForSale, testHardwareDevice, type HardwareDevice, } from '../../server/hardwareAdapters.js';
...
describe('hardware adapters', () => {
    ...
    it('records failed readiness checks for incomplete network printer configuration', async () => {
        ...
        const updateCall = (dbModule.query as any).mock.calls.find(([sql]: any[]) => String(sql).includes("SET last_check_status = $1"));
