...
import { authenticateIntegrationApiKey, createIntegrationApiKey, hashIntegrationApiKey, ingestStockWebhook, } from '../../server/integrationAccess.js';
...
describe('integration access and stock webhooks', () => {
    ...
    it('authenticates active API keys by hash and updates last used time', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE integration_api_keys SET last_used_at = NOW()'), ['tenant_1', 'iak_1']);
    });
    it('applies stock snapshot webhooks to product and location stock with ledger evidence', async () => {
        (dbModule.query as any).mockImplementation((sql: string) => {
            if (sql.includes('FROM integration_webhook_events') && sql.includes('idempotency_key'))
                return Promise.resolve([]);
            if (sql.includes('FROM products') && sql.includes("id = $1")) {
                return Promise.resolve([{ id: 'prod_1', name: 'Milk', stock: 3, min_stock: 2, barcode: '6001' }]);
            }
            if (sql.includes('FROM product_location_stock'))
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE products SET stock = $1"), [8, 'tenant_1', 'prod_1']);
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO product_location_stock'), expect.arrayContaining(['tenant_1', 'prod_1', 'main', 8]));
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Milk', 5, 3, 8]));
        expect(recordAuditEventSafe).toHaveBeenCalledWith(expect.objectContaining({
            tenantId: 'tenant_1',
    ...
    it('marks webhook events failed when products cannot be matched', async () => {
        ...
        expect(dbModule.query).toHaveBeenCalledWith(expect.stringContaining("SET status = 'failed'"), expect.arrayContaining(['tenant_1']));
        expect(recordAuditEventSafe).toHaveBeenCalledWith(expect.objectContaining({
