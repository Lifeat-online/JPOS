...
describe('layby schema', () => {
  ...
  it('defines the lay-by ledger tables in MariaDB schema', () => {
    const schema = fs.readFileSync(schemaPath, 'utf8');
    expect(schema).toContain('CREATE TABLE IF NOT EXISTS layby_orders');
    expect(schema).toContain("status ENUM('active','completed','cancelled')");
    expect(schema).toContain('CREATE TABLE IF NOT EXISTS layby_items');
    expect(schema).toContain('reserved_quantity DECIMAL(12,3)');
    expect(schema).toContain('CREATE TABLE IF NOT EXISTS layby_payments');
    expect(schema).toContain("method ENUM('cash','payfast','card','wallet','account')");
  });
