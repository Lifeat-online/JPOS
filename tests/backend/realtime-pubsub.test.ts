...
import { advanceRealtimeCursor, buildFetchRealtimeEventsQuery, buildRealtimePubsubSchemaStatements, ensureRealtimePubsubSchema, fetchRealtimeEvents, initialRealtimeCursor, parseRealtimeEventRow, publishRealtimeEvent, realtimeFanoutEnabled, } from "../../server/realtimePubsub.ts";
describe("database realtime pub/sub fan-out", () => {
    ...
    it("publishes room events with instance id, channel, event name, payload, and expiry", async () => {
        ...
        const id = await publishRealtimeEvent({ channel: "tenant:tenant_1", eventName: "sales_update", payload: { saleId: "sale_1" } }, {
        ...
        expect(calls[0].sql).toContain("($6::text || ' minutes')::interval");
