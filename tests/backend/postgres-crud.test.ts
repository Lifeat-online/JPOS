...
describe("db-crud", () => {
  ...
  it("creates a product with generated id", async () => {
    ...
    const product = await createProduct("tenant_1", {
      name: "Sample",
      price: 25,
      category: "Food",
      stock: 10,
  ...
  it("updates a product and fetches the new row", async () => {
    (dbModule.query as any)
      ...
      .mockResolvedValueOnce([
        { id: "prod_1", name: "Sample Updated", price: 30 },
      ]);
  ...
  it("creates a sale without treating transaction row tuples as recipe rows", async () => {
    ...
    expect(conn.query).not.toHaveBeenCalledWith(
      expect.stringContaining("UPDATE bulk_items SET stock = stock - $1"),
      expect.anything(),
    );
  });
  ...
  it("does not stamp ordered_at for held workstation items", async () => {
    ...
    const itemInsert = conn.query.mock.calls.find(([sql]: any[]) =>
      String(sql).includes("INSERT INTO sale_items"),
    );
    expect(String(itemInsert?.[0]).replace(/\s+/g, " ")).toContain(
  ...
  it("stamps ordered_at with server time when sending workstation items", async () => {
    ...
    expect(String(itemInsert?.[0]).replace(/\s+/g, " ")).toContain(
      "$1, $2, $3, $4, $5, $6, $7, $8, CURRENT_TIMESTAMP, NOW(), NOW())",
    );
  });
  it("updates a sale and replaces its items", async () => {
    ...
    expect(conn.query).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining(
        "SELECT status, transaction_type, staff_id, customer_id, offline_event_id FROM sales",
      ),
      ["tenant_1", "sale_1"],
    );
    expect(conn.query).toHaveBeenCalledWith(
      expect.stringContaining("tax_periods tp"),
      ["tenant_1", "sale_1"],
    );
    expect(conn.query).toHaveBeenCalledWith(
      expect.stringContaining("UPDATE sales SET"),
      [25, "kitchen", "tenant_1", "sale_1"],
    );
    expect(conn.query).toHaveBeenCalledWith(
      expect.stringContaining("SELECT * FROM sale_items WHERE sale_id = $1"),
      ["sale_1"],
    );
    expect(conn.query).toHaveBeenCalledWith(
      expect.stringContaining("DELETE FROM sale_items WHERE sale_id = $1"),
      ["sale_1"],
    );
    expect(conn.query).toHaveBeenCalledWith(
  ...
  it("preserves existing workstation timestamps and stamps newly re-added items on resend", async () => {
    ...
    const conn = {
      ...
      query: vi.fn().mockImplementation((sql: string) => {
        if (sql.includes("SELECT status, transaction_type")) {
          return Promise.resolve([
            [{ status: "kitchen", transaction_type: "sale" }],
          ]);
        }
    ...
    const itemInserts = conn.query.mock.calls.filter(([sql]: any[]) =>
      String(sql).includes("INSERT INTO sale_items"),
    );
    expect(itemInserts).toHaveLength(2);
    expect(String(itemInserts[0][0]).replace(/\s+/g, " ")).toContain(
      "$1, $1, NULL, NULL, $9, NOW(), NOW())",
    );
    expect(itemInserts[0][1]).toEqual(
      expect.arrayContaining([orderedAt, acceptedAt]),
    );
    expect(itemInserts[0][1]).not.toContain(clientClockValue);
    expect(String(itemInserts[1][0]).replace(/\s+/g, " ")).toContain(
      "CURRENT_TIMESTAMP, NULL, NULL, NULL, $9, NOW(), NOW())",
    );
  ...
  it.each([
    ...
    "stamps %s items without trusting client timestamps",
    async (status, column) => {
      (dbModule.query as any).mockResolvedValue([{}]);
    ...
    const conn = {
      ...
      query: vi.fn().mockImplementation((sql: string) => {
        ...
        if (sql.includes("FROM stock_batches")) {
          return Promise.resolve([
            [
              { id: "batch_old", remainingQuantity: 1 },
              { id: "batch_new", remainingQuantity: 5 },
            ],
          ]);
        }
        return Promise.resolve([[]]);
  ...
  it("receives purchase orders by booking audited stock movements once", async () => {
    const conn = {
      ...
      query: vi.fn().mockImplementation((sql: string) => {
        ...
        if (sql.includes("SELECT id, name, stock")) {
          return Promise.resolve([
            [{ id: "prod_1", name: "Coffee Beans", stock: 4 }],
          ]);
        }
        return Promise.resolve([[]]);
      }),
  ...
  it("rejects duplicate purchase-order receiving before stock is booked again", async () => {
    ...
    (dbModule.getConnection as any).mockResolvedValue(conn);
    await expect(
      receivePurchaseOrder("tenant_1", "po_1", {}, { staffId: "mgr_1" }),
        ...
        if (sql.includes("FROM product_recipes r")) {
          return Promise.resolve([
            [
              ...
              {
                bulk_item_id: "bulk_oat",
                quantity: "1",
                yield_quantity: "1",
                waste_percent: "0",
                substitute_group: "milk",
                substitute_rank: 0,
                stock: "0",
              },
              {
                bulk_item_id: "bulk_almond",
                quantity: "1.2",
                yield_quantity: "1",
                waste_percent: "0",
                substitute_group: "milk",
  ...
  it("builds recipe gross-margin reports from yield waste and substitutions", async () => {
    ...
    expect(
      report.rows[0].lines.find((line) => line.bulkItemId === "chicken"),
    ).toMatchObject({ costed: true });
  ...
  it("records manager-review conflicts when an offline sync would oversell stock", async () => {
    ...
    const conflictAudit = conn.query.mock.calls.find(
      ([sql, params]: any[]) =>
        String(sql).includes("INSERT INTO audit_events") &&
        params?.[2] === "offline.sync_conflict",
    );
    ...
    const conn = {
      ...
      query: vi.fn().mockImplementation((sql: string) => {
        if (sql.includes("SELECT id, name, wallet_balance")) {
          return Promise.resolve([
            [{ id: "cust_1", name: "Lebo", walletBalance: "100.00" }],
          ]);
        }
        return Promise.resolve([[]]);
      }),
    ...
    const conn = {
      ...
      query: vi.fn().mockImplementation((sql: string) => {
        if (sql.includes("FROM staff")) {
          return Promise.resolve([
            [{ id: "staff_1", name: "Jess", walletBalance: "90.00" }],
          ]);
        }
        return Promise.resolve([[]]);
      }),
  ...
  describe("transaction-safe checkout side effects", () => {
    ...
    it("persists QR provider reference metadata on sale payments", async () => {
      ...
      const paymentInsert = conn.query.mock.calls.find(([sql]: any[]) =>
        String(sql).includes("INSERT INTO sale_payments"),
      );
    ...
    it("records BNPL refund references and marks the original payment refunded", async () => {
      ...
      conn.query.mockImplementation((sql: string) => {
        if (
          sql.includes("FROM sales") &&
          sql.includes("WHERE tenant_id = $1 AND id = $2")
        ) {
          return Promise.resolve([
            [
              {
      ...
      expect(conn.query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          "refunded",
          "Refund approved in Mobicred portal",
          "sale_bnpl_1",
        ]),
      );
      expect(conn.commit).toHaveBeenCalled();
    ...
    it("updates sale payment provider reconciliation status with an audit event", async () => {
      ...
      (dbModule.query as any)
        ...
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([]);
      await updateSalePaymentProviderStatus(
        "tenant_1",
        "sale_bnpl_1",
        "pay_bnpl_1",
        {
    ...
    it("calculates earned loyalty points from active tiers before updating the customer balance", async () => {
      ...
      conn.query.mockImplementation((sql: string) => {
        ...
        if (
          sql.includes("SELECT bulk_item_id") ||
      ...
      expect(conn.query).toHaveBeenCalledWith(
        expect.stringContaining("UPDATE customers SET loyalty_points = $1"),
        [90, "tenant_1", "cust_1"],
      );
      expect(conn.query).toHaveBeenCalledWith(
    ...
    it("rolls back entire transaction when a side effect fails", async () => {
      ...
      await expect(createSale("tenant_1", sale as any)).rejects.toThrow(
        "DB lock timeout",
      );
      expect(conn.rollback).toHaveBeenCalled();
      expect(conn.commit).not.toHaveBeenCalled();
