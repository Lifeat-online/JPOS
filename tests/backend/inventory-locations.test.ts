...
import { cashierCanAccessLocation, completeStockTransferOrder, upsertProductLocationStock, } from '../../server/inventoryLocations.js';
...
describe('inventory locations', () => {
    ...
    it('updates per-location stock, aggregate product stock, movement, and audit rows', async () => {
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE product_location_stock'), expect.arrayContaining([8, 4, 6, 'mgr_1', 'Manager', 'tenant_1', 'prod_1', 'branch_1']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("UPDATE products SET stock = $1"), [8, 'tenant_1', 'prod_1']);
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO stock_movements'), expect.arrayContaining(['tenant_1', 'product', 'prod_1', null, 'Milk', 5, 3, 8, 'location_stock_adjustment']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO audit_events'), expect.arrayContaining(['tenant_1', 'inventory_location_stock.updated', 'product_location_stock']));
    ...
    it('completes transfer orders by moving stock between locations without changing aggregate quantity', async () => {
        ...
        conn.query.mockImplementation((sql: string, params?: any[]) => {
            ...
            if (sql.includes('FROM product_location_stock') && sql.includes("location_id = $3")) {
                const locationId = params?.[2];
                if (locationId === 'main')
                    return Promise.resolve([[{ quantity: 10, minStock: 5, reorderThreshold: 5 }]]);
        ...
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE product_location_stock'), expect.arrayContaining([6, 'mgr_1', 'Manager', 'tenant_1', 'prod_1', 'main']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE product_location_stock'), expect.arrayContaining([5, 'mgr_1', 'Manager', 'tenant_1', 'prod_1', 'branch_2']));
        expect(conn.query).toHaveBeenCalledWith(expect.stringContaining("SET status = 'completed'"), expect.arrayContaining(['mgr_1', 'Manager', 'tenant_1', 'transfer_1']));
        expect(conn.commit).toHaveBeenCalled();
    });
