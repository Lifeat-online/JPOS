...
function UserMenu({
    ...
    const isThisActiveTerminal = Boolean(companionState.companionDeviceId && companionState.activeTerminalDeviceId === companionState.companionDeviceId);
        ...
        <div className="relative" ref={ref}>
            ...
            >
                ...
                <div className="hidden sm:flex flex-col items-start">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{currentUserRole || 'staff'}</span>
                </div>
            </button>
...
function DesktopNav({
    ...
    return (
        <nav className="hidden lg:flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
            ...
            {/* Divider */}
            {secondaryNav.length > 0 && <div className="w-px h-5 bg-slate-300 dark:bg-slate-600 mx-1" />}
...
export default function App() {
    ...
    const { cart, setCart, setActiveCategory, setSelectedCustomerId, setActiveTableNumber, setActiveOrderId, selectedCustomerId, activeTableNumber } =
        usePosStore();
    ...
    const posCustomerProfiles = useMemo(() => buildPosCustomerProfiles(customers, staff), [customers, staff]);
    ...
    // Count open bar tabs
    const openTabsCount = useMemo(() => sales.filter((s) => s.isTab && s.status === 'open').length, [sales]);
    ...
    const { primaryNav, secondaryNav, navItems } = useMemo(() => {
        ...
        return {
            ...built,
            navItems: [...built.navItems, ...(isDev ? [{ id: 'dev' as const, icon: Code2, label: 'Dev' }] : [])],
        };
    ...
    return (
        <div className="h-screen w-full bg-slate-50 dark:bg-slate-950 flex flex-col font-sans overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
                ...
                {view === 'wallets' && <WalletAdminView staff={staff} customers={customers} currentUserStaff={currentUserStaff} />}
            ...
            <AnimatePresence>
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-800">
                                    ...
                                    <p className="text-xs text-blue-600 dark:text-blue-400">
                                        Click the install icon (⊕) in the address bar, or go to Menu → Install MasePOS
                                    </p>
                                </div>
                ...
                {checkout.splitPaymentModal && (
                    <SplitPaymentModal
                        ...
                        customerAccountRemaining={Math.max(
                            0,
                            Number(customers.find((c) => c.id === selectedCustomerId)?.accountLimit || 0) -
                                Number(customers.find((c) => c.id === selectedCustomerId)?.accountBalance || 0),
                        )}
                ...
                {checkout.checkoutModal.isOpen && checkout.checkoutModal.saleData && <Receipt sale={checkout.checkoutModal.saleData} config={config} />}

                {receiptToPrint && !checkout.checkoutModal.isOpen && <Receipt sale={receiptToPrint} config={config} />}
