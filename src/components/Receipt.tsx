        ...
        >
            ...
            <div className="text-center mb-4">
                ...
                {(sale as any).isOfflineSale && (
                    <p className="text-red-600 font-bold mt-1">{(sale as any).syncStatus === 'synced' ? 'SYNCED' : 'PENDING SYNC'}</p>
                )}
            ...
            <div className="space-y-1 mb-2">
                ...
                {sale.pointsDiscount !== undefined && sale.pointsDiscount > 0 && (
                    <div className="flex justify-between text-[11px]">
                        ...
                        <span>
                            -{currency}
                            {Number(sale.pointsDiscount || 0).toFixed(2)}
                        </span>
                    </div>
                )}
                ...
                {sale.payments && sale.payments.length > 0 ? (
                        ...
                        {sale.payments.map((p, idx) => (
                            <div key={idx} className="space-y-0.5">
                                <div className="flex justify-between text-[11px]">
                                    ...
                                    <span>
                                        {currency}
                                        {Number(p.tenderedAmount || p.amount).toFixed(2)}
                                    </span>
                                </div>
                                {(p.changeAmount ?? 0) > 0 && (
                                ...
                                {(p.cashOutAmount ?? 0) > 0 && (
                                    <div className="flex justify-between text-[11px] pl-2">
                                        ...
                                        <span>
                                            {currency}
                                            {Number(p.cashOutAmount).toFixed(2)}
                                        </span>
                                    </div>
                                )}
                                {p.providerReference && (
                    ...
                    <>
                        <div className="flex justify-between">
                            <span>TENDERED ({(sale.paymentMethod || 'Unknown').toUpperCase()})</span>
                            <span>
                                {currency}
                                {Number(sale.tenderedAmount || sale.total || 0).toFixed(2)}
                            </span>
                        </div>
                        {sale.changeAmount !== undefined && sale.changeAmount > 0 && (
                            <div className="flex justify-between font-bold">
                                ...
                                <span>
                                    {currency}
                                    {Number(sale.changeAmount || 0).toFixed(2)}
                                </span>
                            </div>
                        )}
                        {sale.tipAmount != null && Number(sale.tipAmount) > 0 && (
                            <div className="flex justify-between">
                                ...
                                <span>
                                    {currency}
                                    {Number(sale.tipAmount || 0).toFixed(2)}
                                </span>
                            </div>
                        )}
                    </>
            ...
            <div className="text-center mt-2 text-[11px]">
                {(sale as any).isOfflineSale && (
                    <p className="text-red-600 font-bold mb-1">OFFLINE SALE — {(sale as any).syncStatus === 'synced' ? 'Synced to server' : 'Pending sync'}</p>
                )}
