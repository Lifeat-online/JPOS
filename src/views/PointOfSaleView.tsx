...
export const PointOfSaleView: React.FC<PointOfSaleViewProps> = ({
    ...
    const drawerReasons = ['Make change', 'Check drawer', 'Cash drop', 'Manager count', 'Other'];

    const getCartQuantityForProduct = (productId: string) =>
        cart.reduce((sum, item) => (item.id === productId || (item as any).productId === productId ? sum + Number(item.quantity || 0) : sum), 0);
    ...
    const canAddProductToCart = (product: Product, showNotice = true) => {
        ...
        if (remainingAfterCart <= 0) {
            if (showNotice)
                setStockNotice({
                    type: 'error',
                    message: `Only ${stock} ${stock === 1 ? 'unit is' : 'units are'} available for ${product.name}. The cart already has ${inCart}.`,
                });
            return false;
        }
        ...
        if (stock <= minStock || remainingAfterCart - 1 <= minStock) {
            if (showNotice)
                setStockNotice({
                    type: 'warning',
                    message: `${product.name} is running low. ${Math.max(0, remainingAfterCart - 1)} will remain after this add.`,
                });
        } else if (showNotice) {
            setStockNotice(null);
        }
    ...
    const laybyTaxAmount = useMemo(() => {
        ...
        return laybyTaxInclusive ? cartTotal - cartTotal / (1 + laybyTaxRate / 100) : cartTotal * (laybyTaxRate / 100);
    ...
    const cashierUpsellPrompt = useMemo(() => {
        ...
        const demandWindow = hour < 11 ? 'morning demand' : hour < 15 ? 'lunch demand' : hour < 18 ? 'afternoon add-on' : 'evening basket';

        const ranked = products
            .filter((product) => {
                if (cartProductIds.has(product.id)) return false;
                const inCart = cart.reduce(
                    (sum, item) => (item.id === product.id || (item as any).productId === product.id ? sum + Number(item.quantity || 0) : sum),
                    0,
                );
    ...
    const selectedTableLabel =
        activeRestaurantTables.find((t) => t.id === (activeTableNumber || selectedTableForOrder))?.label || activeTableNumber || selectedTableForOrder;
    ...
    const openTableCount = useMemo(() => {
        const tableIds = new Set(
            sales.filter((s) => Boolean(s.tableNumber) && (s.status === 'open' || s.status === 'kitchen')).map((s) => String(s.tableNumber)),
        );
        return tableIds.size;
    }, [sales]);
    const openTabsCount = useMemo(() => sales.filter((s) => s.isTab && (s.status === 'open' || s.status === 'kitchen')).length, [sales]);
    ...
    const blockingStockIssues = useMemo(
        () =>
            cart
                .map((item) => {
    ...
    }, [companionMode, assignedCompanionMode, poleDisplayDeviceId, companionDeviceId, terminalId, currentUserStaff?.name, longTermAssignment, displaySnapshot]);
    ...
    const attachedQueueCount = useMemo(() => {
        ...
        return sales.reduce((count, sale) => {
            ...
            return (
                ...
                sale.items.filter((item) => {
                    const orderItem = item as any;
                    return orderItem.workstationId === attachedWorkstationId && (orderItem.status === 'pending' || orderItem.status === 'accepted');
                }).length
            );
        }, 0);
    ...
    const submitWalletCashMovement = async () => {
        ...
        try {
            ...
            setWalletCashSuccess(
                `${walletCashDirection === 'in' ? 'Top-up' : 'Payout'} recorded. New wallet balance: R${Number(result.nextBalance || 0).toFixed(2)}.`,
            );
            setWalletCashAmount('');
            setWalletCashNote('');
            setTimeout(() => {
    ...
    const categoriesForSection = useMemo(() => {
        if (activeSection === 'All') {
            const categories = allowedSections.filter((sec) => sec !== 'All').flatMap((sec) => Object.keys(categoryTree[sec] || {}));
    ...
    const categoryProductCount = (category: string) =>
        products.filter((p) => {
    ...
    const quickActionBase =
        'min-h-[76px] rounded-2xl border bg-white p-3 text-left shadow-sm transition-all active:scale-[0.98] disabled:opacity-45 dark:bg-slate-900';
    ...
    const hasOfflineRecovery =
        offlineStatus.isOffline || offlineStatus.pendingCount > 0 || offlineStatus.syncStatus === 'error' || Boolean(offlineStatus.lastError);
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-50 dark:bg-slate-950">
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-10 max-w-md w-full shadow-2xl text-center border border-slate-100 dark:border-slate-800/60">
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-primary text-white rounded-xl font-bold uppercase tracking-widest text-sm shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all"
                    >
                        Open Register
                    </button>
                </div>
    ...
    return (
        <div className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-hidden relative">
            {cart.length > 0 && !suppressBillPrint && (
                <BillPrint cart={cart} customer={selectedCustomer} config={config} subtotal={cartTotal} discount={totalDiscount} />
            )}
                ...
                <nav className="w-full lg:w-28 bg-white dark:bg-slate-900 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-700/60 flex lg:flex-col items-center py-2 lg:py-5 px-4 lg:px-2 gap-3 overflow-x-auto no-scrollbar shrink-0 shadow-sm lg:shadow-none z-10">
                    {allowedSections.map((section) => (
                        <button
                            ...
                            className={`min-w-21 lg:w-full h-14 lg:h-18 rounded-xl flex flex-col items-center justify-center gap-1 transition-all border-2 shrink-0 ${
                        ...
                        >
                            ...
                            <span className="max-w-full px-1 text-center text-[9px] lg:text-[10px] font-bold uppercase tracking-wide leading-tight truncate">
                                {section}
                            </span>
                        </button>
                    ))}
                    ...
                    {allowedCategories.map((cat) => (
                        <button
                            ...
                            className={`min-w-20 lg:w-16 h-12 lg:h-16 rounded-xl flex flex-col items-center justify-center gap-1 transition-all border-2 shrink-0 ${
                                activeCategory === cat
                                    ? 'bg-[#eff6ff] text-primary border-[#bfdbfe] shadow-sm'
                                    : 'text-slate-400 dark:text-slate-500 border-transparent bg-slate-50 dark:bg-[#0B1120] lg:bg-transparent lg:dark:bg-transparent hover:bg-slate-100 dark:hover:bg-slate-800'
                            }`}
            ...
            <section className="flex-1 flex flex-col min-w-0 min-h-0 bg-slate-50/30">
                <div className="p-4 lg:p-6 pb-2 flex flex-col sm:flex-row gap-3 lg:gap-4 shrink-0">
                    ...
                    <div className="flex h-12 lg:h-13.5 shrink-0 rounded-xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 p-1 shadow-sm">
                        <button
                            type="button"
                        ...
                        <button
                            type="button"
                            onClick={() => {
                                setTerminalCategoryLayout('grid');
                                setGridShowingProducts(false);
                            }}
                    ...
                    <div role="group" aria-label="Daily POS actions" className="mx-4 lg:mx-6 mb-2 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-7">
                        ...
                        >
                            ...
                            <span className="mt-0.5 block text-[10px] font-bold text-slate-400">
                                {attachedWorkstation ? attachedWorkstation.name : 'Workstations'}
                            </span>
                        </button>
                    </div>
                )}
                ...
                {hasRecoveryWork && (
                    <section
                        aria-label="POS recovery actions"
                        className="mx-4 lg:mx-6 mb-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900"
                    >
                        ...
                        <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
                                ...
                                <article className="rounded-xl border border-blue-100 bg-blue-50 p-3 dark:border-blue-900/40 dark:bg-blue-950/20">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="mt-0.5 text-xs font-semibold text-blue-700/80 dark:text-blue-300/70">
                                                Wallet, account, and loyalty flows need a linked profile.
                                            </p>
                                        </div>
                                    </div>
                                    <button
                            ...
                            {hasOfflineRecovery && (
                                <article className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/50">
                                    <div className="flex items-start gap-3">
                                        <RefreshCw
                                            className={`mt-0.5 h-5 w-5 shrink-0 text-slate-600 dark:text-slate-300 ${offlineStatus.syncStatus === 'syncing' ? 'animate-spin' : ''}`}
                                        />
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="mt-0.5 text-xs font-semibold text-slate-500">
                                                {offlineStatus.lastError ||
                                                    `${offlineStatus.pendingCount || 0} queued sale${offlineStatus.pendingCount === 1 ? '' : 's'}`}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="mt-3 grid grid-cols-2 gap-2">
                            ...
                            {needsPrinterRecovery && <PrinterReadinessPanel tenantId={tenantId} compact readiness={printerReadiness} />}
                        </div>
                    </section>
                )}
                ...
                {stockNotice && (
                    <div
                        className={`mx-4 lg:mx-6 mb-2 rounded-2xl border p-3 flex items-start justify-between gap-3 ${
                            ...
                            {parkedSales.slice(0, 8).map((sale) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className="min-w-47.5 rounded-xl bg-white dark:bg-slate-900 border border-indigo-100 dark:border-indigo-900/40 p-3 text-left shadow-sm active:scale-95 transition-all"
                                    >
                    ...
                    <div className="mx-4 lg:mx-6 mb-2 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-900/40 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <p className="text-xs font-black uppercase tracking-widest text-orange-600 dark:text-orange-400">
                                Use this register as a workstation?
                            </p>
                            <p className="text-xs text-orange-700/70 dark:text-orange-300/70 mt-0.5">
                                Choose a station to see and manage incoming tickets beside the register.
                            </p>
                        </div>
                        <div className="flex gap-2 overflow-x-auto no-scrollbar">
                            <button
                        ...
                        <div className="mb-4 flex items-center justify-between gap-3">
                            <div>
                                ...
                                <h3 className="text-lg font-black text-slate-900 dark:text-white">
                                    {activeSection === 'All' ? 'Choose a section' : 'Choose a category'}
                                </h3>
                            </div>
                            {activeSection !== 'All' && (
                                <button
                            ...
                            {activeSection === 'All' ? (
                                allowedSections
                                    ...
                                    .map((section) => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => selectSection(section)}
                                            className="min-h-33 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 p-5 text-left shadow-sm transition-all hover:border-primary/40 hover:shadow-md active:scale-[0.99]"
                                        >
                                            <div className="mb-5 w-12 h-12 rounded-xl bg-[#eff6ff] text-primary flex items-center justify-center text-xl">
                                            ...
                                            <div className="mt-1 text-[10px] font-black uppercase tracking-widest text-slate-400">
                                                {sectionProductCount(section)} products
                                            </div>
                                        </button>
                                    ))
                            ) : (
                                <>
                                    <button
                                        ...
                                        className="min-h-33 rounded-2xl border border-primary/20 bg-primary/5 p-5 text-left shadow-sm transition-all hover:border-primary/50 active:scale-[0.99]"
                                    >
                                        <div className="mb-5 w-12 h-12 rounded-xl bg-primary text-white flex items-center justify-center">
                                        ...
                                        <div className="mt-1 text-[10px] font-black uppercase tracking-widest text-primary">
                                            {categoryProductCount('All')} products
                                        </div>
                                    </button>
                                    {allowedCategories
                                        .filter((cat) => cat !== 'All')
                                        .map((cat) => (
                                            <button
                                                ...
                                                className="min-h-33 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 p-5 text-left shadow-sm transition-all hover:border-primary/40 hover:shadow-md active:scale-[0.99]"
                                            >
                                                <div className="mb-5 w-12 h-12 rounded-xl bg-slate-50 dark:bg-[#0B1120] text-xl flex items-center justify-center">
                                                    {getCategoryIcon(cat)}
                                                </div>
                                                <div className="font-black text-slate-900 dark:text-white truncate">{cat}</div>
                                                <div className="mt-1 text-[10px] font-black uppercase tracking-widest text-slate-400">
                                                    {categoryProductCount(cat)} products
                                                </div>
                                            </button>
                                        ))}
                                </>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="lg:hidden flex items-baseline gap-2 mt-1">
                                                ...
                                                <span
                                                    className={`text-[8px] font-black ${isOutOfStock ? 'text-rose-500' : isLowStock ? 'text-amber-500' : 'text-slate-300 dark:text-slate-600'}`}
                                                >
                                        ...
                                        <div className="hidden lg:flex items-end justify-between w-full mt-2">
                                            ...
                                            <div
                                                className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${isOutOfStock ? 'bg-rose-50 text-rose-600' : isLowStock ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}
                                            >
                                                {isOutOfStock ? 'Out' : `${remainingAfterCart} left`}
                                            </div>
                                        </div>
                                        <div className="absolute top-2 right-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1.5">
                                            <div
                                                className={`p-1.5 rounded-lg transition-all shadow-sm ${isOutOfStock ? 'bg-rose-50 text-rose-400' : 'bg-primary/10 text-primary hover:bg-primary hover:text-white'}`}
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleAddToCart(product);
                                                }}
            ...
            {attachedWorkstation && (
                <aside className="hidden 2xl:flex w-85 border-l border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-[#0B1120] shrink-0 min-h-0">
                ...
                {(isCartOpen || window.innerWidth >= 1024) && (
                    <>
                        ...
                        <motion.aside
                            ...
                            className={`fixed bottom-0 left-0 right-0 lg:relative lg:inset-auto z-50 lg:z-10 w-full lg:w-90 max-h-[90vh] lg:max-h-none bg-white dark:bg-slate-900 lg:border-l border-slate-200 dark:border-slate-700/60 flex flex-col shrink-0 shadow-2xl rounded-t-3xl lg:rounded-none overflow-hidden`}
                        >
                            ...
                            {!attachedWorkstation || sidePanelMode === 'cart' || isWideRegister ? (
                                <>
                                    <div className="p-5 border-b border-slate-200 dark:border-slate-700/60 flex justify-between items-center bg-white dark:bg-slate-900 sticky top-0 z-10">
                                            ...
                                            {activeOrderId && (
                                                <div className="flex items-center gap-1.5 mt-1">
                                                    ...
                                                    <span className="text-[10px] font-black text-orange-500 uppercase tracking-widest">
                                                        Order Sent · #{activeOrderId.slice(-6).toUpperCase()}
                                                    </span>
                                                </div>
                                            )}
                                            {selectedCustomerId && (
                                                <div className="flex flex-col gap-1 mt-1">
                                                    ...
                                                    {(selectedCustomer?.accountEnabled || selectedCustomerAccountBalance > 0) && (
                                                        <div className="text-[9px] font-black text-amber-600 dark:text-amber-400">
                                                            Account: R{selectedCustomerAccountBalance.toFixed(2)} owing / R
                                                            {selectedCustomerAccountLimit.toFixed(2)} limit
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                        ...
                                        <button
                                            onClick={() => setIsCartOpen(false)}
                                            className="lg:hidden p-2 bg-slate-50 dark:bg-[#0B1120] rounded-full text-slate-400 hover:text-slate-600 dark:text-slate-300"
                                        >
                                            <X className="w-5 h-5" />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex-1 min-h-0 overflow-y-auto bg-slate-50/50 dark:bg-slate-950/50">
                                        <div className="px-5 py-4 space-y-4 min-h-40">
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            {offlineStatus.lastSummary?.message && (
                                                                <p className="mt-1 text-[10px] font-black uppercase tracking-widest opacity-80">
                                                                    {offlineStatus.lastSummary.message}
                                                                    {offlineStatus.lastSummary.nextRetryAt
                                                                        ? ` / next retry ${formatOfflineRetryTime(offlineStatus.lastSummary.nextRetryAt)}`
                                                                        : ''}
                                                                </p>
                                                            )}
                                                            <div className="mt-3 flex flex-wrap gap-2">
                                                                ...
                                                                {!offlineStatus.isOffline && offlineStatus.pendingCount > 0 && (
                                                                    ...
                                                                    >
                                                                        <RefreshCw
                                                                            className={`h-3.5 w-3.5 ${offlineStatus.syncStatus === 'syncing' ? 'animate-spin' : ''}`}
                                                                        />
                                                                        Sync
                                                                    </button>
                                                                )}
                                                    ...
                                                    {offlineReviewOpen && visibleOfflineItems.length > 0 && (
                                                        <div className="mt-3 space-y-2">
                                                            {visibleOfflineItems.map((item) => (
                                                                <div
                                                                    key={item.id}
                                                                    className="rounded-xl border border-white/60 bg-white/70 p-2 text-slate-700 shadow-sm dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-200"
                                                                >
                                                                    <div className="flex items-start justify-between gap-2">
                                                                        <div className="min-w-0">
                                                                            ...
                                                                            <p className="mt-0.5 text-[10px] font-bold text-slate-500 dark:text-slate-400">
                                                                                R{Number(item.saleData?.total || 0).toFixed(2)} / {item.method} /{' '}
                                                                                {item.attempts} attempt{item.attempts === 1 ? '' : 's'}
                                                                                {item.syncBatchId
                                                                                    ? ` / batch ${String(item.syncSequence || 0).padStart(2, '0')}`
                                                                                    : ''}
                                                                            </p>
                                                                        </div>
                                                                        <span
                                                                    ...
                                                                    {item.lastError && (
                                                                        <p className="mt-2 wrap-break-word text-[10px] font-bold leading-snug text-rose-600 dark:text-rose-300">
                                                                            {item.lastError}
                                                                        </p>
                                                                    )}
                                            ...
                                            {cart.length === 0 ? (
                                                <div className="min-h-40 flex flex-col items-center justify-center text-slate-300 dark:text-slate-600 opacity-50 space-y-2 text-center p-8 grayscale">
                                                ...
                                                cart.map((item, idx) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={cartId}
                                                            className={`bg-white dark:bg-slate-900 p-4 rounded-2xl flex items-center justify-between border shadow-sm transition-all ${isCartItemOutOfStock ? 'border-rose-200 dark:border-rose-900/50' : 'border-slate-100 dark:border-slate-800/60 hover:border-primary/20'}`}
                                                        >
                                                            <div className="flex-1 pr-4 min-w-0">
                                                                ...
                                                                {modifiers.length > 0 && (
                                                                    <div className="flex flex-wrap gap-1 mt-1">
                                                                        {modifiers.map((m: any) => (
                                                                            <span
                                                                                key={m.optionId}
                                                                                className="text-[8px] font-bold bg-slate-50 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded-md"
                                                                            >
                                                                                + {m.name}
                                                                            </span>
                                                                        ))}
                                                                ...
                                                                {isCartItemOutOfStock ? (
                                                                    <p className="mt-1 text-[10px] font-black text-rose-600 uppercase tracking-widest">
                                                                        Stock issue: {stock <= 0 ? 'out of stock' : `${stock} available`}
                                                                    </p>
                                                            ...
                                                            <div className="flex items-center gap-4 bg-slate-50 dark:bg-[#0B1120] rounded-xl p-1 shrink-0">
                                                                <button
                                                                    ...
                                                                    className="w-8 h-8 flex items-center justify-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 rounded-lg text-xs font-black shadow-sm active:scale-90"
                                                                >
                                                                    -
                                                                </button>
                                        ...
                                        <div className="p-5 lg:p-6 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800/60 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
                                            {config?.business?.isRestaurantMode && activeTableNumber && (
                                                <div className="flex justify-between items-center mb-4 p-3 bg-primary/10 rounded-xl border border-primary/20">
                                                    <span className="font-bold text-primary flex items-center gap-2">
                                                        <Utensils className="w-4 h-4" /> Table {activeTableNumber}
                                                    </span>
                                                    <button
                                                        onClick={() => {
                                                            setActiveTableNumber(null);
                                            ...
                                            {(selectedCustomer?.accountEnabled || selectedCustomerAccountBalance > 0) && (
                                                <div
                                                    className={`mx-5 mb-4 rounded-2xl border p-3 ${
                                                        ...
                                                        <div className="w-full">
                                                            ...
                                                            {selectedCustomerAccountRemaining < amountDue && (
                                                                <p className="mt-2 text-[10px] font-bold">
                                                                    This order is above the remaining account allowance.
                                                                </p>
                                                            )}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-start gap-3">
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            <p className="mt-1 truncate text-sm font-black text-slate-900 dark:text-white">
                                                                {cashierUpsellPrompt.product.name}
                                                            </p>
                                                            <div className="mt-2 flex flex-wrap gap-1.5">
                                                                {cashierUpsellPrompt.reasons.map((reason) => (
                                                                    <span
                                                                        key={reason}
                                                                        className="rounded-lg bg-white px-2 py-1 text-[9px] font-black uppercase tracking-widest text-slate-500 shadow-sm dark:bg-slate-900 dark:text-slate-300"
                                                                    >
                                                                        {reason}
                                                                    </span>
                                                                ))}
                                            ...
                                            <div className="flex justify-between items-center mb-6">
                                                <div>
                                                    <span className="font-bold text-slate-400 dark:text-slate-500 text-xs uppercase tracking-widest block">
                                                        Grand Total
                                                    </span>
                                                    {selectedCustomerId &&
                                                        (() => {
                                                                ...
                                                                <div className="flex items-center gap-2 mt-1">
                                                                    {pointsDiscount > 0 ? (
                                                                        ...
                                                                        >
                                                                            Redeem {pts} pts → −R
                                                                            {Math.min(
                                                                                Math.floor(pts / config!.business!.pointsRequiredForDiscount!) *
                                                                                    config!.business!.discountAmountForPoints!,
                                                                                cartTotal,
                                                                            ).toFixed(2)}
                                                                        </button>
                                                                    ) : (
                                                                        <span className="text-[9px] font-bold text-slate-400">
                                                                            {pts} pts (need {config?.business?.pointsRequiredForDiscount} to redeem)
                                                                        </span>
                                                                    )}
                                                                </div>
                                                            ) : null;
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <span className="font-black text-4xl text-slate-900 dark:text-white tracking-tighter">
                                                        R{Number(amountDue).toFixed(2)}
                                                    </span>
                                                    {totalDiscount > 0 && (
                                            ...
                                            <div className="grid grid-cols-2 gap-3 mb-4">
                                                <button
                                                    disabled={isProcessing || cart.length === 0 || hasBlockingStockIssues}
                                                    onClick={() => {
                                                        setTenderModal({ isOpen: true, method: 'cash' });
                                                        setTenderedAmount('');
                                                    }}
                                                ...
                                                <button
                                                    disabled={isProcessing || cart.length === 0 || hasBlockingStockIssues}
                                                    onClick={() => {
                                                        setTenderModal({ isOpen: true, method: 'card' });
                                                        setTenderedAmount('');
                                                    }}
                                                ...
                                                <button
                                                    ...
                                                    title={
                                                        offlineStatus.isOffline
                                                            ? 'QR payments require online provider confirmation'
                                                            : 'Capture QR or mobile-wallet payment'
                                                    }
                                                ...
                                                <button
                                                    ...
                                                    title={
                                                        offlineStatus.isOffline
                                                            ? 'BNPL payments require online provider approval'
                                                            : 'Capture PayJustNow, Mobicred, or PayFlex payment'
                                                    }
                                            ...
                                            {canPayWithCustomerWallet && (
                                                <button
                                                    disabled={
                                                        ...
                                                        cart.length === 0 ||
                                                        hasBlockingStockIssues ||
                                                        selectedCustomerWalletBalance < amountDue ||
                                                        walletOnlineRequired
                                                    }
                                            ...
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={() => clearCart()}
                                                    title="Clear Cart"
                                                    className="h-14 w-14 bg-slate-50 dark:bg-[#0B1120] text-red-500 rounded-2xl flex items-center justify-center hover:bg-red-50 transition-all border border-slate-100 dark:border-slate-800/60 active:scale-95 shrink-0"
                                                >
                                                    <Trash2 className="w-5 h-5" />
                                                </button>
                                                <button
                                                    title="Add Note"
                                                    className="flex-1 h-14 bg-slate-50 text-slate-400 dark:text-slate-500 rounded-2xl flex items-center justify-center hover:bg-slate-100 dark:bg-slate-800 transition-all border border-slate-100 dark:border-slate-800/60 active:scale-95 gap-2"
                                                >
                                                ...
                                                <button
                                                    onClick={openDrawerModal}
                                                    title="Open drawer with reason"
                                                    className="flex-1 h-14 bg-slate-50 dark:bg-[#0B1120] text-emerald-500 rounded-2xl flex items-center justify-center hover:bg-emerald-50 hover:border-emerald-100 transition-all border border-slate-100 dark:border-slate-800/60 active:scale-95 gap-2"
                                                >
                                        ...
                                        <div className="p-5 space-y-4">
                                            <div className="flex items-center gap-4">
                                                <div className="w-20 h-20 rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-950 shrink-0 border border-slate-100 dark:border-slate-800">
                                                    <img
                                                        src={getProductImage(priceCheckProduct)}
                                                        alt={priceCheckProduct.name}
                                                        className="w-full h-full object-cover"
                                                        referrerPolicy="no-referrer"
                                                    />
                                                </div>
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="mt-1 text-xs font-bold text-slate-500 truncate">
                                                        {priceCheckProduct.barcode || 'No barcode saved'}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-3 gap-3">
                                                <div className="rounded-2xl bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 p-3">
                                                    ...
                                                    <p
                                                        className={`mt-1 text-lg font-black ${isOut ? 'text-rose-600' : isLow ? 'text-amber-600' : 'text-slate-900 dark:text-white'}`}
                                                    >
                                                        {stock}
                                                    </p>
                                                </div>
                                                <div className="rounded-2xl bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 p-3">
                                                    ...
                                                    <p className="mt-1 text-lg font-black text-slate-900 dark:text-white">
                                                        {getCartQuantityForProduct(priceCheckProduct.id)}
                                                    </p>
                                                </div>
                                                <div className="rounded-2xl bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 p-3">
                                                    ...
                                                    <p
                                                        className={`mt-1 text-lg font-black ${isOut ? 'text-rose-600' : isLow ? 'text-amber-600' : 'text-slate-900 dark:text-white'}`}
                                                    >
                                                        {Math.max(0, remainingAfterCart - 1)}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            >
                                                {isOut
                                                    ? 'This item cannot be added because available stock is already used.'
                                                    : isLow
                                                      ? 'This item can be sold, but stock is running low.'
                                                      : 'This item is available to sell.'}
                                            </div>
                        ...
                        >
                            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p className="text-xs font-medium text-slate-500 mt-0.5">
                                        The register clears immediately so you can help the next customer.
                                    </p>
                                </div>
                                <button
                                    onClick={() => setParkModalOpen(false)}
                            ...
                            <div className="p-5 space-y-4">
                                {parkError && (
                                    <div className="rounded-2xl bg-rose-50 border border-rose-200 p-3 text-sm font-bold text-rose-700">{parkError}</div>
                                )}
                                ...
                                <div className="rounded-2xl bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 p-4">
                                    ...
                                    <p className="mt-1 text-xs font-semibold text-slate-500">
                                        {cart.length} item{cart.length === 1 ? '' : 's'} will be parked.
                                    </p>
                                </div>
                ...
                {walletCashModalOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="p-5 space-y-4">
                                {walletCashError && (
                                    <div className="rounded-2xl bg-rose-50 border border-rose-200 p-3 text-sm font-bold text-rose-700">{walletCashError}</div>
                                )}
                                {walletCashSuccess && (
                ...
                {drawerModalOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="p-5 space-y-4">
                                {drawerError && (
                                    <div className="rounded-2xl bg-rose-50 border border-rose-200 p-3 text-sm font-bold text-rose-700">{drawerError}</div>
                                )}
                                    ...
                                    >
                                        {activeRestaurantTables.map((table) => {
                                            const activeSale = sales.find(
                                                (s) => s.tableNumber === table.id && (s.status === 'open' || s.status === 'kitchen') && s.id !== activeOrderId,
                                            );
                                            return (
