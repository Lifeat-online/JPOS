                ...
                {tabs.map(({ id, label, icon: Icon }) => (
                    ...
                    >
                        ...
                        {id === 'wallet' && pendingPayouts.length > 0 && (
                            <span className="min-w-4.5 h-4.5 px-1 rounded-full bg-amber-500 text-white text-[9px] font-black flex items-center justify-center">
            ...
            <main className="flex-1 overflow-y-auto p-4 lg:p-8">
                <div className="max-w-3xl mx-auto space-y-6">
                            ...
                            {completedSales.length > 0 && (
                                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                                    ...
                                    {completedSales.slice(0, 3).map((sale) => (
                                        <div
                                            key={sale.id}
                                            className="flex items-center justify-between px-5 py-3.5 border-b border-slate-50 dark:border-slate-800/50 last:border-0"
                                        >
                                            <div>
                                        ...
                                        >
                                            <div className="text-left">
                                                <div className="flex items-center gap-2 mb-0.5">
                                                    ...
                                                    <span
                                                        className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest ${statusColors[sale.status] || statusColors.pending}`}
                                                    >
                                                        {sale.status}
                                                    </span>
                                                </div>
                                                <p className="text-xs text-slate-400">
                                                    {formatDate(sale.createdAt)} · {(sale.paymentMethod || 'Unknown').toUpperCase()}
                                                </p>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <p className="font-black text-slate-900 dark:text-white">{formatCurrency(sale.total)}</p>
                    ...
                    {activeTab === 'wallet' && (
                        <div className="space-y-5">
                            {/* Balance card */}
                            <div className="bg-linear-to-br from-primary to-blue-700 rounded-3xl p-6 text-white shadow-xl shadow-primary/30">
                            ...
                            {payoutRequests.length > 0 && (
                                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                                    ...
                                    {payoutRequests.map((req, idx) => (
                                        <div
                                            key={req.id}
                                            className={`flex items-center justify-between px-5 py-3.5 gap-4 ${idx > 0 ? 'border-t border-slate-50 dark:border-slate-800/50' : ''}`}
                                        >
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span
                                                        className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest ${statusColors[req.status] || statusColors.pending}`}
                                                    >
                                                        {req.status}
                                                    </span>
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                                <div className="flex items-center gap-4 mb-6">
                                    <img
                                        src={
                                            user.photoURL ||
                                            `https://ui-avatars.com/api/?name=${encodeURIComponent(customer.name)}&background=2563EB&color=fff&size=80`
                                        }
                                        alt="avatar"
