...
function normalizeDeviceType(value: unknown): HardwareDeviceType {
    ...
    if (DEVICE_TYPE_SET.has(text))
        return text as HardwareDeviceType;
    if (text === "printer" || text === "receipt")
        return "receipt_printer";
    if (text === "kitchen" || text === "kitchen_ticket")
        return "kitchen_printer";
    if (text === "drawer")
        return "cash_drawer";
    if (text === "scanner")
        return "barcode_scanner";
    if (text === "display" || text === "customer_display")
        return "pole_display";
    if (text === "terminal" || text === "payment_terminal")
...
function normalizeConnectionType(value: unknown, deviceType: HardwareDeviceType): HardwareConnectionType {
    const text = clean(value, 80).toLowerCase().replace(/[\s-]+/g, "_");
    if (CONNECTION_TYPE_SET.has(text))
        return text as HardwareConnectionType;
    if (!text) {
        if (deviceType === "receipt_printer")
            return "browser_print";
        if (deviceType === "barcode_scanner")
            return "keyboard_wedge";
        if (deviceType === "card_terminal")
            return "payment_provider";
        return "local_bridge";
    }
    if (text === "network" || text === "tcp" || text === "lan")
...
function connectionReadiness(device: HardwareDevice) {
    ...
    if (device.connectionType === "escpos_network" && !config.host)
        return { ok: false, message: "Network ESC/POS devices need a host/IP address." };
    if (device.connectionType === "local_bridge" && !config.bridgeUrl)
        return { ok: false, message: "Local bridge devices need a bridge URL." };
...
async function recordHardwareDeviceEvent(tenantId: string, deviceId: string | null, eventType: string, commandType: string, status: HardwareEventStatus, requestPayload: unknown, responsePayload: unknown, errorMessage: string | null, actor: HardwareActor) {
    const eventId = id("hwe");
    await query(`INSERT INTO hardware_device_events (
...
export async function listHardwareDevices(tenantId: string, filters: {
    ...
    const rows = await query<any>(`SELECT *
      ...
      ORDER BY device_type ASC, is_default DESC, name ASC`, params);
...
export async function createHardwareDevice(tenantId: string, input: any, actor: HardwareActor = {}) {
    ...
    if (normalized.isDefault) {
        await query(`UPDATE hardware_devices SET is_default = 0 WHERE tenant_id = $1 AND device_type = $2`, [tenantId, normalized.deviceType]);
    }
    await query(`INSERT INTO hardware_devices (
...
export async function updateHardwareDevice(tenantId: string, deviceId: string, input: any, actor: HardwareActor = {}) {
    ...
    if (normalized.isDefault) {
        await query(`UPDATE hardware_devices SET is_default = 0 WHERE tenant_id = $1 AND device_type = $2 AND id <> $3`, [tenantId, normalized.deviceType, deviceId]);
    }
    await query(`UPDATE hardware_devices
        SET name = $1,
            device_type = $2,
            connection_type = $3,
            status = $4,
...
export async function deleteHardwareDevice(tenantId: string, deviceId: string, actor: HardwareActor = {}) {
    await query(`DELETE FROM hardware_devices WHERE tenant_id = $1 AND id = $2`, [tenantId, deviceId]);
    await recordAuditEventSafe({
        tenantId,
...
export async function testHardwareDevice(tenantId: string, deviceId: string, actor: HardwareActor = {}, context: any = {}) {
    ...
    const eventId = await recordHardwareDeviceEvent(tenantId, device.id, "hardware.test", command.commandType, ready ? "sent" : "failed", { context }, response, ready ? null : response.message, actor);
    await query(`UPDATE hardware_devices
        SET last_check_status = $1,
            last_check_message = $2,
            last_checked_at = NOW(),
            updated_at = NOW()
      WHERE tenant_id = $3 AND id = $4`, [ready ? "ready" : "attention", response.message, tenantId, device.id]);
...
export async function queueKitchenPrintJobsForSale(tenantId: string, sale: any, actor: HardwareActor = {}) {
    ...
    for (const [workstationId, items] of groups.entries()) {
        ...
        if (!device) {
            const eventId = await recordHardwareDeviceEvent(tenantId, null, "hardware.kitchen_ticket", "escpos_kitchen_ticket", "skipped", { saleId: sale?.id || null, workstationId, itemCount: items.length }, { message: "No kitchen printer configured for workstation." }, "No kitchen printer configured for workstation.", actor);
        ...
        const eventId = await recordHardwareDeviceEvent(tenantId, device.id, "hardware.kitchen_ticket", command.commandType, command.readiness?.ok ? "queued" : "failed", { saleId: sale?.id || null, workstationId, itemCount: items.length }, { command, message: command.readiness?.message }, command.readiness?.ok ? null : command.readiness?.message || "Kitchen printer is not ready.", actor);
...
export async function queueCashDrawerPulseForNoSale(tenantId: string, actor: HardwareActor = {}, context: any = {}) {
    ...
    if (!device)
        return null;
    const command = buildHardwareCommand(device, context);
    const eventId = await recordHardwareDeviceEvent(tenantId, device.id, "hardware.cash_drawer_pulse", command.commandType, command.readiness?.ok ? "queued" : "failed", {
        cashSessionId: context.cashSessionId || null,
        movementId: context.movementId || null,
        reason: context.reason || null,
    }, { command, message: command.readiness?.message }, command.readiness?.ok ? null : command.readiness?.message || "Cash drawer is not ready.", actor);
