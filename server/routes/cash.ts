...
import { createPayoutRequest, updatePayoutRequest, updateCustomerPayoutRequest, createCustomerPayoutRequest, } from "../db-crud.js";
import { getManagerCashSummary, getManagerCashMovements, exportManagerCashMovementsCsv, recordManagerCashMovement, createCashCustodyTransfer, getCashCustodyTransfers, confirmCashCustodyTransfer, cancelCashCustodyTransfer, getCashClosePreview, getCashCloseCheckpoints, createCashCloseCheckpoint, exportCashCloseCheckpointCsv, recordWalletCashMovement, recordRegisterWalletCashMovement, transferCashSessionToManagerFloat, } from "../managerCash.js";
...
async function recordCashMovement(tenantId: string, data: any) {
    ...
    await query(`INSERT INTO cash_movements (
...
function expectedCashDeltaForMovement(type: string, direction: string, amount: number) {
    ...
    if (type === "cash_added")
        return amount;
    if (type === "cash_drop" || type === "cash_removed")
        return -amount;
    if (direction === "in")
        return amount;
    if (direction === "out")
...
async function getSessionRecordedTips(tenantId: string, sessionId: string, closedAt: Date) {
    ...
    WHERE cs.tenant_id = $3 AND cs.id = $4
    LIMIT 1`, [closedAt, closedAt, tenantId, sessionId]);
...
cashRouter.get("/cash-sessions", optionalAuth, async (req: any, res) => {
    try {
        ...
        const rows = await query("SELECT * FROM cash_sessions WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2", [req.params.tenantId, limit]);
...
cashRouter.get("/cash-sessions/:id/movements", requireAuth, async (req: any, res) => {
    try {
      ...
      ORDER BY created_at ASC`, [req.params.tenantId, req.params.id]);
...
cashRouter.post("/cash-sessions", requireAuth, async (req: any, res) => {
    try {
        const activeRegisterRows = await query<any>("SELECT COUNT(*) AS active_count FROM cash_sessions WHERE tenant_id = $1 AND status = 'open'", [req.params.tenantId]);
        ...
        await query(`INSERT INTO cash_sessions (
...
cashRouter.post("/cash-sessions/:id/movements", requireAuth, async (req: any, res) => {
    try {
        ...
        if (delta !== 0) {
            await query(`UPDATE cash_sessions SET expected_cash = COALESCE(expected_cash, 0) + $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3 AND status = 'open'`, [delta, req.params.tenantId, req.params.id]);
        }
        res.json(movement);
    }
    catch (err: any) {
...
cashRouter.put("/cash-sessions/:id", requireAuth, async (req: any, res) => {
    try {
        ...
        if (["reviewed", "reconciled", "disputed"].includes(updates.reviewStatus) &&
        ...
        if (updates.submittedAt !== undefined) {
            fields.push("submitted_at = $1");
            values.push(updates.submittedAt ? new Date(updates.submittedAt) : null);
        }
        if (updates.reviewedAt !== undefined) {
            fields.push("reviewed_at = $1");
            values.push(updates.reviewedAt ? new Date(updates.reviewedAt) : null);
        }
        if (updates.reviewedBy !== undefined) {
            fields.push("reviewed_by = $1");
            values.push(updates.reviewedBy || null);
        }
        if (updates.reconciledAt !== undefined) {
            fields.push("reconciled_at = $1");
            values.push(updates.reconciledAt ? new Date(updates.reconciledAt) : null);
        }
        if (updates.reconciledBy !== undefined) {
            fields.push("reconciled_by = $1");
            values.push(updates.reconciledBy || null);
        }
        if (updates.closedAt !== undefined) {
            fields.push("closed_at = $1");
            values.push(updates.closedAt ? new Date(updates.closedAt) : null);
        }
        if (updates.actualCash !== undefined) {
            fields.push("actual_cash = $1");
            values.push(updates.actualCash);
        }
        if (updates.openingBreakdown !== undefined) {
            fields.push("opening_breakdown = $1");
            values.push(JSON.stringify(updates.openingBreakdown || {}));
        }
        if (updates.closingBreakdown !== undefined) {
            fields.push("closing_breakdown = $1");
            values.push(JSON.stringify(updates.closingBreakdown || {}));
        }
        if (updates.difference !== undefined) {
            fields.push("difference = $1");
            values.push(updates.difference);
        }
        if (updates.accumulatedTips !== undefined) {
            fields.push("accumulated_tips = $1");
            values.push(updates.accumulatedTips);
        }
        if (updates.netTips !== undefined) {
            fields.push("net_tips = $1");
            values.push(updates.netTips);
        }
        ...
        if (updates.managerNotes !== undefined) {
            fields.push("manager_notes = $1");
            values.push(updates.managerNotes);
        }
        if (updates.varianceReason !== undefined) {
            fields.push("variance_reason = $1");
            values.push(updates.varianceReason);
        }
        if (updates.expectedCash !== undefined) {
            fields.push("expected_cash = $1");
            values.push(updates.expectedCash);
        }
        if (updates.expectedCashDelta !== undefined) {
            fields.push("expected_cash = expected_cash + $1");
            values.push(updates.expectedCashDelta);
        }
        if (updates.tipsDelta !== undefined) {
            fields.push("accumulated_tips = accumulated_tips + $1");
            values.push(updates.tipsDelta);
        }
        if (fields.length > 0 || walletTipsDelta > 0) {
            ...
            try {
                ...
                if (fields.length > 0) {
                    ...
                    await conn.query(`UPDATE cash_sessions SET ${fields.join(", ")} WHERE id = $1 AND tenant_id = $2`, values);
                }
                if (walletTipsDelta > 0) {
                    await conn.query(`UPDATE staff SET wallet_balance = COALESCE(wallet_balance, 0) + $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`, [walletTipsDelta, req.params.tenantId, walletTipsStaffId]);
                }
                await conn.commit();
            }
            catch (err) {
...
cashRouter.put("/cash-sessions/:id/review", requireAuth, async (req: any, res) => {
    try {
        ...
        if (reviewStatus === "reconciled") {
            fields.splice(3, 0, "reconciled_at = NOW()", "reconciled_by = $1");
            values.splice(2, 0, req.user?.staffId || null);
        }
        values.push(req.params.id, req.params.tenantId);
        await query(`UPDATE cash_sessions SET ${fields.join(", ")} WHERE id = $1 AND tenant_id = $2`, values);
        if (reviewStatus === "reconciled") {
...
cashRouter.get("/manager-cash/summary", requireAuth, async (req: any, res) => {
    try {
        if (!canManageCash(req.user?.role))
            return denyWithAudit(req, res, "manager_cash.summary_view", "Only managers and admins can view the manager cash summary.");
...
cashRouter.put("/manager-cash/transfers/:transferId/confirm", requireAuth, async (req: any, res) => {
    try {
        if (!canManageCash(req.user?.role))
            return denyWithAudit(req, res, "manager_cash.transfer_confirm", "Only managers and admins can confirm cash custody transfers.", { transferId: req.params.transferId });
...
cashRouter.put("/manager-cash/transfers/:transferId/cancel", requireAuth, async (req: any, res) => {
    try {
        if (!canManageCash(req.user?.role))
            return denyWithAudit(req, res, "manager_cash.transfer_cancel", "Only managers and admins can cancel cash custody transfers.", { transferId: req.params.transferId });
...
cashRouter.get("/manager-cash/close/:checkpointId/export", requireAuth, async (req: any, res) => {
    try {
        if (!canManageCash(req.user?.role))
            return denyWithAudit(req, res, "manager_cash.close_export", "Only managers and admins can export end-of-day cash close records.", { checkpointId: req.params.checkpointId });
