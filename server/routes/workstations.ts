...
workstationsRouter.get("/companion-device-assignments", requireAuth, async (req: any, res) => {
    try {
        const rows = await query(`SELECT cda.id, cda.tenant_id AS tenantId, cda.device_id AS deviceId, cda.device_name AS deviceName,
        ...
        WHERE cda.tenant_id = $1
        ORDER BY cda.updated_at DESC`, [req.params.tenantId]);
        res.json(rows);
    }
    catch (err: any) {
        res.status(500).json({ error: err.message });
...
workstationsRouter.put("/companion-device-assignments/:deviceId", requireAuth, async (req: any, res) => {
    try {
        ...
        await query(`INSERT INTO companion_device_assignments (id, tenant_id, device_id, device_name, workstation_id, default_mode, assigned_by, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
       ON CONFLICT (tenant_id, device_id) DO UPDATE SET
         workstation_id = EXCLUDED.workstation_id, device_name = EXCLUDED.device_name,
         default_mode = EXCLUDED.default_mode, assigned_by = EXCLUDED.assigned_by, updated_at = NOW()`, [id, req.params.tenantId, req.params.deviceId, deviceName || req.params.deviceId, workstationId || null, defaultMode || "display", req.user?.staffId || null]);
        res.json({ success: true });
    }
    catch (err: any) {
...
workstationsRouter.delete("/companion-device-assignments/:deviceId", requireAuth, async (req: any, res) => {
    try {
        ...
        await query(`DELETE FROM companion_device_assignments WHERE tenant_id = $1 AND device_id = $2`, [req.params.tenantId, req.params.deviceId]);
        res.json({ success: true });
    }
    catch (err: any) {
        res.status(400).json({ error: err.message });
...
workstationsRouter.post("/messages", requireAuth, async (req: any, res) => {
    try {
        ...
        if (io)
            broadcastToMessages(io, req.params.tenantId, { type: "new_message", message: data });
