...
export async function getDashboardKpis(tenantId: string, now = new Date()) {
    ...
    const [salesRows, activeTableRows, cashRows, lowStockSummaryRows, lowStockRows, staffRows] = await Promise.all([
        query<any>(`SELECT
         SUM(CASE WHEN status = 'completed' AND created_at >= $1 THEN 1 ELSE 0 END) AS todayCompletedCount,
         SUM(CASE WHEN status = 'completed' AND created_at >= $2 THEN total ELSE 0 END) AS todayCompletedRevenue,
         SUM(CASE WHEN status = 'completed' AND created_at >= $3 THEN 1 ELSE 0 END) AS lastHourCompletedCount,
         SUM(CASE WHEN status = 'completed' AND created_at >= $4 THEN total ELSE 0 END) AS lastHourCompletedRevenue,
         ...
         SUM(CASE WHEN status = 'completed' AND table_number IS NOT NULL AND table_number <> '' AND created_at >= $5 THEN 1 ELSE 0 END) AS tableSaleCount,
         COUNT(DISTINCT CASE WHEN status = 'completed' AND table_number IS NOT NULL AND table_number <> '' AND created_at >= $6 THEN table_number ELSE NULL END) AS servedTableCount
       FROM sales
       WHERE tenant_id = $7`, [
        ...
        query<any>(`SELECT COUNT(*) AS activeTableCount
       ...
       FROM cash_sessions
       WHERE tenant_id = $1
         AND COALESCE(closed_at, submitted_at, created_at) >= $2
         AND COALESCE(closed_at, submitted_at, created_at) <= $3`, [tenantId, dbTimestamp(dayStart), dbTimestamp(now)]),
        query<any>(`SELECT
       ...
       WHERE tenant_id = $1
         ...
         AND COALESCE(stock, 0) <= COALESCE(min_stock, 0)`, [tenantId]),
        query<any>(`SELECT id,
              name,
              category,
              stock,
