...
export async function getPushSettings(tenantId: string): Promise<PushSettings> {
  const rows = await query<any>(
    `SELECT tenant_id, vapid_public_key, vapid_private_key, subject, enabled
...
export async function savePushSubscription(tenantId: string, staffId: string | null, payload: any, meta: { deviceLabel?: string; userAgent?: string } = {}) {
  ...
  if (staffIds?.length) {
    ...
    return query<any>(
      `SELECT id, endpoint, p256dh, auth, expiration_time
         FROM push_subscriptions
        WHERE tenant_id = ? AND disabled_at IS NULL AND staff_id IN (${placeholders})`,
      [tenantId, ...staffIds]
    );
  }
  return query<any>(
...
export async function sendPushNotification(
  ...
  options: { staffIds?: string[]; ttl?: number; urgency?: "very-low" | "low" | "normal" | "high" } = {}
) {
  const settings = await getPushSettings(tenantId);
  ...
  await Promise.all(rows.map(async (row) => {
    try {
      ...
      if (err?.statusCode === 404 || err?.statusCode === 410) {
        await query(
          `UPDATE push_subscriptions SET disabled_at = NOW(), updated_at = NOW() WHERE tenant_id = ? AND id = ?`,
          [tenantId, row.id]
        );
      } else {
