...
async function listForeignKeyDependencies() {
  ...
  for (const row of rows as any[]) {
    ...
    if (!tableName || !referencedTableName || tableName === referencedTableName) continue;
...
export async function createDatabaseBackup(options: { createdBy?: string | null; note?: string | null } = {}) {
  await ensureBackupDir();
...
export async function restoreDatabaseBackup(id: string, options: RestoreOptions = {}): Promise<RestoreResult> {
  ...
  const orderedTables = orderTables(
    backup.tables.map((table) => table.name).filter((tableName) => liveTableSet.has(tableName)),
    dependencies
  );
