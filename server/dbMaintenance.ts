...
async function listColumnMetadata() {
    ...
    for (const row of rows as any[]) {
        ...
        list.push({
            ...
            ordinalPosition: Number(rowValue(row, "ordinalPosition") || list.length + 1),
        });
...
async function listForeignKeyDependencies() {
    ...
    for (const row of rows as any[]) {
        const tableName = String(rowValue(row, "tableName") || "");
        const referencedTableName = String(rowValue(row, "referencedTableName") || "");
...
function orderTables(tables: string[], dependencies: Map<string, Set<string>>) {
    ...
    while (pending.size > 0) {
        ...
        for (const table of [...pending].sort()) {
            const refs = [...(dependencies.get(table) || [])].filter((ref) => known.has(ref) && pending.has(ref));
            if (refs.length > 0)
                continue;
...
function summarizeBackup(backup: DatabaseBackupFile, filename: string): DatabaseBackupSummary {
    const tables = backup.tables.map((table) => ({ name: table.name, rows: table.rows.length }));
    return {
        id: backup.id,
        filename,
...
function assertBackupFile(value: unknown): DatabaseBackupFile {
    ...
    if (backup.version !== BACKUP_VERSION || !backup.id || !Array.isArray(backup.tables)) {
...
export async function listDatabaseBackups() {
    ...
    for (const entry of entries) {
        if (!entry.isFile() || !entry.name.endsWith(".json"))
            continue;
        try {
            const backup = parseBackupJson(await fs.readFile(path.join(backupDir(), entry.name), "utf8"));
...
export async function createDatabaseBackup(options: {
    ...
    const [identity, tables, columnsByTable, primaryKeys, dependencies] = await Promise.all([
        getDatabaseIdentity(),
        listDatabaseTables(),
        listColumnMetadata(),
        listPrimaryKeys(),
    ...
    await fs.writeFile(backupFilePath(id), JSON.stringify(backup, null, 2), "utf8");
...
function normalizeValueForColumn(value: unknown, column: ColumnMetadata) {
    ...
    if (column.dataType === "json" && typeof value === "object" && value !== null) {
        return JSON.stringify(value);
    }
    if (typeof value === "string" && (column.dataType.includes("date") || column.dataType.includes("time"))) {
...
async function rowExists(tableName: string, primaryKey: string[], row: Record<string, unknown>) {
    const where = primaryKey.map((column) => `${quoteIdentifier(column)} = $1`).join(" AND ");
    const rows = await query<Record<string, unknown>>(`SELECT 1 AS found FROM ${quoteIdentifier(tableName)} WHERE ${where} LIMIT 1`, primaryKey.map((column) => row[column]));
    return rows.length > 0;
}
async function insertRow(tableName: string, columns: ColumnMetadata[], row: Record<string, unknown>) {
    const sql = `INSERT INTO ${quoteIdentifier(tableName)} (${columns.map((column) => quoteIdentifier(column.name)).join(", ")})
    VALUES (${columns.map(() => "?").join(", ")})`;
    await query(sql, columns.map((column) => normalizeValueForColumn(row[column.name], column)));
}
async function updateRow(tableName: string, columns: ColumnMetadata[], primaryKey: string[], row: Record<string, unknown>) {
    const updateColumns = columns.filter((column) => !primaryKey.includes(column.name));
    if (updateColumns.length === 0)
        return;
    const sql = `UPDATE ${quoteIdentifier(tableName)}
    SET ${updateColumns.map((column) => `${quoteIdentifier(column.name)} = ?`).join(", ")}
    WHERE ${primaryKey.map((column) => `${quoteIdentifier(column)} = ?`).join(" AND ")}`;
    await query(sql, [
        ...updateColumns.map((column) => normalizeValueForColumn(row[column.name], column)),
        ...primaryKey.map((column) => row[column]),
    ]);
}
export async function restoreDatabaseBackup(id: string, options: RestoreOptions = {}): Promise<RestoreResult> {
    const backup = await readDatabaseBackup(id);
    const startedAt = new Date().toISOString();
    const dryRun = Boolean(options.dryRun);
    const overwriteExisting = Boolean(options.overwriteExisting);
    const [liveTables, columnsByTable, primaryKeys, dependencies] = await Promise.all([
        listDatabaseTables(),
        listColumnMetadata(),
        listPrimaryKeys(),
        listForeignKeyDependencies(),
    ...
    const backupTables = new Map(backup.tables.map((table) => [table.name, table]));
    const orderedTables = orderTables(backup.tables.map((table) => table.name).filter((tableName) => liveTableSet.has(tableName)), dependencies);
    ...
    for (const tableName of orderedTables) {
        ...
        const usableColumns = table.columns
            ...
            .map((column) => liveColumns.find((liveColumn) => liveColumn.name === column)!)
            .filter(Boolean);
        const missingColumns = table.columns.filter((column) => !liveColumnNames.has(column));
        const primaryKey = (primaryKeys.get(tableName) || table.primaryKey || []).filter((column) => liveColumnNames.has(column));
        ...
        if (primaryKey.length === 0) {
            tableResult.skipped = table.rows.length;
            tableResult.status = "skipped";
            tableResult.message = "No primary key is available for safe merge restore.";
            result.tables.push(tableResult);
            continue;
        }
        for (const sourceRow of table.rows) {
            const row = Object.fromEntries(usableColumns.map((column) => [column.name, sourceRow[column.name]])) as Record<string, unknown>;
            const hasPrimaryKey = primaryKey.every((column) => row[column] !== undefined && row[column] !== null);
            if (!hasPrimaryKey) {
            ...
            try {
                ...
                if (exists)
                    tableResult.updated += 1;
                else
                    tableResult.inserted += 1;
            }
            catch (err: any) {
                tableResult.failed += 1;
                tableResult.message = err?.message || "One or more rows failed to restore.";
            }
        }
        tableResult.status = tableResult.failed > 0
            ? (tableResult.inserted + tableResult.updated > 0 ? "partial" : "failed")
            : "ok";
        result.tables.push(tableResult);
    }
