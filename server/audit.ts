...
export function normalizeStockMovementReasonCode(reason?: string | null, referenceType?: string | null): StockMovementReasonCode {
    const raw = String(reason || "").trim().toLowerCase();
    const codeish = raw.replace(/[\s-]+/g, "_");
    if (stockMovementReasonCodeSet.has(codeish))
        return codeish as StockMovementReasonCode;
    if (["sale_completed", "sale_deduction", "checkout"].includes(codeish))
        return "sale";
    if (["refund_restock", "refund_reversal", "refund"].includes(codeish))
        return "refund";
    if (["void_restock", "void_reversal", "void"].includes(codeish))
        return "void";
    if (["manual_adjustment", "adjustment", "stock_adjustment"].includes(codeish))
        return "adjustment";
    if (["stock_take", "stocktake", "cycle_count", "spot_check", "count_correction"].includes(codeish))
        return "count_correction";
    if (["purchase_order", "invoice_receiving", "receiving", "received"].includes(codeish))
        return "receiving";
    if (["stock_transfer", "transfer"].includes(codeish))
        return "transfer";
    if (["waste", "wastage", "expired", "expiry", "spoiled", "spoilage", "damage", "damaged"].includes(codeish))
        return "wastage";
    if (["shrink", "shrinkage", "theft", "loss", "lost", "missing"].includes(codeish))
        return "shrinkage";
    ...
    if (reference === "void")
        return "void";
    if (reference === "stock_take_session" || reference === "stock_take_item")
        return "count_correction";
    if (reference === "purchase_order" || reference === "receiving" || reference === "invoice")
        return "receiving";
    if (reference === "stock_transfer" || reference === "transfer")
        return "transfer";
    return "adjustment";
}
...
export async function recordStockMovement(conn: Queryable, input: StockMovementInput) {
    ...
    await conn.query(`INSERT INTO stock_movements (
...
export async function applyProductStockDelta(conn: Queryable, input: Omit<StockMovementInput, "itemType" | "previousQuantity" | "newQuantity"> & {
    productId: string;
}) {
    const [rows] = await conn.query<any>(`SELECT id, name, stock
       FROM products
      WHERE tenant_id = $1 AND id = $2
      LIMIT 1
      FOR UPDATE`, [input.tenantId, input.productId]);
    ...
    await conn.query(`UPDATE products SET stock = $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`, [newQuantity, input.tenantId, input.productId]);
    try {
        const [locationRows] = await conn.query<any>(`SELECT quantity
         FROM product_location_stock
        WHERE tenant_id = $1 AND product_id = $2 AND location_id = $3
        LIMIT 1
        FOR UPDATE`, [input.tenantId, input.productId, locationId]);
