...
export async function saveRetentionPolicy(tenantId: string, policyInput: unknown, actor: Actor = {}) {
    ...
    await query(`INSERT INTO app_settings (tenant_id, retention_policy, created_at, updated_at)
         VALUES ($1, $2, NOW(), NOW())
         ON CONFLICT (tenant_id) DO UPDATE SET retention_policy = EXCLUDED.retention_policy,
                                               updated_at = NOW()`, [tenantId, serialized]);
...
export async function getRetentionPreview(tenantId: string, policyInput?: unknown): Promise<RetentionPreview> {
    ...
    const [customerNotesCount, messagesCount, pushCount, companionCount, auditCount] = await Promise.all([
        ...
        WHERE tenant_id = $1
          ...
          AND notes <> ''
          AND COALESCE(updated_at, created_at) < $2`, [tenantId, customerNotesCutoff]),
        countRows(`SELECT COUNT(*) AS count
         FROM messages
        WHERE tenant_id = $1 AND created_at < $2`, [tenantId, messagesCutoff]),
        countRows(`SELECT COUNT(*) AS count
         FROM push_subscriptions
        WHERE tenant_id = $1
          AND COALESCE(disabled_at, last_seen_at, updated_at, created_at) < $2`, [tenantId, deviceCutoff]),
        countRows(`SELECT COUNT(*) AS count
         FROM companion_device_assignments
        WHERE tenant_id = $1 AND updated_at < $2`, [tenantId, deviceCutoff]),
        countRows(`SELECT COUNT(*) AS count
         FROM audit_events
        WHERE tenant_id = $1
          AND created_at < $2
          AND action NOT LIKE 'retention_policy.%'`, [tenantId, auditCutoff]),
    ]);
    return {
...
export async function applyRetentionPolicy(tenantId: string, policyInput?: unknown, actor: Actor = {}): Promise<RetentionApplyResult> {
    ...
    await query(`UPDATE customers
      ...
      WHERE tenant_id = $1
        AND notes IS NOT NULL
        AND notes <> ''
        AND COALESCE(updated_at, created_at) < $2`, [tenantId, preview.customerNotes.cutoff]);
    await query(`DELETE FROM messages WHERE tenant_id = $1 AND created_at < $2`, [tenantId, preview.messages.cutoff]);
    await query(`DELETE FROM push_subscriptions
      WHERE tenant_id = $1
        AND COALESCE(disabled_at, last_seen_at, updated_at, created_at) < $2`, [tenantId, preview.stalePushSubscriptions.cutoff]);
    await query(`DELETE FROM companion_device_assignments WHERE tenant_id = $1 AND updated_at < $2`, [tenantId, preview.staleCompanionDevices.cutoff]);
    await query(`DELETE FROM audit_events
      WHERE tenant_id = $1
        AND created_at < $2
        AND action NOT LIKE 'retention_policy.%'`, [tenantId, preview.auditLogs.cutoff]);
    ...
    await query(`INSERT INTO app_settings (tenant_id, retention_policy, created_at, updated_at)
         VALUES ($1, $2, NOW(), NOW())
         ON CONFLICT (tenant_id) DO UPDATE SET retention_policy = EXCLUDED.retention_policy,
                                               updated_at = NOW()`, [tenantId, JSON.stringify(appliedPolicy)]);
    await recordAuditEventSafe({
        tenantId,
