...
function normalizeReportAudience(value: unknown) {
    ...
    if (audience === "accountant" || audience === "compliance" || audience === "owner")
        return audience;
    return "owner";
}
...
function reviewFocusForReport(audience: string, item: any) {
    ...
    if (audience === "accountant") {
        ...
        if (title.includes("cash") || source.includes("cash"))
            return "cash control";
        if (title.includes("wallet"))
            return "wallet liability";
        if (title.includes("refund") || title.includes("void"))
    ...
    if (audience === "compliance") {
        if (title === "permission.denied" || title.startsWith("auth."))
            return "security access";
        if (title.startsWith("settings.") || title.startsWith("staff."))
            return "administration change";
        if (title.startsWith("customer."))
            return "customer data change";
        if (title.startsWith("ai."))
            return "AI approval trace";
        if (title.startsWith("offline."))
            return "offline sync trace";
        return "retention trail";
    }
    if (title.includes("refund") || title.includes("void"))
        return "sales exception";
    if (title.includes("cash") || source.includes("cash"))
...
export async function getManagerActivityHistory(tenantId: string, filters: ActivityFilters = {}) {
    ...
    const stockPromise = includeStock ? (() => {
        ...
        OR LOWER(COALESCE(note, '')) LIKE $11
            ...
            params.push(`%${search}%`, `%${search}%`, `%${search}%`, `%${search}%`, `%${search.replace(/[\s-]+/g, "_")}%`, `%${search}%`, `%${search}%`, `%${search}%`, `%${search}%`, `%${search}%`, `%${search}%`);
        }
        params.push(limit);
        return query<any>(`SELECT
         id,
...
export async function getManagerAuditReport(tenantId: string, filters: ActivityFilters = {}) {
    ...
    const titleOrSourceMatches = (pattern: RegExp) => items.filter((item: any) => (pattern.test(String(item.title || "")) || pattern.test(String(item.source || "")))).length;
    const summary = {
