...
export function normalizeDeliveryProvider(value: unknown): DeliveryProvider {
    ...
    if (text === "uber" || text === "uber_eats" || text === "ubereats")
        return "uber_eats";
    if (text === "mr_d" || text === "mrd" || text === "mr_d_food" || text === "mr_d_foods")
        return "mr_d";
    throw new Error("Unsupported delivery provider");
}
export function normalizeDeliveryStatus(value: unknown): DeliveryOrderStatus {
    const text = clean(value, 80).toLowerCase().replace(/[\s-]+/g, "_");
    if (!text || ["created", "received", "pending", "placed", "new"].includes(text))
        return "new";
    if (["accepted", "confirmed"].includes(text))
        return "accepted";
    if (["preparing", "in_progress", "cooking"].includes(text))
        return "preparing";
    if (["ready", "ready_for_pickup", "ready_for_collection"].includes(text))
        return "ready";
    if (["dispatched", "picked_up", "collected", "out_for_delivery"].includes(text))
        return "dispatched";
    if (["completed", "delivered", "fulfilled"].includes(text))
        return "completed";
    if (["cancelled", "canceled", "rejected"].includes(text))
        return "cancelled";
    return "new";
}
...
function normalizeOrderInput(input: DeliveryOrderInput) {
    ...
    const externalOrderId = clean(input.externalOrderId
        ?? readPath(raw, ["externalOrderId", "external_order_id", "orderId", "order_id", "id", "uuid", "display_id"]), 128);
    if (!externalOrderId)
        throw new Error("External delivery order ID is required");
    const items = normalizeItems(input, raw);
    const itemSubtotal = money(items.reduce((sum: number, item: {
        quantity: number;
        price: number;
...
export async function ingestDeliveryOrder(tenantId: string, input: DeliveryOrderInput, actor: Actor = {}) {
    ...
    const existing = await query<any>(`SELECT id FROM delivery_orders
      WHERE tenant_id = $1 AND provider = $2 AND external_order_id = $3
      LIMIT 1`, [tenantId, normalized.provider, normalized.externalOrderId]);
    const orderId = existing[0]?.id || id("do");
    const rawPayload = JSON.stringify(normalized.rawPayload || {});
    if (existing.length > 0) {
        await query(`UPDATE delivery_orders
          SET status = $1,
              customer_name = $2,
              customer_phone = $3,
              delivery_address = $4,
        ...
        await query(`INSERT INTO delivery_orders (
    ...
    await query(`DELETE FROM delivery_order_items WHERE tenant_id = $1 AND delivery_order_id = $2`, [tenantId, orderId]);
    for (const item of normalized.items) {
        await query(`INSERT INTO delivery_order_items (
...
export async function listDeliveryOrders(tenantId: string, filters: {
    provider?: string | null;
    status?: string | null;
    id?: string | null;
} = {}) {
    ...
    const orders = await query<any>(`SELECT
       id,
    ...
    if (orders.length === 0)
        return [];
    const placeholders = orders.map(() => "$1").join(",");
    const items = await query<any>(`SELECT
       id,
     ...
     WHERE tenant_id = $1 AND delivery_order_id IN (${placeholders})
     ORDER BY created_at ASC, id ASC`, [tenantId, ...orders.map(order => order.id)]);
...
export async function updateDeliveryOrderStatus(tenantId: string, orderId: string, status: string, actor: Actor = {}) {
    ...
    await query(`UPDATE delivery_orders
        SET status = $1,
            ...
            updated_at = NOW()
      WHERE tenant_id = $3 AND id = $4`, [nextStatus, nextStatus, tenantId, orderId]);
    await recordAuditEventSafe({
        tenantId,
