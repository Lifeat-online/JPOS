...
function reviewState(status: string) {
    if (["approved", "confirmed", "settled", "refunded", "partial_refund"].includes(status))
        return "matched";
...
function buildWhere(tenantId: string, filters: PaymentProviderReportFilters) {
    ...
    if (provider) {
        where.push("LOWER(COALESCE(sp.provider, '')) = $1");
        params.push(provider);
    }
    const status = clean(filters.status).toLowerCase();
    if (status) {
        where.push("LOWER(COALESCE(sp.provider_status, 'unconfirmed')) = $1");
        params.push(status);
    }
    return { where, params };
}
export async function getPaymentProviderReconciliationReport(tenantId: string, filters: PaymentProviderReportFilters = {}) {
