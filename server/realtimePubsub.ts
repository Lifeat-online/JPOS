...
export const REALTIME_INSTANCE_ID =
  process.env.JPOS_REALTIME_INSTANCE_ID || `instance_${crypto.randomUUID()}`;

export function realtimeFanoutEnabled(env: NodeJS.ProcessEnv = process.env) {
  return (
    env.JPOS_REALTIME_FANOUT === "database" ||
...
export async function publishRealtimeEventIfEnabled(
  event: Pick<RealtimePubsubEvent, "channel" | "eventName" | "payload">
) {
  if (!realtimeFanoutEnabled()) return null;
...
export function parseRealtimeEventRow(row: any): RealtimePubsubEvent | null {
  if (!row?.id || !row?.channel || !row?.event_name) return null;
...
export async function deleteExpiredRealtimeEvents(
  runQuery: RealtimeQueryRunner = query
) {
