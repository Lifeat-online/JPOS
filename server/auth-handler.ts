...
async function ensureTenantExists(tenantId: string, tenantName: string) {
    const tenants = await query<any>("SELECT id, name FROM tenants WHERE id = $1 LIMIT 1", [tenantId]);
    if (tenants.length > 0) {
        return tenants[0].name || tenantName;
    }
    await query("INSERT INTO tenants (id, name, created_at, updated_at) VALUES ($1, $2, NOW(), NOW())", [tenantId, tenantName]);
    return tenantName;
}
async function normalizeDevStaff(staff: StaffAuthRow): Promise<StaffAuthRow> {
    ...
    try {
        await query("UPDATE staff SET tenant_id = $1, role = $2, email = $3, status = $4, updated_at = NOW() WHERE id = $5", [DEV_TENANT_ID, 'dev', DEV_EMAIL, 'active', staff.id]);
        await query("UPDATE users SET tenant_id = $1, email = $2, name = $3, updated_at = NOW() WHERE uid = $4", [DEV_TENANT_ID, DEV_EMAIL, staff.name, staff.id]);
    }
    catch (error) {
...
async function issueAuthResponse(req: Request, staff: {
    id: string;
    tenant_id: string;
    tenant_name?: string;
    name: string;
...
async function ensureDemoTenant(mode: DemoSeedMode) {
    ...
    try {
        ...
        const [tenants] = await conn.query<any>("SELECT id FROM tenants WHERE id = $1", [tenantId]);
        if (tenants.length === 0) {
            await conn.query("INSERT INTO tenants (id, name, created_at, updated_at) VALUES ($1, $2, NOW(), NOW())", [tenantId, tenantName]);
        }
        else {
            await conn.query("UPDATE tenants SET name = $1, updated_at = NOW() WHERE id = $2", [tenantName, tenantId]);
        }
        const [users] = await conn.query<any>("SELECT uid FROM users WHERE uid = $1", [staffId]);
        if (users.length === 0) {
            await conn.query("INSERT INTO users (uid, tenant_id, email, name, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())", [staffId, tenantId, email, name]);
        }
        else {
            await conn.query("UPDATE users SET tenant_id = $1, email = $2, name = $3, updated_at = NOW() WHERE uid = $4", [tenantId, email, name, staffId]);
        }
        const [staffRows] = await conn.query<any>("SELECT id FROM staff WHERE id = $1", [staffId]);
        if (staffRows.length === 0) {
            await conn.query(`INSERT INTO staff (id, tenant_id, name, role, email, password_hash, status, created_at, updated_at)
         VALUES ($1, $2, $3, 'admin', $4, $5, 'active', NOW(), NOW())`, [staffId, tenantId, name, email, passwordHash]);
        }
        else {
            await conn.query(`UPDATE staff
         SET tenant_id = $1, name = $2, role = 'admin', email = $3, password_hash = $4, status = 'active', updated_at = NOW()
         WHERE id = $5`, [tenantId, name, email, passwordHash, staffId]);
        }
        const business = {
            name: tenantName,
            currency: 'R',
        ...
        const [settings] = await conn.query<any>("SELECT tenant_id FROM app_settings WHERE tenant_id = $1", [tenantId]);
        ...
        ) VALUES ($1, '10000100', '46f0cd694581a', 'jt7v60h69n8a1', 1, $2, NULL, 'demo', 1, NOW(), NOW())`, [tenantId, JSON.stringify(business)]);
        }
        else {
            await conn.query(`UPDATE app_settings
         SET business = $1, slug = 'demo', setup_completed = 1, updated_at = NOW()
         WHERE tenant_id = $2`, [JSON.stringify(business), tenantId]);
        }
        await conn.commit();
    }
    catch (error) {
...
export async function handleEnrollment(req: Request, res: Response) {
    try {
        ...
        const existing = await query<any>("SELECT id FROM staff WHERE email = $1 AND status = 'active' LIMIT 1", [email]);
        ...
        try {
            await conn.beginTransaction();
            await conn.query("INSERT INTO tenants (id, name, created_at, updated_at) VALUES ($1, $2, NOW(), NOW())", [tenantId, businessName]);
            await conn.query("INSERT INTO users (uid, tenant_id, email, name, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())", [staffId, tenantId, email, ownerName]);
            await conn.query(`INSERT INTO staff (id, tenant_id, name, role, email, password_hash, status, created_at, updated_at)
         VALUES ($1, $2, $3, 'admin', $4, $5, 'active', NOW(), NOW())`, [staffId, tenantId, ownerName, email, passwordHash]);
            await conn.query(`INSERT INTO app_settings (
          tenant_id, payfast_merchant_id, payfast_merchant_key, payfast_passphrase,
          payfast_sandbox, business, categories, slug, setup_completed, created_at, updated_at
        ) VALUES ($1, '10000100', '46f0cd694581a', 'jt7v60h69n8a1', 1, $2, NULL, NULL, 0, NOW(), NOW())`, [tenantId, JSON.stringify(business)]);
            await conn.commit();
        }
        catch (error) {
            await conn.rollback();
...
export async function handleLogin(req: Request, res: Response) {
    try {
      ...
      WHERE LOWER(s.email) = $1 AND s.status = 'active'
    `;
...
export async function handleGetMe(req: Request, res: Response) {
    ...
    const rows = await query<any>(`SELECT two_factor_enabled AS twoFactorEnabled, two_factor_confirmed_at AS twoFactorConfirmedAt
       FROM staff
      WHERE tenant_id = $1 AND id = $2
      LIMIT 1`, [req.user.tenantId, req.user.staffId || req.user.uid]);
...
async function loadCurrentStaffSecurity(req: Request) {
    const rows = await query<any>(`SELECT id, name, role, email, password_hash AS passwordHash,
...
export async function handleTwoFactorStatus(req: Request, res: Response) {
    try {
        if (!requireTwoFactorEligibleUser(req, res))
            return;
    ...
    try {
        ...
        await query(`UPDATE staff
          SET two_factor_secret = $1,
              ...
              updated_at = NOW()
        WHERE tenant_id = $2 AND id = $3`, [secret, req.user!.tenantId, req.user!.staffId || req.user!.uid]);
        await recordAuditEventSafe({
            tenantId: req.user!.tenantId,
    ...
    try {
        ...
        await query(`UPDATE staff
          SET two_factor_enabled = 1,
              ...
              updated_at = NOW()
        WHERE tenant_id = $1 AND id = $2`, [req.user!.tenantId, req.user!.staffId || req.user!.uid]);
        await recordAuditEventSafe({
            tenantId: req.user!.tenantId,
...
export async function handleSetupPassword(req: Request, res: Response) {
    try {
        ...
        const passwordHash = await hashPassword(password);
        const result = await query("UPDATE staff SET password_hash = $1 WHERE id = $2 AND tenant_id = $3", [passwordHash, staffId, req.user.tenantId]);
        await recordAuditEventSafe({
            tenantId: req.user.tenantId,
