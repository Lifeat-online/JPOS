...
export async function recordManagerOverride(tenantId: string, input: ManagerOverrideInput) {
  ...
  if (!tenantId) throw new Error("Tenant is required for manager override recording");
  if (!targetType || !targetId || !action) throw new Error("Manager override target and action are required");
  if (reason.length < 3) throw new Error("A manager override reason is required");
