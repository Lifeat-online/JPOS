...
function priorityFor(stock: number, minStock: number): ReorderPriority {
    if (stock <= 0)
        return "critical";
    if (stock <= Math.max(1, minStock * 0.5))
        return "high";
    return "normal";
}
...
async function getProductVelocity(tenantId: string) {
    ...
    for (const row of rows || []) {
        ...
        if (row.productId)
            byProduct.set(String(row.productId), value);
        if (row.productName)
            byProduct.set(String(row.productName), value);
    }
    return byProduct;
}
export async function listReorderRecommendations(tenantId: string, filters: {
...
export async function refreshReorderRecommendations(tenantId: string, options: RefreshOptions = {}) {
    ...
    const [products, activeRows, velocity] = await Promise.all([
       ...
       LIMIT 200`, productParams),
        query<any>(`SELECT *
       FROM reorder_recommendations
       WHERE tenant_id = $1
         AND status IN ('open','in_review','approved')`, [tenantId]),
    ...
    for (const product of products || []) {
        ...
        if (triggerType === "critical_only" && stock > 0)
            continue;
        if (triggerType === "days_cover" && stock > minStock && (daysRemaining === null || daysRemaining > daysOfCover))
            continue;
        ...
        if (existing?.id) {
            await query(`UPDATE reorder_recommendations
            SET product_name = $1,
                priority = $2,
                current_stock = $3,
                min_stock = $4,
        ...
        await query(`INSERT INTO reorder_recommendations (
...
export async function createReorderNotificationRule(tenantId: string, input: ReorderNotificationRuleInput = {}) {
    ...
    await query(`INSERT INTO reorder_notification_rules (
...
export async function updateReorderNotificationRule(tenantId: string, id: string, input: ReorderNotificationRuleInput = {}) {
    const existing = await getReorderNotificationRule(tenantId, id);
    if (!existing)
        throw new Error("Reorder notification rule not found");
    const fields: string[] = [];
    const values: any[] = [];
    ...
    await query(`UPDATE reorder_notification_rules
        SET ${fields.join(", ")}
...
export async function runReorderNotificationRule(tenantId: string, id: string, input: ReorderActor = {}) {
    ...
    await query(`UPDATE reorder_notification_rules
        SET last_run_at = NOW(),
            last_result = $1,
            updated_at = NOW()
      WHERE tenant_id = $2 AND id = $3`, [json(runResult, {}), tenantId, id]);
    await recordAuditEvent({ query } as any, {
        tenantId,
...
export async function dismissReorderRecommendation(tenantId: string, id: string, input: ReorderActor & {
    note?: string | null;
} = {}) {
    const recommendation = await getReorderRecommendation(tenantId, id);
    if (!recommendation)
