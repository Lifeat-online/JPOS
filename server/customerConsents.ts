...
export function normalizeCustomerConsentInput(input: unknown): NormalizedConsentInput[] {
    ...
    for (const [key, rawValue] of Object.entries(input as Record<string, any>)) {
        ...
        if (!consentType || !rawValue || typeof rawValue !== "object")
            continue;
...
async function selectCurrentConsentStatus(tenantId: string, customerId: string, consentType: CustomerConsentType) {
    const rows = await query<{
      ...
      LIMIT 1`, [tenantId, customerId, consentType]);
...
export async function upsertCustomerConsents(tenantId: string, customerId: string, input: unknown, actor: ConsentActor = {}): Promise<CustomerConsentMap> {
    const entries = normalizeCustomerConsentInput(input);
    if (entries.length === 0)
        return listCustomerConsents(tenantId, customerId);
    const changes: Array<{
        consentType: CustomerConsentType;
        previousStatus: CustomerConsentStatus;
        status: CustomerConsentStatus;
    }> = [];
    for (const entry of entries) {
        ...
        await query(`INSERT INTO customer_consents (
       ...
       ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, COALESCE($10::timestamptz, NOW()), $11::timestamptz, NOW(), NOW())
       ...
       DO UPDATE SET status = EXCLUDED.status,
        ...
        await query(`INSERT INTO customer_consent_events (
