...
function normalizeActionType(value: SensitiveActionType | string): SensitiveActionType {
    ...
    if (actionTypeSet.has(normalized))
        return normalized as SensitiveActionType;
...
export async function verifySensitiveActionForRequest(req: Request, requestedActionType: SensitiveActionType | string, details: Record<string, unknown> = {}): Promise<SensitiveActionResult> {
    ...
    const staffRows = await query<any>(`SELECT id, name, password_hash AS passwordHash, security_pin_hash AS securityPinHash
       FROM staff
