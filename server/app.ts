...
export async function createApp(io: any = null) {
    ...
    const shouldExposeLicenceServer = process.env.JPOS_HOSTED === "true" ||
    ...
    const integrationWebhookRateLimit = createAuthRateLimit(parsePositiveEnvNumber(process.env.INTEGRATION_WEBHOOK_RATE_LIMIT_WINDOW_MS, 60 * 1000), parsePositiveEnvNumber(process.env.INTEGRATION_WEBHOOK_RATE_LIMIT_MAX, isProduction ? 120 : 500));
    const sensitiveRouteRateLimit = createAuthRateLimit(parsePositiveEnvNumber(process.env.SENSITIVE_ROUTE_RATE_LIMIT_WINDOW_MS, 60 * 1000), parsePositiveEnvNumber(process.env.SENSITIVE_ROUTE_RATE_LIMIT_MAX, isProduction ? 30 : 300));
    ...
    app.get("/api/mariadb/tenants/:tenantId/live", requireAuth, async (req, res) => {
        ...
        `, [tenantId]);
          ...
          `, [tenantId]);
                const activeTablesCountRows = await query<any>(`SELECT COUNT(*) AS activeTableCount FROM restaurant_tables WHERE tenant_id = $1 AND status = 'active'`, [tenantId]);
            ...
            `, [tenantId]);
                const workstationRows = await query<any>(`SELECT id, name, type FROM workstations WHERE tenant_id = $1 AND status = 'active' ORDER BY name ASC`, [tenantId]);
                const workstationTimingRows = await query<any>(`SELECT
