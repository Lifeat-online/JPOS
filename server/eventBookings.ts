...
function normalizeDepositStatus(value: unknown, depositAmount = 0): DepositStatus {
    const next = String(value || "").trim().toLowerCase();
    if (["none", "unpaid", "paid", "refunded"].includes(next))
        return next as DepositStatus;
    return depositAmount > 0 ? "unpaid" : "none";
}
function normalizeReminderStatus(value: unknown, reminderAt?: unknown): ReminderStatus {
    const next = String(value || "").trim().toLowerCase();
    if (["none", "pending", "sent", "failed", "skipped"].includes(next))
        return next as ReminderStatus;
...
async function customerSnapshot(tenantId: string, customerId?: string | null) {
    ...
    if (!id)
        return null;
    const rows = await query<any>(`SELECT id, name, phone, email FROM customers WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [tenantId, id]);
    return rows[0] || null;
}
async function tableSnapshot(tenantId: string, tableIds: string[]) {
    if (tableIds.length === 0)
        return [];
    const placeholders = tableIds.map(() => "$1").join(", ");
    const rows = await query<any>(`SELECT id, label FROM restaurant_tables WHERE tenant_id = $1 AND id IN (${placeholders}) ORDER BY label ASC`, [tenantId, ...tableIds]);
    const found = new Set(rows.map((row: any) => row.id));
    const missing = tableIds.filter((id) => !found.has(id));
    if (missing.length > 0)
        throw new Error(`Unknown restaurant table: ${missing[0]}`);
    return rows;
}
async function resolveTables(tenantId: string, input: {
    tableIds?: unknown;
    ...
    if (tableIds.length === 0)
        return { tableIds, tableNumbers: explicitLabels };
...
async function assertNoTableReservationConflict(tenantId: string, booking: {
    id?: string | null;
    ...
    if (!["confirmed", "in_progress"].includes(booking.status) || booking.tableIds.length === 0)
        return;
    ...
    const exclude = booking.id ? "AND id <> $1" : "";
    if (booking.id)
        params.push(booking.id);
    const rows = await query<any>(`SELECT id, title, start_at AS startAt, end_at AS endAt, table_ids AS tableIds, table_numbers AS tableNumbers
       FROM event_bookings
      WHERE tenant_id = $1
    ...
    for (const row of rows) {
        ...
        if (!candidateIds.some((id) => requested.has(id)))
            continue;
...
export async function listEventBookings(tenantId: string, filters: {
    from?: string;
    to?: string;
    status?: string;
    eventType?: string;
...
export async function createEventBooking(tenantId: string, input: EventBookingInput = {}) {
    ...
    await query(`INSERT INTO event_bookings (
...
export async function updateEventBooking(tenantId: string, id: string, input: EventBookingInput = {}) {
    ...
    if (input.customerId !== undefined) {
        ...
        if (input.customerName === undefined)
            Object.assign(changes, set("customer_name", customer?.name || null, "customerName"));
        if (input.contactPhone === undefined)
            Object.assign(changes, set("contact_phone", customer?.phone || null, "contactPhone"));
        if (input.contactEmail === undefined)
            Object.assign(changes, set("contact_email", customer?.email || null, "contactEmail"));
    }
    if (input.customerName !== undefined)
        Object.assign(changes, set("customer_name", text(input.customerName), "customerName"));
    if (input.contactPhone !== undefined)
        Object.assign(changes, set("contact_phone", text(input.contactPhone, 64), "contactPhone"));
    if (input.contactEmail !== undefined)
        Object.assign(changes, set("contact_email", text(input.contactEmail), "contactEmail"));
    ...
    if (input.eventType !== undefined)
        Object.assign(changes, set("event_type", normalizeType(input.eventType), "eventType"));
    if (input.status !== undefined)
        Object.assign(changes, set("status", normalizeStatus(input.status, existing.status), "status"));
    if (input.startAt !== undefined)
        Object.assign(changes, set("start_at", input.startAt, "startAt"));
    if (input.endAt !== undefined)
        Object.assign(changes, set("end_at", input.endAt || null, "endAt"));
    if (input.guestCount !== undefined)
        Object.assign(changes, set("guest_count", Math.max(0, Math.floor(number(input.guestCount))), "guestCount"));
    ...
    if (input.depositAmount !== undefined)
        Object.assign(changes, set("deposit_amount", Math.max(0, number(input.depositAmount)), "depositAmount"));
    if (input.depositStatus !== undefined)
        Object.assign(changes, set("deposit_status", normalizeDepositStatus(input.depositStatus, next.depositAmount), "depositStatus"));
    if (input.depositDueAt !== undefined)
        Object.assign(changes, set("deposit_due_at", input.depositDueAt || null, "depositDueAt"));
    if (input.depositPaidAt !== undefined)
        Object.assign(changes, set("deposit_paid_at", input.depositPaidAt || null, "depositPaidAt"));
    if (input.depositReference !== undefined)
        Object.assign(changes, set("deposit_reference", text(input.depositReference, 128), "depositReference"));
    if (input.menuNotes !== undefined)
        Object.assign(changes, set("menu_notes", text(input.menuNotes, 2000), "menuNotes"));
    if (input.internalNotes !== undefined)
        Object.assign(changes, set("internal_notes", text(input.internalNotes, 2000), "internalNotes"));
    if (input.reminderAt !== undefined)
        Object.assign(changes, set("reminder_at", input.reminderAt || null, "reminderAt"));
    if (input.reminderStatus !== undefined)
        Object.assign(changes, set("reminder_status", normalizeReminderStatus(input.reminderStatus, next.reminderAt), "reminderStatus"));
    if (input.reminderSentAt !== undefined)
        Object.assign(changes, set("reminder_sent_at", input.reminderSentAt || null, "reminderSentAt"));
    if (input.reminderNote !== undefined)
        Object.assign(changes, set("reminder_note", text(input.reminderNote, 2000), "reminderNote"));
    if (fields.length === 0)
        return existing;
    ...
    await query(`UPDATE event_bookings SET ${fields.join(", ")} WHERE tenant_id = $1 AND id = $2`, values);
    await recordAuditEvent({ query } as any, {
        tenantId,
...
export async function deleteEventBooking(tenantId: string, id: string, actor: Actor = {}) {
    ...
    if (!existing)
        throw new Error("Event booking not found");
    await query(`DELETE FROM event_bookings WHERE tenant_id = $1 AND id = $2`, [tenantId, id]);
    await recordAuditEvent({ query } as any, {
        tenantId,
