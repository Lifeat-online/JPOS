...
function normalizeType(value: unknown): InventoryLocationType {
    ...
    if (["branch", "warehouse", "register", "kitchen", "other"].includes(text))
        return text as InventoryLocationType;
    return "branch";
}
...
export async function ensureDefaultInventoryLocation(tenantId: string, runner?: Queryable) {
    ...
    await run(runner, `INSERT INTO inventory_locations (id, tenant_id, name, type, status, is_default, created_at, updated_at)
     VALUES ($1, $2, $3, 'branch', 'active', 1, NOW(), NOW())
     ON CONFLICT (tenant_id, id) DO NOTHING`, [DEFAULT_INVENTORY_LOCATION_ID, tenantId, DEFAULT_INVENTORY_LOCATION_NAME]);
    await run(runner, `INSERT INTO product_location_stock (
       tenant_id, product_id, location_id, quantity, min_stock, reorder_threshold, created_at, updated_at
     )
     SELECT tenant_id, id, $1, COALESCE(stock, 0), COALESCE(min_stock, 0), COALESCE(min_stock, 0), NOW(), NOW()
       FROM products
      WHERE tenant_id = $2
     ON CONFLICT (tenant_id, product_id, location_id) DO NOTHING`, [DEFAULT_INVENTORY_LOCATION_ID, tenantId]);
}
export async function listInventoryLocations(tenantId: string) {
    await ensureDefaultInventoryLocation(tenantId);
    const rows = await query<any>(`SELECT l.id,
      ...
      ORDER BY l.is_default DESC, l.name ASC`, [tenantId]);
...
export async function createInventoryLocation(tenantId: string, input: InventoryLocationInput, actor: InventoryActor = {}) {
    ...
    try {
        ...
        if (isDefault) {
            await conn.query(`UPDATE inventory_locations SET is_default = 0, updated_at = NOW() WHERE tenant_id = $1`, [tenantId]);
        }
        await conn.query(`INSERT INTO inventory_locations (
         id, tenant_id, name, type, status, is_default, address, notes, created_at, updated_at
       ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())`, [id, tenantId, name, type, status, isDefault, clean(input.address) || null, clean(input.notes) || null]);
...
export async function updateInventoryLocation(tenantId: string, id: string, input: InventoryLocationInput, actor: InventoryActor = {}) {
    ...
    if (input.name !== undefined) {
        fields.push("name = $1");
        values.push(clean(input.name));
    }
    if (input.type !== undefined) {
        fields.push("type = $1");
        values.push(normalizeType(input.type));
    }
    ...
    if (input.address !== undefined) {
        ...
        values.push(clean(input.address) || null);
    }
    if (input.notes !== undefined) {
        fields.push("notes = $1");
        values.push(clean(input.notes) || null);
    }
    ...
    try {
        ...
        await conn.query(`UPDATE inventory_locations SET ${fields.join(", ")} WHERE tenant_id = $1 AND id = $2`, values);
        await recordAuditEvent(conn, {
            tenantId,
...
export async function listProductLocationStocks(tenantId: string, filters: {
    productId?: string | null;
    locationId?: string | null;
} = {}) {
    await ensureDefaultInventoryLocation(tenantId);
    ...
    const rows = await query<any>(`SELECT p.id AS productId,
      ...
      ORDER BY p.name ASC, l.is_default DESC, l.name ASC`, params);
...
async function syncProductAggregateStock(conn: Queryable, tenantId: string, productId: string) {
    const [rows] = await conn.query<any>(`SELECT COALESCE(SUM(quantity), 0) AS aggregateStock
    ...
    await conn.query(`UPDATE products SET stock = $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`, [aggregateStock, tenantId, productId]);
...
export async function upsertProductLocationStock(tenantId: string, input: ProductLocationStockInput) {
    ...
    try {
        ...
        const [productRows] = await conn.query<any>(`SELECT id, name, stock, min_stock AS minStock FROM products WHERE tenant_id = $1 AND id = $2 LIMIT 1 FOR UPDATE`, [tenantId, productId]);
        const product = (productRows as any[])[0];
        if (!product)
            throw new Error("Product not found");
        const [locationRows] = await conn.query<any>(`SELECT id, name FROM inventory_locations WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [tenantId, locationId]);
        const location = (locationRows as any[])[0];
        if (!location)
            throw new Error("Inventory location not found");
        const [existingRows] = await conn.query<any>(`SELECT quantity, min_stock AS minStock, reorder_threshold AS reorderThreshold
         FROM product_location_stock
        WHERE tenant_id = $1 AND product_id = $2 AND location_id = $3
        LIMIT 1
        FOR UPDATE`, [tenantId, productId, locationId]);
        ...
        if (existing) {
            await conn.query(`UPDATE product_location_stock
            SET quantity = $1,
                min_stock = $2,
                reorder_threshold = $3,
                updated_by = $4,
                updated_by_name = $5,
                updated_at = NOW()
          WHERE tenant_id = $6 AND product_id = $7 AND location_id = $8`, [
                quantity,
                minStock,
                reorderThreshold,
                input.staffId || null,
            ...
            await conn.query(`INSERT INTO product_location_stock (
           tenant_id, product_id, location_id, quantity, min_stock, reorder_threshold,
           updated_by, updated_by_name, created_at, updated_at
         ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())`, [tenantId, productId, locationId, quantity, minStock, reorderThreshold, input.staffId || null, input.staffName || null]);
        }
...
export async function createStockTransferOrder(tenantId: string, input: StockTransferInput) {
    ...
    if (!fromLocationId || !toLocationId)
        throw new Error("Choose source and destination locations");
    if (fromLocationId === toLocationId)
        throw new Error("Source and destination locations must be different");
    const items = cleanTransferItems(input.items);
    if (!items.length)
        throw new Error("Choose at least one product to transfer");
    ...
    try {
        await conn.beginTransaction();
        await ensureDefaultInventoryLocation(tenantId, conn);
        await conn.query(`INSERT INTO stock_transfer_orders (
        ...
        for (const item of items) {
            const [productRows] = await conn.query<any>(`SELECT id, name FROM products WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [tenantId, item.productId]);
            const product = (productRows as any[])[0];
            if (!product)
                throw new Error(`Product ${item.productId} was not found`);
            await conn.query(`INSERT INTO stock_transfer_items (
           id, tenant_id, transfer_id, product_id, product_name, quantity, created_at, updated_at
         ) VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())`, [makeId("transfer_item"), tenantId, id, item.productId, item.productName || product.name, item.quantity]);
        }
        await recordAuditEvent(conn, {
            tenantId,
...
async function getLocationQuantity(conn: Queryable, tenantId: string, productId: string, locationId: string, fallbackProductMinStock = 0) {
    const [rows] = await conn.query<any>(`SELECT quantity, min_stock AS minStock, reorder_threshold AS reorderThreshold
       FROM product_location_stock
      WHERE tenant_id = $1 AND product_id = $2 AND location_id = $3
      LIMIT 1
      FOR UPDATE`, [tenantId, productId, locationId]);
...
export async function completeStockTransferOrder(tenantId: string, id: string, actor: InventoryActor = {}) {
    ...
    try {
        await conn.beginTransaction();
        const [orderRows] = await conn.query<any>(`SELECT * FROM stock_transfer_orders WHERE tenant_id = $1 AND id = $2 LIMIT 1 FOR UPDATE`, [tenantId, id]);
        const order = (orderRows as any[])[0];
        if (!order)
            throw new Error("Stock transfer was not found");
        if (order.status === "completed")
            throw new Error("Stock transfer is already completed");
        if (order.status === "cancelled")
            throw new Error("Cancelled stock transfers cannot be completed");
        const [itemRows] = await conn.query<any>(`SELECT * FROM stock_transfer_items WHERE tenant_id = $1 AND transfer_id = $2 ORDER BY created_at ASC`, [tenantId, id]);
        ...
        for (const item of items) {
            ...
            const [productRows] = await conn.query<any>(`SELECT id, name, min_stock AS minStock FROM products WHERE tenant_id = $1 AND id = $2 LIMIT 1 FOR UPDATE`, [tenantId, productId]);
            const product = (productRows as any[])[0];
            if (!product)
            ...
            await conn.query(`UPDATE product_location_stock
            SET quantity = $1, updated_by = $2, updated_by_name = $3, updated_at = NOW()
          WHERE tenant_id = $4 AND product_id = $5 AND location_id = $6`, [fromNew, actor.staffId || null, actor.staffName || null, tenantId, productId, order.from_location_id]);
            if (to.exists) {
                await conn.query(`UPDATE product_location_stock
              SET quantity = $1, updated_by = $2, updated_by_name = $3, updated_at = NOW()
            WHERE tenant_id = $4 AND product_id = $5 AND location_id = $6`, [toNew, actor.staffId || null, actor.staffName || null, tenantId, productId, order.to_location_id]);
            }
            else {
                await conn.query(`INSERT INTO product_location_stock (
             tenant_id, product_id, location_id, quantity, min_stock, reorder_threshold,
             updated_by, updated_by_name, created_at, updated_at
           ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())`, [tenantId, productId, order.to_location_id, toNew, to.minStock, to.reorderThreshold, actor.staffId || null, actor.staffName || null]);
            }
            await conn.query(`UPDATE stock_transfer_items
            SET from_previous_quantity = $1,
                from_new_quantity = $2,
                to_previous_quantity = $3,
                to_new_quantity = $4,
                updated_at = NOW()
          WHERE tenant_id = $5 AND id = $6`, [from.quantity, fromNew, to.quantity, toNew, tenantId, item.id]);
        ...
        await conn.query(`UPDATE stock_transfer_orders
          SET status = 'completed',
              completed_by = $1,
              completed_by_name = $2,
              completed_at = NOW(),
              updated_at = NOW()
        WHERE tenant_id = $3 AND id = $4`, [actor.staffId || null, actor.staffName || null, tenantId, id]);
        await recordAuditEvent(conn, {
            tenantId,
...
export async function listStockTransferOrders(tenantId: string, filters: {
    status?: string | null;
    limit?: number | string | null;
} = {}) {
    const params: any[] = [tenantId];
    const where = ["sto.tenant_id = $1"];
    if (filters.status) {
        where.push("sto.status = $1");
    ...
    params.push(limit);
    const rows = await query<any>(`SELECT sto.*,
    ...
    const itemRows = await query<any>(`SELECT *
       FROM stock_transfer_items
      WHERE tenant_id = $1
        AND transfer_id IN (${ids.map(() => "?").join(", ")})
      ORDER BY created_at ASC`, [tenantId, ...ids]);
...
export async function getStaffInventoryLocationAccess(tenantId: string, staffId?: string | null) {
    ...
    const rows = await query<any>(`SELECT default_location_id AS defaultLocationId,
...
export function cashierCanAccessLocation(role: string | null | undefined, access: {
    assignedLocationIds?: string[];
}, locationId?: string | null) {
    const normalizedRole = clean(role).toLowerCase();
    if (["admin", "manager", "dev"].includes(normalizedRole))
        return true;
