...
function serializeLocationStock(row: any) {
  ...
  const reorderThreshold = toNumber(
    row.reorderThreshold ?? row.reorder_threshold,
    minStock,
  );
  return {
...
function serializeTransfer(row: any, items: any[] = []) {
  return {
    ...
    items: items.map((item) => ({
      ...
      fromPreviousQuantity: toNumber(
        item.fromPreviousQuantity ?? item.from_previous_quantity,
      ),
      fromNewQuantity: toNumber(item.fromNewQuantity ?? item.from_new_quantity),
      toPreviousQuantity: toNumber(
        item.toPreviousQuantity ?? item.to_previous_quantity,
      ),
...
async function run(
  runner: Queryable | null | undefined,
  sql: string,
  params: any[] = [],
) {
...
export async function createInventoryLocation(
  ...
  try {
    ...
    if (isDefault) {
      await conn.query(
        `UPDATE inventory_locations SET is_default = 0, updated_at = NOW() WHERE tenant_id = $1`,
        [tenantId],
      );
    }
    await conn.query(
    ...
    await conn.commit();
    return (await listInventoryLocations(tenantId)).find(
...
export async function updateInventoryLocation(
  ...
  if (!fields.length)
    return (
      (await listInventoryLocations(tenantId)).find(
        (location) => location.id === id,
      ) || null
    );
  const conn = await getConnection();
  try {
    ...
    await conn.query(
      `UPDATE inventory_locations SET ${fields.join(", ")} WHERE tenant_id = $1 AND id = $2`,
      values,
    );
    await recordAuditEvent(conn, {
      tenantId,
...
export async function listProductLocationStocks(
  tenantId: string,
  filters: {
    productId?: string | null;
    locationId?: string | null;
...
async function syncProductAggregateStock(
  conn: Queryable,
  tenantId: string,
  productId: string,
) {
  ...
  const aggregateStock = toNumber(
    (rows as any[])[0]?.aggregateStock ?? (rows as any[])[0]?.aggregate_stock,
  );
  await conn.query(
...
export async function upsertProductLocationStock(
  tenantId: string,
  input: ProductLocationStockInput,
) {
  ...
  try {
    ...
    const [productRows] = await conn.query<any>(
      `SELECT id, name, stock, min_stock AS minStock FROM products WHERE tenant_id = $1 AND id = $2 LIMIT 1 FOR UPDATE`,
      [tenantId, productId],
    );
    const product = (productRows as any[])[0];
    if (!product) throw new Error("Product not found");
    const [locationRows] = await conn.query<any>(
      `SELECT id, name FROM inventory_locations WHERE tenant_id = $1 AND id = $2 LIMIT 1`,
      [tenantId, locationId],
    );
    ...
    const reorderThreshold = Math.max(
      ...
      toNumber(
        ...
        toNumber(
          existing?.reorderThreshold ?? existing?.reorder_threshold ?? minStock,
        ),
      ),
    );
    if (existing) {
    ...
    const aggregateStock = await syncProductAggregateStock(
      conn,
      tenantId,
      productId,
    );
    ...
    await conn.commit();
    const [row] = await listProductLocationStocks(tenantId, {
      productId,
      locationId,
    });
    return row;
...
export async function completeStockTransferOrder(
  tenantId: string,
  id: string,
  actor: InventoryActor = {},
) {
  ...
  try {
    ...
    const [orderRows] = await conn.query<any>(
      `SELECT * FROM stock_transfer_orders WHERE tenant_id = $1 AND id = $2 LIMIT 1 FOR UPDATE`,
      [tenantId, id],
    );
    ...
    const [itemRows] = await conn.query<any>(
      `SELECT * FROM stock_transfer_items WHERE tenant_id = $1 AND transfer_id = $2 ORDER BY created_at ASC`,
      [tenantId, id],
    );
    ...
    for (const item of items) {
      ...
      const [productRows] = await conn.query<any>(
        `SELECT id, name, min_stock AS minStock FROM products WHERE tenant_id = $1 AND id = $2 LIMIT 1 FOR UPDATE`,
        [tenantId, productId],
      );
      const product = (productRows as any[])[0];
      if (!product) throw new Error(`Product ${productId} was not found`);
      const from = await getLocationQuantity(
        conn,
        tenantId,
        productId,
        order.from_location_id,
        toNumber(product.minStock ?? product.min_stock),
      );
      if (from.quantity < quantity) {
        throw new Error(
          `${product.name} has only ${from.quantity} available at ${order.from_location_id}`,
        );
      }
      const to = await getLocationQuantity(
        conn,
        tenantId,
        productId,
      ...
      await conn.query(
        `UPDATE product_location_stock
          ...
          WHERE tenant_id = $4 AND product_id = $5 AND location_id = $6`,
        [
          fromNew,
          actor.staffId || null,
...
export async function listStockTransferOrders(
  tenantId: string,
  filters: {
    status?: string | null;
  ...
  const limit = Math.min(
    Math.max(Math.floor(toNumber(filters.limit, 100)), 1),
    500,
  );
  params.push(limit);
  const rows = await query<any>(
  ...
  return rows.map((row: any) =>
    serializeTransfer(row, itemsByTransfer.get(row.id) || []),
  );
}
...
export async function getStaffInventoryLocationAccess(
  tenantId: string,
  staffId?: string | null,
) {
  await ensureDefaultInventoryLocation(tenantId);
  if (!staffId) {
    return {
      defaultLocationId: DEFAULT_INVENTORY_LOCATION_ID,
      assignedLocationIds: [],
    };
  }
  const rows = await query<any>(
  ...
  const assignedLocationIds = normalizeAssignedLocations(
    staff.assignedLocationIds ?? staff.assigned_location_ids,
  );
  return {
    defaultLocationId: clean(
      staff.defaultLocationId ?? staff.default_location_id,
      assignedLocationIds[0] || DEFAULT_INVENTORY_LOCATION_ID,
    ),
    assignedLocationIds,
    role: staff.role || null,
  };
}
export function cashierCanAccessLocation(
