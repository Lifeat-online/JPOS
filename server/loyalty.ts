...
function serializeDays(value: unknown) {
  if (Array.isArray(value)) return JSON.stringify(numberArray(value));
...
function normalizeRulePayload(input: Partial<LoyaltyRewardRule>) {
  const name = text(input.name, 160);
  if (!name) throw new Error("Reward rule name is required.");
...
function activeOnDate(rule: LoyaltyRewardRule, now: Date) {
  if (rule.startsAt) {
    ...
    if (Number.isFinite(starts.getTime()) && starts.getTime() > now.getTime()) return false;
  }
  if (rule.endsAt) {
    const ends = new Date(String(rule.endsAt));
    if (Number.isFinite(ends.getTime()) && ends.getTime() < now.getTime()) return false;
  }
...
function ruleTargetSubtotal(rule: LoyaltyRewardRule, items: LoyaltySaleItem[], fallbackSubtotal: number) {
  if (rule.ruleType === "base" || rule.ruleType === "time_window") return fallbackSubtotal;
...
export async function listLoyaltyTiers(tenantId: string): Promise<LoyaltyTier[]> {
  const rows = await rowsFromRunner(dbRunner(), `SELECT * FROM loyalty_tiers WHERE tenant_id = ? ORDER BY min_points ASC, name ASC`, [tenantId]);
...
export async function createLoyaltyTier(tenantId: string, input: Partial<LoyaltyTier>) {
  ...
  await query(
    `INSERT INTO loyalty_tiers (id, tenant_id, name, status, min_points, earn_multiplier, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
    [id, tenantId, data.name, data.status, data.minPoints, data.earnMultiplier],
  );
...
export async function updateLoyaltyTier(tenantId: string, tierId: string, input: Partial<LoyaltyTier>) {
  ...
  await query(
    `UPDATE loyalty_tiers
        SET name = ?, status = ?, min_points = ?, earn_multiplier = ?, updated_at = NOW()
      WHERE tenant_id = ? AND id = ?`,
    [data.name, data.status, data.minPoints, data.earnMultiplier, tenantId, tierId],
  );
...
export async function listLoyaltyRewardRules(tenantId: string): Promise<LoyaltyRewardRule[]> {
  const rows = await rowsFromRunner(dbRunner(), `SELECT * FROM loyalty_reward_rules WHERE tenant_id = ? ORDER BY rule_type ASC, updated_at DESC, name ASC`, [tenantId]);
...
export async function updateLoyaltyRewardRule(tenantId: string, ruleId: string, input: Partial<LoyaltyRewardRule>) {
  ...
  await query(
    `UPDATE loyalty_reward_rules
        SET name = ?, status = ?, rule_type = ?, points_per_currency = ?, multiplier = ?, bonus_points = ?,
            min_subtotal = ?, starts_at = ?, ends_at = ?, target_product_ids = ?, target_categories = ?, days_of_week = ?,
            updated_at = NOW()
      WHERE tenant_id = ? AND id = ?`,
    [
...
export async function calculateLoyaltyAward(
  ...
  if (!customerId) return { ...baseResult, reason: "No customer selected." };

  const settingsRows = await rowsFromRunner(activeRunner, `SELECT business FROM app_settings WHERE tenant_id = ? LIMIT 1`, [tenantId]);
  const business = parseBusiness(settingsRows[0]);
  if (!business?.enableLoyalty) return { ...baseResult, reason: "Loyalty is disabled." };

  const customerRows = await rowsFromRunner(activeRunner,
    `SELECT id, loyalty_points, loyalty_member_status, loyalty_tier_id
       FROM customers
      WHERE tenant_id = ? AND id = ?
      LIMIT 1 FOR UPDATE`,
    [tenantId, customerId],
  ...
  if (!customer) return { ...baseResult, enabled: true, reason: "Customer is not a loyalty member." };
  ...
  const tiers = (await rowsFromRunner(activeRunner,
    `SELECT * FROM loyalty_tiers WHERE tenant_id = ? AND status = 'active' ORDER BY min_points DESC, name ASC`,
    [tenantId],
  )).map(normalizeTier);
  ...
  const rules = (await rowsFromRunner(activeRunner,
    `SELECT * FROM loyalty_reward_rules WHERE tenant_id = ? AND status = 'active' ORDER BY rule_type ASC, updated_at DESC`,
    [tenantId],
  )).map(normalizeRule);
  ...
  const matchedRules: Array<{ id: string; name: string; points: number }> = [];
  if (basePoints > 0) matchedRules.push({ id: baseRule?.id || "base", name: baseRule?.name || "Base earning", points: basePoints });

  let extraPoints = 0;
  for (const rule of rules.filter(rule => rule.ruleType !== "base")) {
    if (!activeOnDate(rule, now)) continue;
    if (earningTotal < rule.minSubtotal) continue;
