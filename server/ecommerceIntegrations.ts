...
function buildTargetExports(products: ReturnType<typeof normalizedProduct>[], generatedDate: string) {
    ...
    const targets = [
        ...
        },
        {
            ...
            header: ["ID", "Type", "SKU", "Name", "Published", "Is featured$1", "Visibility in catalog", "Short description", "Description", "Tax status", "In stock$1", "Stock", "Regular price", "Categories", "Images"],
            rows: wooRows,
        },
        {
            targetId: "takealot",
...
export async function getEcommerceMarketplaceExport(tenantId: string, filters: EcommerceExportFilters = {}) {
    ...
    const rows = await query<ProductRow>(`SELECT
       id,
       name,
       price,
     ...
     FROM products
     WHERE tenant_id = $1
     ORDER BY category ASC, name ASC`, [tenantId]);
