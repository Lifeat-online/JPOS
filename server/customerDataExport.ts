...
function consentSummary(consents: CustomerConsentMap) {
  return Object.fromEntries(
    Object.entries(consents).map(([type, record]) => [type, record.status || "unknown"]),
  );
}
...
export async function getCustomerDataExport(tenantId: string, customerId: string) {
  ...
  const sales = await query<CustomerSaleRow>(
    `SELECT
       id,
       staff_id AS staffId,
       total,
  ...
  const saleItems = await listSaleChildren<any>(
    "sale_items",
    saleIds,
    `SELECT
       id,
  ...
  const salePayments = await listSaleChildren<any>(
    "sale_payments",
    saleIds,
    `SELECT
       id,
