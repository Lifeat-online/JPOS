...
export async function addStaffCoachingNote(tenantId: string, input: CoachingNoteInput, actor: Actor = {}) {
    ...
    if (!title)
        throw new Error("Coaching title is required.");
    if (!note)
        throw new Error("Coaching note is required.");
    ...
    const source = ["manager", "ai", "performance"].includes(clean(input.source))
        ? clean(input.source)
        : "manager";
    const staffRows = await query<any>(`SELECT id, name FROM staff WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [tenantId, staffId]);
    const staff = staffRows[0];
    if (!staff)
        throw new Error("Staff member not found.");
    const noteId = id("coach");
    await query(`INSERT INTO staff_coaching_notes (
...
export async function getStaffPerformanceReport(tenantId: string, filters: StaffPerformanceFilters = {}) {
    ...
    const staffWhere = ["tenant_id = $1"];
    if (filters.staffId) {
        staffWhere.push("id = $1");
        staffParams.push(filters.staffId);
    }
    const staffRows = await query<any>(`SELECT id, name, role, status
       FROM staff
      WHERE ${staffWhere.join(" AND ")}
      ORDER BY name ASC`, staffParams);
    const rowsByStaff = new Map<string, ReturnType<typeof emptyStaffRow>>();
    for (const staff of staffRows)
        rowsByStaff.set(String(staff.id), emptyStaffRow(staff));
    const rangeParams: any[] = [tenantId, range.fromSql, range.toSql];
    const scopedStaffFilter = filters.staffId ? "AND (staff_id = $1 OR refunded_by = $2 OR voided_by = $3)" : "";
    if (filters.staffId)
        rangeParams.push(filters.staffId, filters.staffId, filters.staffId);
    const sales = await query<any>(`SELECT
        id,
        staff_id AS staffId,
        total,
      ...
      WHERE tenant_id = $1
        ...
        AND created_at <= $3
        ${scopedStaffFilter}
      ORDER BY created_at ASC`, rangeParams);
    ...
    const itemStaffFilter = filters.staffId ? "AND s.staff_id = $1" : "";
    if (filters.staffId)
        itemParams.push(filters.staffId);
    const itemRows = await query<any>(`SELECT
      ...
      WHERE s.tenant_id = $1
        ...
        ${itemStaffFilter}
      ORDER BY COALESCE(si.delivered_at, si.ready_at, si.accepted_at, si.ordered_at, s.created_at) ASC`, itemParams);
    ...
    const scoreStaffFilter = filters.staffId ? "AND staff_id = $1" : "";
    if (filters.staffId)
        scoreParams.push(filters.staffId);
    const scoreRows = await query<any>(`SELECT
        staff_id AS staffId,
      ...
      WHERE tenant_id = $1
        ${scoreStaffFilter}
      ORDER BY created_at DESC, score DESC`, scoreParams);
    for (const score of scoreRows) {
        ...
        row.coachingHistory.push(...row.aiScore.coachingNotes.map((note: string, index: number) => ({
    ...
    const noteStaffFilter = filters.staffId ? "AND staff_id = $1" : "";
    if (filters.staffId)
        noteParams.push(filters.staffId);
    const noteRows = await query<any>(`SELECT
        id,
        staff_id AS staffId,
        staff_name AS staffName,
