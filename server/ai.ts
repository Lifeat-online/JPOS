...
export function getAiProviderStatus(settings?: Partial<AiSettings>) {
    return {
        ...
        vertex: Boolean(getProviderApiKey({ ...settings, provider: "vertex" }) &&
...
export async function getAiSettings(tenantId: string): Promise<AiSettings> {
    const rows = await query<any>("SELECT * FROM ai_settings WHERE tenant_id = $1 LIMIT 1", [tenantId]);
    const row = rows[0];
    if (!row)
...
export async function saveAiSettings(tenantId: string, input: Partial<AiSettings>): Promise<AiSettings> {
    ...
    await query(`INSERT INTO ai_settings (
...
function getProviderApiKey(settings: Partial<AiSettings>) {
    ...
    if (configured) {
        ...
        if (!configured.startsWith("{"))
            return normalizeProviderApiKey(configured);
    }
    ...
    if (settings.provider === "openrouter")
        return normalizeProviderApiKey(process.env.OPENROUTER_API_KEY || "");
    return "";
}
...
export async function listAiModels(tenantId: string, input: Partial<AiSettings> = {}): Promise<AiModelOption[]> {
    ...
    if (settings.provider === "anythingllm")
        return listAnythingLlmModels(settings);
    if (settings.provider === "google")
        return listGoogleModels(settings);
    if (settings.provider === "vertex")
        return listVertexModels(settings);
    if (settings.provider === "openrouter")
        return listOpenRouterModels(settings);
    throw new Error(`Unsupported AI provider: ${settings.provider}`);
}
export async function testAiProviderContact(tenantId: string, input: Partial<AiSettings> & {
    message?: string;
    images?: string[];
    documents?: AiFileInput[];
} = {}) {
...
async function insertAudit(tenantId: string, action: string, requestedBy: string | null, status: string, details: any) {
    await query(`INSERT INTO ai_audit_log (id, tenant_id, action, requested_by, provider, status, details, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [id("aiaudit"), tenantId, action, requestedBy, "openai", status, JSON.stringify(details || {})]);
}
...
function buildBusinessMetrics(dataset: any) {
    ...
    const refundSales = sales.filter((sale) => (String(rowValue(sale, "refund_status", "refundStatus") || "none") !== "none" ||
    ...
    const offlineSales = sales.filter((sale) => (Boolean(rowValue(sale, "offline_event_id", "offlineEventId")) ||
    ...
    const paymentCountBySale = salePayments.reduce<Record<string, number>>((acc, payment) => {
        ...
        if (acc[saleId] > 1)
            splitPaymentSaleIds.add(saleId);
        return acc;
    }, {});
    ...
    for (const event of auditEvents) {
        ...
        if (localReceiptNumber)
            localReceiptNumbers.add(String(localReceiptNumber));
    }
    ...
    const pettyCashTotal = sumRows(managerCashMovements.filter((movement) => rowValue(movement, "movement_type", "movementType") === "petty_cash"), (movement) => rowValue(movement, "amount"));
    const payoutCashTotal = sumRows(managerCashMovements.filter((movement) => rowValue(movement, "movement_type", "movementType") === "payout"), (movement) => rowValue(movement, "amount"));
    ...
    const overdueStockTakeSessions = stockTakeSessions.filter((session) => (["active", "submitted"].includes(String(rowValue(session, "status") || "")) &&
    ...
    const payfastPaymentCount = salePayments.filter((payment) => (String(rowValue(payment, "method") || "").toLowerCase() === "payfast" ||
        String(rowValue(payment, "provider") || "").toLowerCase() === "payfast")).length + sales.filter((sale) => Boolean(rowValue(sale, "payfast_payment_id", "payfastPaymentId"))).length;
    return {
        sales: {
...
async function getBusinessDataset(tenantId: string) {
    const [products, staff, sales, saleItems, salePayments, cashSessions, cashMovements, managerCashMovements, cashCloseCheckpoints, cashCustodyTransfers, customers, customerPayoutRequests, staffPayoutRequests, auditEvents, stockMovements, managerTasks, stockTakeSessions, stockTakeItems, stockTakeRules, laybyOrders, laybyItems, laybyPayments, purchaseOrders, vendors, companionDevices, pushSubscriptions, promotions, integrationApiKeys, integrationWebhookEvents, configRows, packageRows,] = await Promise.all([
        query<any>("SELECT id, name, category, section, stock, min_stock, price, cost_price, workstation_id FROM products WHERE tenant_id = $1", [tenantId]),
        query<any>("SELECT id, name, role, status, wallet_balance FROM staff WHERE tenant_id = $1", [tenantId]),
        query<any>(`SELECT
       ...
       FROM sales WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 500`, [tenantId]),
        query<any>(`SELECT
       ...
       LIMIT 1000`, [tenantId]),
        query<any>("SELECT * FROM cash_sessions WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 150", [tenantId]),
        query<any>("SELECT * FROM cash_movements WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 300", [tenantId]),
        query<any>("SELECT * FROM manager_cash_movements WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 300", [tenantId]),
        query<any>("SELECT * FROM cash_close_checkpoints WHERE tenant_id = $1 ORDER BY business_date DESC LIMIT 30", [tenantId]),
        query<any>("SELECT * FROM cash_custody_transfers WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 100", [tenantId]),
        query<any>("SELECT id, name, wallet_balance, account_enabled, account_limit, account_balance, loyalty_points, created_at FROM customers WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 300", [tenantId]),
        query<any>("SELECT id, customer_id, customer_name, amount, status, created_at, processed_at FROM customer_payout_requests WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 100", [tenantId]),
        query<any>("SELECT id, staff_id, staff_name, customer_id, customer_name, amount, status, created_at, processed_at FROM payout_requests WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 100", [tenantId]),
        query<any>("SELECT id, action, entity_type, entity_id, related_sale_id, staff_id, staff_name, customer_id, source, details, created_at FROM audit_events WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 500", [tenantId]),
        query<any>("SELECT id, item_type, product_id, bulk_item_id, item_name, quantity_delta, previous_quantity, new_quantity, reason, reason_code, reference_type, reference_id, sale_id, staff_id, staff_name, note, created_at FROM stock_movements WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 500", [tenantId]),
        query<any>("SELECT id, task_type, title, summary, priority, status, source_type, source_id, related_sale_id, related_product_id, assigned_to, requested_by, decided_by, details, due_at, created_at, updated_at FROM manager_tasks WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 500", [tenantId]),
        query<any>("SELECT id, name, type, status, assigned_by, assigned_by_name, due_at, submitted_at, approved_at, approved_by, approved_by_name, created_at, updated_at FROM stock_take_sessions WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 150", [tenantId]),
        query<any>("SELECT id, session_id, product_id, product_name, expected_quantity, counted_quantity, variance_quantity, assigned_to, assigned_to_name, counted_by, counted_by_name, status, counted_at, confirmed_at, created_at, updated_at FROM stock_take_items WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 500", [tenantId]),
        query<any>("SELECT id, name, status, schedule_type, run_time, product_scope, product_count, category, assigned_to, assigned_to_name, last_run_for_date, last_run_at, created_at, updated_at FROM stock_take_rules WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 100", [tenantId]),
        query<any>("SELECT id, customer_id, customer_name, staff_id, staff_name, status, total_amount, deposit_amount, amount_paid, balance_due, refund_amount, forfeited_amount, due_date, completed_sale_id, created_at, updated_at FROM layby_orders WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 200", [tenantId]),
        query<any>(`SELECT
       ...
       LIMIT 300`, [tenantId]),
        query<any>("SELECT id, vendor_id, status, type, total_amount, expected_delivery_date, invoice_status, created_at, updated_at FROM purchase_orders WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 200", [tenantId]),
        query<any>("SELECT id, name, status, created_at, updated_at FROM vendors WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 200", [tenantId]),
        query<any>("SELECT id, device_id, device_name, workstation_id, default_mode, assigned_by, created_at, updated_at FROM companion_device_assignments WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 200", [tenantId]),
        query<any>("SELECT id, staff_id, device_label, disabled_at, last_seen_at, created_at, updated_at FROM push_subscriptions WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 200", [tenantId]),
        query<any>("SELECT id, name, code, status, discount_type, discount_value, applies_to, target_product_ids, target_categories, starts_at, ends_at, redemption_count FROM promotions WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 100", [tenantId]),
        query<any>("SELECT id, name, status, scopes, last_used_at, created_at, updated_at FROM integration_api_keys WHERE tenant_id = $1 ORDER BY updated_at DESC LIMIT 100", [tenantId]),
        query<any>("SELECT id, source, event_type, status, error_message, processed_at, created_at FROM integration_webhook_events WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 200", [tenantId]),
        query<any>("SELECT business FROM app_settings WHERE tenant_id = $1 LIMIT 1", [tenantId]),
        query<any>("SELECT COUNT(*) AS active_registers FROM cash_sessions WHERE tenant_id = $1 AND status = 'open'", [tenantId]),
    ]);
    return {
        products,
        staff,
...
export function buildDeterministicInsights(tenantId: string, dataset: any): AiInsight[] {
    ...
    insights.push(makeInsight(tenantId, "sales", metrics.sales.revenue > 0 ? "success" : "info", "Sales pulse", `Completed revenue is R${metrics.sales.revenue.toFixed(2)} across ${metrics.sales.completedCount} completed sales.`, `Use the R${metrics.sales.averageOrder.toFixed(2)} average order value alongside payment mix, table/tab, and offline-sale signals when shaping upsells or shift targets.`, [
    ...
    insights.push(makeInsight(tenantId, "sales", metrics.performance.upsellCandidates.length > 0 ? "info" : "success", "Cashier upsell prompts", metrics.performance.upsellCandidates.length > 0
        ...
        : "No high-confidence upsell prompt is available yet because there is not enough completed sale and margin history.", metrics.performance.upsellCandidates.length > 0
    ...
    ].slice(0, 5), metrics.performance.upsellCandidates.length > 0 ? 82 : 68));
    insights.push(makeInsight(tenantId, "restaurant", metrics.performance.menuCandidates.length > 0 || metrics.stock.shrinkageOrWastageCount > 0 ? "warning" : "info", "Menu/product optimization", metrics.performance.menuCandidates.length > 0
        ...
        : "Menu and product optimization needs more sale, cost, recipe, stock, and wastage history before ranking changes confidently.", "Review low-margin categories, recipe-cost gaps, low-stock products, shrinkage/wastage, and time-of-day demand before changing menus, prices, bundles, or product placement.", [
    ...
    ].slice(0, 6), metrics.performance.menuCandidates.length > 0 ? 86 : 72));
    if (metrics.tasks.openCount > 0) {
        insights.push(makeInsight(tenantId, "staff", metrics.tasks.highPriorityCount > 0 ? "critical" : "warning", "Manager action queue", `${metrics.tasks.openCount} Action Center task${metrics.tasks.openCount === 1 ? "" : "s"} are open or in review, including ${metrics.tasks.highPriorityCount} high-priority item${metrics.tasks.highPriorityCount === 1 ? "" : "s"}.`, "Work the queue from highest priority first, and convert AI recommendations into tracked manager tasks before anyone changes stock, cash, refunds, or settings.", [
    ...
    if (metrics.offline.auditEventCount > 0 || metrics.tasks.offlineSyncCount > 0 || metrics.sales.offlineSaleCount > 0) {
        insights.push(makeInsight(tenantId, "sales", metrics.offline.conflictEventCount > 0 || metrics.tasks.offlineSyncCount > 0 ? "warning" : "info", "Offline sync health", `${metrics.sales.offlineSaleCount} recent sale${metrics.sales.offlineSaleCount === 1 ? "" : "s"} carry offline sync metadata, with ${metrics.offline.conflictEventCount} conflict event${metrics.offline.conflictEventCount === 1 ? "" : "s"}.`, metrics.offline.conflictEventCount > 0 || metrics.tasks.offlineSyncCount > 0
    ...
    if (metrics.stock.lowStockCount > 0) {
        insights.push(makeInsight(tenantId, "stock", metrics.stock.lowStockCount > 5 || metrics.stock.shrinkageOrWastageCount > 0 ? "critical" : "warning", "Stock needs attention", `${metrics.stock.lowStockCount} products are at or below their minimum stock level, with ${metrics.stock.countCorrectionCount} recent count-correction movement${metrics.stock.countCorrectionCount === 1 ? "" : "s"}.`, "Prioritize low-stock items that also show count corrections, shrinkage, wastage, or overdue supplier orders.", [
        ...
        insights.push(makeInsight(tenantId, "stock", metrics.stock.shrinkageOrWastageCount > 0 || metrics.stock.countCorrectionCount > 0 ? "warning" : "success", metrics.stock.shrinkageOrWastageCount > 0 ? "Stock exceptions" : "Stock looks stable", metrics.stock.shrinkageOrWastageCount > 0
            ...
            : "No products are currently below their configured minimum stock.", metrics.stock.shrinkageOrWastageCount > 0
    ...
    if (metrics.stocktake.submittedSessionCount > 0 || metrics.stocktake.varianceItemCount > 0 || metrics.stocktake.overdueSessionCount > 0) {
        insights.push(makeInsight(tenantId, "stock", metrics.stocktake.overdueSessionCount > 0 || metrics.stocktake.varianceItemCount > 5 ? "warning" : "info", "Stocktake variance watch", `${metrics.stocktake.varianceItemCount} stocktake item${metrics.stocktake.varianceItemCount === 1 ? "" : "s"} have variance, totalling ${metrics.stocktake.absoluteVarianceQuantity.toFixed(3)} units absolute variance.`, "Approve clean stocktakes promptly and send high-variance items to recount before posting count-correction movements.", [
    ...
    if (metrics.stock.activePurchaseOrderCount > 0 || metrics.stock.overduePurchaseOrderCount > 0) {
        insights.push(makeInsight(tenantId, "stock", metrics.stock.overduePurchaseOrderCount > 0 ? "warning" : "info", "Purchasing pipeline", `${metrics.stock.activePurchaseOrderCount} purchase order${metrics.stock.activePurchaseOrderCount === 1 ? "" : "s"} are still draft or sent, with ${metrics.stock.overduePurchaseOrderCount} overdue expected deliver${metrics.stock.overduePurchaseOrderCount === 1 ? "y" : "ies"}.`, "Use open purchase orders together with low-stock and receiving gaps before creating more supplier work.", [
    ...
    insights.push(makeInsight(tenantId, "cash", cashNeedsReview ? "warning" : "success", "Cash control", cashNeedsReview
        ...
        : "No cash variance is visible in recent submitted, closed, or EOD checkpoint data.", cashNeedsReview
    ...
    insights.push(makeInsight(tenantId, "staff", metrics.sales.refundCount > 0 || metrics.sales.voidCount > 0 || cashNeedsReview || metrics.tasks.stockVarianceCount > 0 ? "warning" : "info", "Exception insight watch", `Refunds, voids, cash variance, staff task exceptions, and stock variance signals are being tracked for manager review.`, "Use this as an approval-first exception queue: assign a manager task before refund follow-up, cash correction, stock correction, discounting, or staff coaching changes are applied.", [
    ...
    if (metrics.layby.activeCount > 0) {
        insights.push(makeInsight(tenantId, "customer", metrics.layby.overdueCount > 0 ? "warning" : "info", "Lay-by exposure", `${metrics.layby.activeCount} active lay-by order${metrics.layby.activeCount === 1 ? "" : "s"} hold R${metrics.layby.balanceDue.toFixed(2)} balance due and ${metrics.layby.reservedQuantity.toFixed(3)} reserved unit${metrics.layby.reservedQuantity === 1 ? "" : "s"}.`, metrics.layby.overdueCount > 0
    ...
    if (metrics.restaurant.enabled || metrics.restaurant.openOrderCount > 0 || metrics.restaurant.openTabCount > 0) {
        ...
        insights.push(makeInsight(tenantId, "restaurant", metrics.restaurant.openOrderCount > 8 || metrics.restaurant.kitchenItemCount > 12 || liveTiming.staleTimerCount > 0 ? "warning" : "info", "Restaurant load", `${metrics.restaurant.openOrderCount} orders are open, kitchen, or pending, with ${metrics.restaurant.openTabCount} open tab${metrics.restaurant.openTabCount === 1 ? "" : "s"}.`, liveTiming.staleTimerCount > 0
    ...
    insights.push(makeInsight(tenantId, "integration", metrics.integration.failedWebhookEventCount > 0 ? "warning" : "info", "Integration health", `${metrics.integration.configuredConnectorCount} connector signal${metrics.integration.configuredConnectorCount === 1 ? "" : "s"} are visible across payment, API-key, webhook, accounting, delivery, e-commerce, and ERP paths.`, metrics.integration.failedWebhookEventCount > 0
    ...
    insights.push(makeInsight(tenantId, "customer", metrics.customer.overLimitAccountCount > 0 ? "warning" : customerExposure > 0 ? "info" : "success", "Customer and wallet exposure", customerExposure > 0
        ? `Wallet, account, and payout exposure totals R${customerExposure.toFixed(2)} across current customer and payout records.`
        : "No customer wallet, account, or payout exposure is currently visible.", metrics.customer.overLimitAccountCount > 0
    ...
    insights.push(makeInsight(tenantId, "staff", "info", "Staff coverage", `${metrics.staff.activeCount} active staff members are configured and ${metrics.sales.openOrderCount} orders are currently active.`, "Use staff scores together with cash variance, stocktake participation, refund/void patterns, and Action Center follow-through for coaching.", [
...
async function replaceInsights(tenantId: string, insights: AiInsight[]) {
    await query("DELETE FROM ai_insights WHERE tenant_id = $1 AND status = 'open'", [tenantId]);
    for (const insight of insights) {
        await query(`INSERT INTO ai_insights (
...
export async function listInsights(tenantId: string): Promise<AiInsight[]> {
    const rows = await query<any>(`SELECT * FROM ai_insights WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 50`, [tenantId]);
...
export async function deleteInsight(tenantId: string, insightId: string) {
    const result: any = await query("DELETE FROM ai_insights WHERE tenant_id = $1 AND id = $2", [tenantId, insightId]);
    return {
...
export function buildDeterministicStaffScores(tenantId: string, dataset: any, periodStart: Date, periodEnd: Date): StaffScore[] {
    ...
    return staffRows
        ...
        const staffRefunds = sales.filter((sale: any) => (rowValue(sale, "refunded_by", "refundedBy") === staff.id ||
            (rowValue(sale, "staff_id", "staffId") === staff.id && String(rowValue(sale, "refund_status", "refundStatus") || "none") !== "none")));
        const staffVoids = sales.filter((sale: any) => (rowValue(sale, "voided_by", "voidedBy") === staff.id ||
        ...
        const staffTasks = managerTasks.filter((task: any) => (rowValue(task, "assigned_to", "assignedTo") === staff.id ||
            rowValue(task, "requested_by", "requestedBy") === staff.id ||
            rowValue(task, "decided_by", "decidedBy") === staff.id));
        const openAssignedTasks = staffTasks.filter((task: any) => (rowValue(task, "assigned_to", "assignedTo") === staff.id &&
        ...
        const countedStockItems = stockTakeItems.filter((item: any) => (rowValue(item, "counted_by", "countedBy") === staff.id ||
        ...
        const offlineIssues = auditEvents.filter((event: any) => (String(rowValue(event, "action") || "").startsWith("offline.") &&
            rowValue(event, "staff_id", "staffId") === staff.id));
        const noSaleMovements = cashMovements.filter((movement: any) => (rowValue(movement, "staff_id", "staffId") === staff.id &&
        ...
        const score = clamp(componentScores.salesThroughput * 0.2 +
...
async function replaceStaffScores(tenantId: string, scores: StaffScore[]) {
    await query("DELETE FROM ai_staff_scores WHERE tenant_id = $1", [tenantId]);
    for (const score of scores) {
        await query(`INSERT INTO ai_staff_scores (
...
export async function listStaffScores(tenantId: string): Promise<StaffScore[]> {
    const rows = await query<any>("SELECT * FROM ai_staff_scores WHERE tenant_id = $1 ORDER BY score DESC, staff_name ASC", [tenantId]);
    return rows.map((r: any) => ({
        id: r.id,
        tenantId: r.tenant_id,
        staffId: r.staff_id,
...
async function getVertexBearerToken(settings: Partial<AiSettings>) {
    ...
    if (!response.ok)
        throw new Error(body?.error_description || body?.error || `Vertex service account auth failed [${response.status}]`);
...
async function callGeminiApiKeyWithFiles(key: string, model: string, payload: any, images: string[], documents: AiFileInput[], label: string): Promise<string> {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent$1key=${encodeURIComponent(key)}`, {
        method: "POST",
...
async function callVertexWithFiles(settings: AiSettings, payload: any, images: string[], documents: AiFileInput[]): Promise<string> {
    ...
    if (!location)
        throw new Error("Google Vertex AI location is required");
    ...
    const querySuffix = token ? "" : `$1key=${encodeURIComponent(key)}`;
    const response = await fetch(`https://${location}-aiplatform.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/locations/${encodeURIComponent(location)}/publishers/google/models/${encodeURIComponent(model)}:generateContent${querySuffix}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
...
async function listOpenAiModels(settings: Partial<AiSettings>) {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(body?.error?.message || `OpenAI model list failed [${response.status}]`);
...
async function listOllamaModels(settings: Partial<AiSettings>) {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(body?.error || `Ollama model list failed [${response.status}]`);
...
async function listAnythingLlmModels(settings: Partial<AiSettings>) {
    ...
    if (!key)
        throw new Error("AnythingLLM API key is not configured");
...
async function listGoogleModels(settings: Partial<AiSettings>) {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models$1key=${encodeURIComponent(key)}`);
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(body?.error?.message || `Google model list failed [${response.status}]`);
...
async function listVertexModels(settings: Partial<AiSettings>) {
    ...
    const token = await getVertexBearerToken(settings);
    if (!token && !key)
        throw new Error("Google Vertex AI access token, service account JSON, or API key is not configured");
    const response = await fetch(`https://${location}-aiplatform.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/locations/${encodeURIComponent(location)}/publishers/google/models${token ? "" : `?key=${encodeURIComponent(key)}`}`, {
    ...
    if (!response.ok) {
        ...
        if ((/missing authentication header/i.test(message) || isVertexBlockedError(message)) && key)
            return listGoogleModels({ ...settings, provider: "google", apiKey: key });
        throw new Error(isVertexBlockedError(message) ? vertexBlockedGuidance(message) : message);
    }
...
async function listOpenRouterModels(settings: Partial<AiSettings>) {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(body?.error?.message || `OpenRouter model list failed [${response.status}]`);
    return uniqueModels((body.data || []).map((model: any) => ({
        id: model.id,
...
async function callAnythingLlmText(settings: AiSettings, message: string, images: string[] = [], documents: AiFileInput[] = []): Promise<string> {
    ...
    if (!workspaceSlug)
        throw new Error("AnythingLLM workspace slug is not configured");
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(providerErrorMessage(body, `AnythingLLM test failed [${response.status}]`, response.status));
...
async function callAnythingLlm(settings: AiSettings, payload: any): Promise<string> {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(body?.error || body?.message || `AnythingLLM request failed [${response.status}]`);
    return body.textResponse || body.response || body.message || "";
}
async function callGoogleText(settings: AiSettings, message: string, images: string[] = [], documents: AiFileInput[] = []): Promise<string> {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(providerErrorMessage(body, `Google AI test failed [${response.status}]`, response.status));
    return body.candidates?.[0]?.content?.parts?.map((part: any) => part.text || "").join("") || "";
}
async function callGoogle(settings: AiSettings, payload: any): Promise<string> {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
        throw new Error(body?.error?.message || `Google AI request failed [${response.status}]`);
    return body.candidates?.[0]?.content?.parts?.map((part: any) => part.text || "").join("") || "";
}
async function callVertexText(settings: AiSettings, message: string, images: string[] = [], documents: AiFileInput[] = []): Promise<string> {
    ...
    const response = await fetch(`https://${location}-aiplatform.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/locations/${encodeURIComponent(location)}/publishers/google/models/${encodeURIComponent(model)}:generateContent${token ? "" : `?key=${encodeURIComponent(key)}`}`, {
        method: "POST",
    ...
    if (!response.ok) {
        ...
        if ((/missing authentication header/i.test(error) || isVertexBlockedError(error)) && key)
            return callGoogleText({ ...settings, provider: "google", apiKey: key }, message, images, documents);
...
async function callVertex(settings: AiSettings, payload: any): Promise<string> {
    ...
    if (!response.ok) {
        ...
        if ((/missing authentication header/i.test(message) || isVertexBlockedError(message)) && key)
            return callGoogle({ ...settings, provider: "google", apiKey: key }, payload);
        throw new Error(isVertexBlockedError(message) ? vertexBlockedGuidance(message) : message);
    }
    return body.candidates?.[0]?.content?.parts?.map((part: any) => part.text || "").join("") || "";
}
