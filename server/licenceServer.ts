...
licenceRouter.post("/admin/licence/revoke", requireAdminKey, async (req, res) => {
  try {
    ensureServerConfigured();
    const { licenceId, reason } = req.body as { licenceId?: string; reason?: string };
    if (!licenceId) {
    ...
    await query(
      "UPDATE licences SET revoked = ?, revoked_at = NOW(), revoked_reason = ? WHERE licence_id = ?",
      [1, reason || null, licenceId]
    );
...
licenceRouter.get("/licence/validate/:licenceId", validateLimiter, async (req, res) => {
  try {
    ...
    const rows = await query<LicenceRecord>(
      "SELECT revoked, revoked_reason, expires_at FROM licences WHERE licence_id = ? LIMIT 1",
      [licenceId]
    );
    const record = rows[0];
    if (!record) {
