...
function normalizePromotionPayload(input: Partial<Promotion>, actor: PromotionActor = {}) {
  ...
  if (discountValue <= 0) throw new Error("Promotion discount value must be greater than zero.");
  if (discountType === "percent" && discountValue > 100) throw new Error("Percentage promotions cannot exceed 100%.");
...
function targetSubtotalForPromotion(promotion: Promotion, items: PromotionSaleItem[]) {
  ...
  const matching = items.filter(item => {
    ...
    if (promotion.appliesTo === "products") return productTargets.has(productId);
...
function discountForPromotion(promotion: Promotion, targetSubtotal: number, cartSubtotal: number) {
  if (targetSubtotal <= 0 || cartSubtotal <= 0) return 0;
...
async function findPromotionForValidation(
  runner: QueryRunner,
  tenantId: string,
  ...
  if (!code && !promotionId) return null;
  ...
  const rows = await rowsFromRunner(
    runner,
    `SELECT * FROM promotions WHERE tenant_id = ? AND (${clauses.join(" OR ")}) LIMIT 1${lock ? " FOR UPDATE" : ""}`,
    params,
  );
...
async function redemptionCount(
  ...
  const rows = await rowsFromRunner(
    runner,
    `SELECT COUNT(*) AS count FROM promotion_redemptions WHERE tenant_id = ? AND promotion_id = ?${customerId ? " AND customer_id = ?" : ""}`,
    params,
  );
...
export async function listPromotions(tenantId: string): Promise<Promotion[]> {
  const rows = await rowsFromRunner(
    dbRunner(),
    `SELECT * FROM promotions WHERE tenant_id = ? ORDER BY updated_at DESC, name ASC`,
    [tenantId],
  );
...
export async function validatePromotionForSale(
  runner: QueryRunner | null,
  tenantId: string,
  input: PromotionValidationInput,
  options: { lock?: boolean; assertClientDiscount?: boolean } = {},
  ...
  if (!promotion) return emptyResult("Promotion code was not found.");
  if (promotion.status !== "active") return { ...emptyResult("Promotion is inactive."), promotion };

  const now = input.now ? new Date(input.now) : new Date();
  if (dateIsAfter(promotion.startsAt, now)) return { ...emptyResult("Promotion is not active yet."), promotion };
  if (dateIsBefore(promotion.endsAt, now)) return { ...emptyResult("Promotion has expired."), promotion };

  const items = Array.isArray(input.items) ? input.items : [];
  const cartSubtotal = numeric(input.subtotal ?? input.totalBeforeDiscount ?? items.reduce((sum, item) => sum + lineGross(item), 0));
  if (cartSubtotal <= 0) return { ...emptyResult("Promotion needs a cart total greater than zero."), promotion };
  ...
  if (promotion.customerScope === "selected") {
    if (!customerId) return { ...emptyResult("Promotion is limited to selected customers."), promotion };
    const allowed = new Set(promotion.targetCustomerIds.map(item => item.toLowerCase()));
    if (!allowed.has(customerId.toLowerCase())) return { ...emptyResult("Promotion is not valid for this customer."), promotion };
  }
  ...
  if (targetSubtotal <= 0) return { ...emptyResult("Promotion does not match any cart items."), promotion };
  const discountAmount = discountForPromotion(promotion, targetSubtotal, cartSubtotal);
  if (discountAmount <= 0) return { ...emptyResult("Promotion did not produce a discount."), promotion };
...
export async function recordPromotionRedemption(
  ...
  if (!validation.valid || !validation.promotion || validation.discountAmount <= 0) return;
  ...
  await runner.query(
    `UPDATE promotions SET redemption_count = redemption_count + 1, updated_at = NOW() WHERE tenant_id = ? AND id = ?`,
