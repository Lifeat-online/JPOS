...
function corsOriginAllowed(origin: string | undefined): boolean {
    if (!origin)
        return true; // same-origin / curl / no Origin header
    const allow = parseCorsOrigins();
    if (allow.length === 0)
        return false; // explicit allowlist only when configured
 ...
 */
export function sendSafeError(res: Response, status: number, publicMessage: string, err: unknown, req: Request): void {
