...
type StepType = "create_vendor" | "create_bulk_item" | "create_sales_unit" | "create_purchase_order" | "receive_invoice" | "book_stock" | "review_event_demand";
...
async function recordAiAgentAudit(tenantId: string, action: string, status: string, actor: InventoryAgentActor | undefined, details: Record<string, any> = {}) {
    try {
        await query(`INSERT INTO ai_audit_log (
...
async function persistInventoryAgentRun(tenantId: string, proposal: InventoryAgentProposal, actor?: InventoryAgentActor) {
    await query(`INSERT INTO ai_agent_runs (
    ...
    for (const item of proposal.steps) {
        await query(`INSERT INTO ai_agent_run_steps (
...
async function saveProposalRun(tenantId: string, proposal: InventoryAgentProposal, options: InventoryAgentRunOptions = {}) {
...
async function loadAgentRunApplyResult(tenantId: string, runId?: string | null): Promise<InventoryAgentApplyResult | null> {
    if (!runId)
        return null;
    const rows = await query<any>(`SELECT id, status, apply_result AS applyResult
       FROM ai_agent_runs
...
async function loadPersistedRunSteps(tenantId: string, runId: string, requestedSteps: InventoryAgentStep[]) {
...
async function updateRunStatus(tenantId: string, runId: string | null | undefined, status: "draft" | "applying" | "completed" | "failed", data: {
    fullAutopilot?: boolean;
    applyResult?: InventoryAgentApplyResult | null;
} = {}) {
    if (!runId)
        return;
    ...
    await query(`UPDATE ai_agent_runs SET ${fields.join(", ")} WHERE tenant_id = $1 AND id = $2`, values);
}
async function updateRunStepStatus(tenantId: string, runId: string | null | undefined, stepId: string, status: "pending" | "approved" | "applied" | "skipped" | "failed", actor: InventoryAgentActor | undefined, data: {
    approved?: boolean;
    result?: any;
    reason?: string | null;
} = {}) {
    ...
    if (data.approved !== undefined) {
        fields.push("approved = $1");
        values.push(data.approved ? 1 : 0);
    }
    if (status === "approved" || data.approved) {
        fields.push("approved_by = $1", "approved_by_name = $1", "approved_at = COALESCE(approved_at, NOW())");
    ...
    if (data.reason !== undefined) {
        fields.push("skip_reason = $1");
        values.push(data.reason || null);
    }
    values.push(tenantId, runId, stepId);
    await query(`UPDATE ai_agent_run_steps SET ${fields.join(", ")} WHERE tenant_id = $1 AND run_id = $2 AND step_id = $3`, values);
}
...
function buildReceiptItemsFromPurchaseOrderPayload(payload: Record<string, any> | undefined | null) {
    ...
    if (!sourceItems.length)
        return { items: undefined as any[] | undefined, reason: null as string | null };
...
async function getProductVelocity(tenantId: string) {
    const rows = await query<any>(`SELECT
     ...
     GROUP BY si.product_id, si.product_name`, [tenantId]).catch(async () => query<any>(`SELECT
       si.product_id AS productId,
       si.product_name AS productName,
       SUM(si.quantity) AS quantitySold,
       COUNT(DISTINCT s.id) AS saleCount,
     ...
     WHERE s.tenant_id = $1
       AND s.status = 'completed'
        AND s.created_at >= NOW() - INTERVAL '90 days'
     GROUP BY si.product_id, si.product_name`, [tenantId]));
...
export async function generateInventoryAgentProposal(tenantId: string, body: any, options: InventoryAgentRunOptions = {}): Promise<InventoryAgentProposal> {
    ...
    if (mode === "low_stock") {
        ...
        if (items.length) {
            steps.push(step("create_purchase_order", `Create draft low-stock purchase order for ${items.length} sales unit item${items.length === 1 ? "" : "s"}`, { vendorId: vendors.find((v) => v.status === "active")?.id || null, status: "draft", type: "once_off", items }, ["Products are at or below minimum stock", "Reorder quantities include 90-day sales velocity where available"], "medium", 0.82));
        }
        for (const item of lowBulk) {
            steps.push(step("create_bulk_item", `Review low bulk stock: ${item.name}`, { existingBulkItemId: item.id, name: item.name, currentStock: item.stock, minStock: item.minStock, suggestedQuantity: Math.max(1, toNumber(item.minStock) * 2 - toNumber(item.stock)) }, ["Bulk item is at or below minimum stock"], "low", 0.76));
        }
    ...
    if (mode === "event") {
        ...
        const steps = [
            step("review_event_demand", `Estimate event stock needs for ${expectedPeople || "the expected"} guests`, {
            ...
            }, ["Manager supplied event details", "Copilot can compare against current stock and sales movement"], "medium", expectedPeople ? 0.74 : 0.48),
        ];
        return saveProposalRun(tenantId, {
            id: proposalId(mode),
            mode,
    ...
    const steps: InventoryAgentStep[] = [
        step("create_vendor", existingVendor ? `Use existing vendor: ${existingVendor.name}` : "Create invoice supplier as vendor", { vendorId: existingVendor?.id || null, vendorName: extractedVendorName, name: extractedVendorName, contactPerson: "", email: "", phone: "", address: "", status: "active", documentNames }, uploadEvidence, existingVendor ? "low" : "medium", extractedVendorName ? 0.78 : (notes || imageCount || documentCount ? 0.48 : 0.32))
    ];
    for (const [index, line] of extractedLines.entries()) {
        ...
        if (!existingBulk) {
            steps.push(step("create_bulk_item", `Create ${line.itemType} stock item: ${line.productName}`, {
            ...
            }, ["AI extracted this line from the uploaded invoice", `Quantity: ${line.quantity}`, `Unit cost: ${line.unitCost}`], line.confidence >= 0.75 ? "medium" : "high", line.confidence));
        }
        const existingProduct = (products as any[]).find((product) => sameName(product.name, line.productName) || (line.barcode && product.barcode === line.barcode));
        if (!existingProduct && line.sellable) {
            steps.push(step("create_sales_unit", `Create sales unit item: ${line.productName}`, {
                name: line.productName,
                ...
                invoiceLineIndex: index,
            }, ["AI marked this line as sellable or did not rule it out", "Selling price defaults to cost plus 35% and can be edited"], "high", Math.min(line.confidence, 0.68)));
        }
    }
    if (extractedLines.length === 0) {
        steps.push(step("create_bulk_item", "Review invoice lines for bulk or single stock items", { invoiceLineCandidates: body?.invoiceLines || [], createMissingOnly: true }, ["AI could not extract line items", "Human must confirm pack size, unit, cost, and barcode before creation", documentCount ? "Invoice documents attached as review evidence" : "No invoice documents attached"], "high", documentCount || imageCount ? 0.35 : 0.25));
    }
    steps.push(step("create_purchase_order", extractedLines.length ? `Create draft purchase order from ${extractedLines.length} extracted line${extractedLines.length === 1 ? "" : "s"}` : "Create draft purchase order from approved invoice lines", {
    ...
    }, ["Draft PO is created from AI-extracted invoice lines", documentCount ? "PDF/document invoice can be used during review" : "No document invoice supplied"], extractedLines.length ? "medium" : "high", extractedLines.length ? 0.72 : 0.34), step("receive_invoice", "Receive invoice against approved purchase order", { invoiceNumber: aiExtraction?.invoiceNumber || body?.invoiceNumber || "", invoiceDate: aiExtraction?.invoiceDate || body?.invoiceDate || null, totals: aiExtraction?.totals || {} }, [aiExtraction?.invoiceNumber ? `Invoice number: ${aiExtraction.invoiceNumber}` : "Requires invoice number confirmation", pdfCount ? "PDF invoice attached" : "No PDF invoice attached"], "medium", aiExtraction?.invoiceNumber ? 0.68 : 0.42), step("book_stock", "Book approved stock quantities into inventory", { adjustments: extractedLines.map((line: any) => ({ name: line.productName, quantity: line.quantity * line.packSize, unit: line.unit, cost: line.unitCost })), source: "copilot_invoice" }, [extractedLines.length ? `${extractedLines.length} stock adjustment candidate${extractedLines.length === 1 ? "" : "s"}` : "Stock movement remains blocked until quantities and units are approved"], "high", extractedLines.length ? 0.62 : 0.4));
    return saveProposalRun(tenantId, {
        id: proposalId(mode),
        mode,
        status: "draft",
...
export async function applyApprovedInventoryAgentSteps(tenantId: string, steps: InventoryAgentStep[], options: InventoryAgentApplyOptions = {}): Promise<InventoryAgentApplyResult> {
    ...
    const purchaseOrdersByStepId = new Map<string, {
        id: string;
        payload: Record<string, any>;
    }>();
    const receivedPurchaseOrderIds = new Set<string>();
    let lastCreatedPurchaseOrder: {
        id?: string;
    } | null = null;
