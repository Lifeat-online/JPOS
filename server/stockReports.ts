...
function buildReceivedPurchaseOrdersQuery(tenantId: string, filters: StockReportFilters, limit: number) {
    const where = ['tenant_id = $1', "status = 'received'", 'received_at IS NOT NULL'];
...
export async function getStockValuationReport(tenantId: string, filters: StockReportFilters = {}) {
    ...
    const csvRows: unknown[][] = [
        [
            ...
            '',
            summary.productBookValue,
            summary.varianceQuantity,
            '',
        ...
        ...productRows.map((row: any) => [
            ...
            '',
            '',
            `Batch tracked ${row.batchTrackedQuantity}; unbatched ${row.unbatchedQuantity}`,
        ]),
        ...batchRows.map((row: any) => [
            ...
            '',
            '',
            `Received ${row.receivedQuantity} on ${reportDateLabel(row.receivedAt)}; expires ${reportDateLabel(row.expiryDate) || 'none'}`,
        ]),
        ...receivingRows.map((row: any) => [
            'receiving_impact',
            generatedAt,
            row.locationId,
            row.locationName,
        ...
        ...movements.map((row: any) => [
            ...
            '',
            row.reasonCode,
            `${row.movementCount} movements; in ${row.quantityIn}; out ${row.quantityOut}`,
        ]),
    ];
    const csv = [header, ...csvRows].map((row) => row.map(csvCell).join(',')).join('\n');
    const topProducts = [...productRows].sort((a: any, b: any) => b.productBookValue - a.productBookValue).slice(0, 12);
    const riskBatches = batchRows.filter((row: any) => row.status === 'expired' || row.status === 'expiring').slice(0, 12);
