...
async function getStaff(tenantId: string, staffId: string) {
  const rows = await query<any>(
    `SELECT id, name, role, pay_rate AS payRate, pay_type AS payType
       FROM staff
...
async function findShiftForClockIn(tenantId: string, staffId: string, at: string, shiftId?: string | null) {
  if (shiftId) {
    ...
    if (!shift || shift.staffId !== staffId || shift.status === "cancelled") throw new Error("Shift not found for this staff member.");
    return shift;
  }
  ...
  const rows = await query<any>(
    `SELECT
      ...
      LIMIT 1`,
    [tenantId, staffId, dateOnly(at)],
  );
  return rows[0] ? shiftFromRow(rows[0]) : null;
}

export async function listStaffShifts(tenantId: string, filters: { startDate?: string; endDate?: string; staffId?: string } = {}) {
  ...
  const rows = await query<any>(
    `SELECT
        ...
        created_by_name AS createdByName, created_at AS createdAt, updated_at AS updatedAt
       FROM staff_shifts
      WHERE tenant_id = ? AND shift_date BETWEEN ? AND ?${staffFilter}
      ORDER BY shift_date ASC, start_at ASC, staff_name ASC`,
    params,
  );
...
export async function createStaffShift(tenantId: string, input: StaffShiftInput, actor: Actor = {}) {
  ...
  if (!input.shiftDate || !input.startAt || !input.endAt) throw new Error("Shift date, start, and end are required.");
  if (minutesBetween(input.startAt, input.endAt) <= 0) throw new Error("Shift end must be after shift start.");
  ...
  await query(
    ...
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ${status === "published" ? "NOW()" : "NULL"}, ?, ?, ?, ?, NOW(), NOW())`,
    [
      shiftId,
      tenantId,
      staff.id,
...
export async function updateStaffShift(tenantId: string, shiftId: string, input: Partial<StaffShiftInput>, actor: Actor = {}) {
  ...
  if (input.staffId !== undefined && input.staffId !== current.staffId) {
    const staff = await getStaff(tenantId, input.staffId);
    fields.push("staff_id = ?", "staff_name = ?", "role = ?");
    values.push(staff.id, staff.name, staff.role || null);
  }
  if (input.shiftDate !== undefined) { fields.push("shift_date = ?"); values.push(dateOnly(input.shiftDate)); }
  if (input.startAt !== undefined) { fields.push("start_at = ?"); values.push(toSqlDateTime(input.startAt)); }
  if (input.endAt !== undefined) { fields.push("end_at = ?"); values.push(toSqlDateTime(input.endAt)); }
  if (input.status !== undefined) {
    const status: ShiftStatus = ["draft", "published", "cancelled", "completed"].includes(input.status) ? input.status : current.status;
    fields.push("status = ?");
    values.push(status);
    if (status === "published") {
      fields.push("published_at = COALESCE(published_at, NOW())", "published_by = ?", "published_by_name = ?");
      values.push(actor.staffId || current.publishedBy || null, actor.staffName || current.publishedByName || null);
    }
  }
  if (input.locationId !== undefined) { fields.push("location_id = ?"); values.push(input.locationId || null); }
  if (input.breakMinutesPlanned !== undefined) { fields.push("break_minutes_planned = ?"); values.push(Math.max(0, Math.floor(toNumber(input.breakMinutesPlanned)))); }
  if (input.notes !== undefined) { fields.push("notes = ?"); values.push(input.notes || null); }
  if (!fields.length) return current;
  fields.push("updated_at = NOW()");
  values.push(tenantId, shiftId);
  await query(`UPDATE staff_shifts SET ${fields.join(", ")} WHERE tenant_id = ? AND id = ?`, values);
  await recordAuditEventSafe({
    tenantId,
  ...
  await query(
    `UPDATE staff_shifts
      ...
      WHERE tenant_id = ?
        ...
        AND status = 'draft'`,
    [actor.staffId || null, actor.staffName || null, tenantId, startDate, endDate],
  );
...
async function getOpenAttendance(tenantId: string, staffId: string) {
  const rows = await query<any>(
    `SELECT
      ...
      WHERE tenant_id = ? AND staff_id = ? AND status = 'open'
...
export async function clockIn(tenantId: string, input: ClockInput, actor: Actor = {}) {
  ...
  if (open) throw new Error("Staff member is already clocked in.");
...
export async function startBreak(tenantId: string, staffId: string, atInput?: string | Date | null) {
  ...
  if (!attendance) throw new Error("Staff member is not clocked in.");
  if (attendance.breakStartedAt) throw new Error("Break is already running.");
  const at = toSqlDateTime(atInput);
  await query(
    `UPDATE staff_attendance
        SET break_started_at = ?, updated_at = NOW()
      WHERE tenant_id = ? AND id = ?`,
    [at, tenantId, attendance.id],
  );
  await recordAuditEventSafe({
    tenantId,
...
export async function endBreak(tenantId: string, staffId: string, atInput?: string | Date | null) {
  const attendance = await getOpenAttendance(tenantId, staffId);
  if (!attendance) throw new Error("Staff member is not clocked in.");
  if (!attendance.breakStartedAt) throw new Error("No active break to end.");
  const at = toSqlDateTime(atInput);
  ...
  await query(
    `UPDATE staff_attendance
        SET break_started_at = NULL,
            ...
            updated_at = NOW()
      WHERE tenant_id = ? AND id = ?`,
    [additionalBreak, tenantId, attendance.id],
  );
  await recordAuditEventSafe({
    tenantId,
  ...
  await query(
    `UPDATE staff_attendance
        SET status = 'closed',
            ...
            updated_at = NOW()
      WHERE tenant_id = ? AND id = ?`,
    [at, breakMinutes, workedMinutes, regularMinutes, overtimeMinutes, payrollAmount, input.note || null, tenantId, attendance.id],
  );
  if (attendance.shiftId) {
    await query(
      `UPDATE staff_shifts SET status = 'completed', updated_at = NOW() WHERE tenant_id = ? AND id = ? AND status <> 'cancelled'`,
      [tenantId, attendance.shiftId],
    );
  }
  await recordAuditEventSafe({
    tenantId,
...
export async function getTimesheetPayrollReport(tenantId: string, filters: { startDate?: string; endDate?: string; staffId?: string } = {}) {
  const startDate = dateOnly(filters.startDate);
  const endDate = dateOnly(filters.endDate || filters.startDate || startDate);
  ...
  const rows = await query<any>(
    `SELECT
      ...
      WHERE a.tenant_id = ?
        AND a.clock_in_at >= ?
        AND a.clock_in_at < ?${staffFilter}
      ORDER BY a.clock_in_at ASC, a.staff_name ASC`,
    params,
  );
