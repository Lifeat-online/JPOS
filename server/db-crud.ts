...
import {
  ...
  assertSaleNotInLockedTaxPeriod,
} from "./taxReports.js";
...
if (!PAYFAST_MERCHANT_ID || !PAYFAST_MERCHANT_KEY || !PAYFAST_PASSPHRASE) {
  console.warn(
    "⚠️  PayFast credentials not configured. Payment processing will fail.",
  );
}
// ─────────────────────────────────────────────────────────────────────────
// Helper Functions
...
function recipeEffectiveQuantity(row: any, saleQuantity = 1) {
  ...
  const wastePercent = recipeWastePercent(
    row.wastePercent ?? row.waste_percent,
  );
  return decimal(
    (baseQuantity / yieldQuantity) *
      (1 + wastePercent / 100) *
      Math.max(0, Number(saleQuantity || 0)),
    0,
  );
}
function serializeRecipeRow(row: any): RecipeItem {
  ...
  return {
    ...
    substituteGroup: nullableText(
      row.substituteGroup ?? row.substitute_group,
      64,
    ),
    substituteRank: Math.max(
      0,
      Math.floor(Number(row.substituteRank ?? row.substitute_rank ?? 0) || 0),
    ),
...
function chooseRecipeSubstitution(rows: any[], saleQuantity: number) {
  const sorted = [...rows].sort(
    (a, b) =>
      Number(a.substitute_rank ?? a.substituteRank ?? 0) -
        Number(b.substitute_rank ?? b.substituteRank ?? 0) ||
      String(a.bulk_item_id ?? a.bulkItemId).localeCompare(
        String(b.bulk_item_id ?? b.bulkItemId),
      ),
  );
  return (
    sorted.find(
      (row) =>
        decimal(row.stock ?? row.availableStock ?? row.available_stock, 0) >=
        recipeEffectiveQuantity(row, saleQuantity),
    ) || sorted[0]
  );
}
async function syncDefaultProductLocationStock(
  tenantId: string,
  productId: string,
  stock: any,
  minStock: any,
...
function paymentProviderValues(payment: any) {
  ...
  assertSafePaymentProviderEvidence(payment, {
    method,
    source: "sale payment",
  });
  const defaultStatus =
    method === "bnpl" ? "approved" : method === "qr" ? "confirmed" : null;
  return [
    nullableText(payment?.provider || payment?.paymentProvider, 64),
    nullableText(
      payment?.providerDeviceId || payment?.deviceId || payment?.terminalId,
      128,
    ),
    nullableText(
    ...
    nullableText(
      payment?.qrPayload || payment?.paymentLink || payment?.qrCode,
      1000,
    ),
  ];
}
  ...
  return Array.isArray(payments)
    ? payments
        ...
        .map((payment: any) => ({
          ...
          authorizationCode:
            payment.authorizationCode || payment.authCode || null,
...
async function applyWalletSalePayment(
  ...
  if (previousBalance < walletAmount) {
    throw new Error(
      `Customer wallet balance is R${previousBalance.toFixed(2)}, which is not enough for this wallet payment.`,
    );
  }
...
async function applyCheckoutSideEffects(
  ...
  if (context.loyaltyPoints !== undefined && context.customerId) {
    await conn.query(
      `UPDATE customers SET loyalty_points = $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`,
  ...
  if (
    ...
    context.cashMovements.length > 0 &&
  ...
  if (context.staffMetrics && context.staffId) {
    ...
    await conn.query(
      `UPDATE staff SET metrics = $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`,
  ...
  if (context.accountBalanceDelta !== undefined && context.customerId) {
    await conn.query(
      `UPDATE customers SET account_balance = GREATEST(0, COALESCE(account_balance, 0) + $1), updated_at = NOW() WHERE tenant_id = $2 AND id = $3`,
...
function anonymizedCustomerName(customerId: string) {
  const suffix = String(customerId || "customer")
    .replace(/[^a-zA-Z0-9]/g, "")
    .slice(-6)
    .toUpperCase();
...
async function consumeProductStockBatches(
  ...
  for (const batch of rows as any[]) {
    ...
    const batchRemaining = Math.max(
      0,
      Number(batch.remainingQuantity || batch.remaining_quantity || 0),
    );
...
async function deductCompletedSaleProductStock(
  ...
  for (const [productId, movement] of totals.entries()) {
    ...
    if (stockDelta && movement.quantity > 0) {
      await consumeProductStockBatches(
        conn,
        tenantId,
        productId,
        movement.quantity,
...
const offlineSyncConflictActions: Record<OfflineSyncConflictType, string> = {
  negative_stock_after_sync:
    "Review the synced sale against current stock, approve the shortage, adjust stock, or create a receiving/count correction.",
  duplicate_local_receipt:
    "Check whether this local receipt already exists in cloud sales before retrying or dismissing the local copy.",
  duplicate_table_or_tab:
...
async function collectOfflineStockConflicts(
  conn: any,
  tenantId: string,
  items: any[] = [],
) {
...
async function collectOfflineSaleSyncConflicts(
  conn: any,
  tenantId: string,
  sale: Partial<Sale>,
) {
  const conflicts: OfflineSyncConflict[] = [
    ...(await collectOfflineStockConflicts(
      conn,
      tenantId,
...
export async function createProduct(
  ...
  await syncDefaultProductLocationStock(
    tenantId,
    id,
    product.stock,
    product.minStock || 0,
...
export async function createCustomer(
  ...
  await query(
    ...
    [
      ...
      (customer as any).membershipStartedAt ||
        ((customer as any).loyaltyMemberStatus === "opted_out"
          ? null
  ...
  const consents =
    (customer as any).consents !== undefined
      ? await upsertCustomerConsents(
          tenantId,
          id,
          (customer as any).consents,
          (customer as any).consentActor || {},
        )
      : defaultCustomerConsentMap();
  return {
    id,
    ...
    loyaltyMemberStatus: ((customer as any).loyaltyMemberStatus ||
      "active") as any,
  ...
  await query(
    ...
    [
      ...
      staff.assignedCategories
        ? JSON.stringify(staff.assignedCategories)
        : "[]",
      staff.idNumber || null,
      staff.payRate || 0,
      staff.payType || null,
      ...
      staff.assignedLocationIds
        ? JSON.stringify(staff.assignedLocationIds)
        : "[]",
...
export async function updateCustomer(
  ...
  if (fields.length === 1) {
    ...
    return {
      ...
      accountBalance:
        r.account_balance !== null ? Number(r.account_balance) : 0,
      discountPercent:
        r.discount_percent !== null ? Number(r.discount_percent) : 0,
      uid: r.uid,
...
export async function getPurchaseOrders(
  ...
  return (rows as any[]).map((row) => ({
    ...
    receivedTotalAmount:
      row.receivedTotalAmount !== null && row.receivedTotalAmount !== undefined
...
export async function createPurchaseOrder(
  ...
  const totalAmount = Number(
    order.totalAmount ??
      items.reduce(
        (sum: number, item: any) =>
          sum + Number(item.quantity || 0) * Number(item.expectedPrice || 0),
        0,
      ),
  );
  await query(
...
function serializeStockBatch(row: any): StockBatch {
  const remainingQuantity = Number(
    row.remainingQuantity ?? row.remaining_quantity ?? 0,
  );
  const status = row.status || "active";
  const daysToExpiry = daysUntilDate(row.expiryDate ?? row.expiry_date);
  const expiryStatus =
    status === "depleted" || remainingQuantity <= 0
      ? "depleted"
  ...
  return {
    ...
    supplierInvoiceNumber:
      row.supplierInvoiceNumber ?? row.supplier_invoice_number ?? null,
    supplierInvoiceDate:
      row.supplierInvoiceDate ?? row.supplier_invoice_date ?? null,
    batchNumber: row.batchNumber ?? row.batch_number ?? null,
    receivedQuantity: Number(
      row.receivedQuantity ?? row.received_quantity ?? 0,
    ),
    remainingQuantity,
    ...
    locationId:
      row.locationId ?? row.location_id ?? DEFAULT_INVENTORY_LOCATION_ID,
    status,
    note: row.note || null,
    daysToExpiry,
    expiryStatus,
  ...
  try {
    ...
    for (const line of receiveItems) {
      if (line.lineIndex !== undefined && line.lineIndex !== null) {
        receiveByIndex.set(
          receiveLineKeyForIndex(Number(line.lineIndex)),
          line,
        );
      }
      if (line.productId) {
    ...
    for (const [index, item] of items.entries()) {
      ...
      if (!productId)
        throw new Error(
          `Purchase order line ${index + 1} is missing a product`,
        );
      const lineInput =
        receiveByIndex.get(receiveLineKeyForIndex(index)) ||
      ...
      const receivedQuantity = cleanReceivingNumber(
        lineInput.receivedQuantity === undefined ||
          lineInput.receivedQuantity === null
          ? orderedQuantity
      ...
      const receivedPrice = cleanReceivingNumber(
        lineInput.receivedPrice === undefined ||
          lineInput.receivedPrice === null
          ? expectedPrice
      ...
      const varianceQuantity = Number(
        (receivedQuantity - orderedQuantity).toFixed(3),
      );
      ...
      if (receivedQuantity > 0) {
        ...
        if (!movement)
          throw new Error(
            `Product ${item.productName || productId} was not found for receiving`,
          );
...
export async function deleteProduct(
  ...
  await query(`DELETE FROM products WHERE tenant_id = $1 AND id = $2`, [
    tenantId,
    productId,
  ]);
}
export async function deleteCustomer(
  ...
  try {
    ...
    const accountBalance = Number(
      customer.accountBalance ?? customer.account_balance ?? 0,
    );
    const blockerChecks = [
      {
        code: "wallet_balance",
      ...
      },
      {
        ...
        count: await countRows(
          conn,
          `SELECT COUNT(*) AS count FROM sales WHERE tenant_id = $1 AND customer_id = $2 AND status IN ('pending','open','kitchen')`,
          [tenantId, customerId],
        ),
        message:
          "Close or cancel open orders, tabs, and kitchen orders before anonymizing.",
      },
      {
        code: "active_laybys",
        count: await countRows(
          conn,
          `SELECT COUNT(*) AS count FROM layby_orders WHERE tenant_id = $1 AND customer_id = $2 AND status = 'active'`,
          [tenantId, customerId],
        ),
      ...
      },
      {
        ...
        count: await countRows(
          conn,
          `SELECT COUNT(*) AS count FROM customer_payout_requests WHERE tenant_id = $1 AND customer_id = $2 AND status IN ('pending','approved')`,
          [tenantId, customerId],
        ),
        message:
          "Resolve pending or approved customer payout requests before anonymizing.",
      },
      {
        code: "active_bookings",
        count: await countRows(
          conn,
          `SELECT COUNT(*) AS count FROM event_bookings WHERE tenant_id = $1 AND customer_id = $2 AND status IN ('inquiry','confirmed','in_progress')`,
          [tenantId, customerId],
        ),
    ...
    if (blockers.length > 0) {
      throw new Error(
        `Customer cannot be anonymized yet: ${blockers.map((item) => item.message).join(" ")}`,
      );
    }
    const reason =
      nullableText(actor.reason, 1000) || "Customer privacy deletion request";
    await conn.query(
      `UPDATE customers
          SET name = $1,
              email = NULL,
    ...
    for (const consentType of CUSTOMER_CONSENT_TYPES) {
      ...
      await conn.query(
        `INSERT INTO customer_consents (
         ...
         DO UPDATE SET status = 'revoked',
                       ...
                       updated_at = NOW()`,
        [
          `consent_${Date.now()}_${Math.random().toString(36).substring(7)}`,
          tenantId,
          customerId,
          consentType,
          reason,
      ...
      await conn.query(
        `INSERT INTO customer_consent_events (
         ...
         ) VALUES ($1, $2, $3, $4, $5, 'revoked', 'customer_anonymization', $6, $7, $8, NOW(), NOW())`,
        [
          makeCustomerConsentEventId(),
          tenantId,
          customerId,
          consentType,
          previousStatus,
    ...
    const retainedSaleCount = await countRows(
      conn,
      `SELECT COUNT(*) AS count FROM sales WHERE tenant_id = $1 AND customer_id = $2`,
      [tenantId, customerId],
    );
    await recordAuditEvent(conn, {
      tenantId,
...
export async function deleteStaff(
  ...
  await query(`DELETE FROM staff WHERE tenant_id = $1 AND id = $2`, [
    tenantId,
    staffId,
  ]);
}
...
export async function deleteTableSection(
  ...
  await query(`DELETE FROM table_sections WHERE tenant_id = $1 AND id = $2`, [
    tenantId,
    sectionId,
  ]);
}
export async function deleteRestaurantTable(
  ...
  await query(
    `DELETE FROM restaurant_tables WHERE tenant_id = $1 AND id = $2`,
    [tenantId, tableId],
  );
}
// ─────────────────────────────────────────────────────────────────────────
...
export async function createSale(
  ...
  if (offlineEventId) {
    const [existing] = await query(
      `SELECT id FROM sales WHERE tenant_id = $1 AND offline_event_id = $2 LIMIT 1`,
  ...
  try {
    ...
    const syncSource = offlineEventId
      ? "offline"
    ...
    const completedSale =
      ...
      ((sale as any).transactionType || "sale") === "sale";
    let promotionValidation: PromotionValidationResult | null = null;
    if (completedSale && saleHasPromotionInput(sale)) {
      promotionValidation = await validatePromotionForSale(
        conn,
        tenantId,
        promotionInputFromSale(sale),
        {
          lock: true,
      ...
      if (!promotionValidation.valid || !promotionValidation.promotion) {
        throw new Error(
          promotionValidation.reason || "Promotion could not be applied.",
        );
      }
    ...
    if (sale.items && Array.isArray(sale.items)) {
      for (const item of sale.items) {
        ...
        const stampSentAt = shouldStampWorkstationSentAt(
          sale.status || "pending",
          item,
        );
        ...
        if (productId) {
          ...
          for (const r of recipe) {
            ...
            substitutionGroups.set(group, [
              ...(substitutionGroups.get(group) || []),
              r,
            ]);
          }
          const rowsToDeduct = [
            ...standaloneRows,
            ...Array.from(substitutionGroups.values()).map((rows) =>
              chooseRecipeSubstitution(rows, item.quantity),
            ),
          ].filter(Boolean);
          for (const r of rowsToDeduct) {
            await conn.query(
              `UPDATE bulk_items SET stock = stock - $1 WHERE id = $2`,
        ...
        if (item.selectedModifiers && Array.isArray(item.selectedModifiers)) {
          for (const mod of item.selectedModifiers) {
            const [opt] = await conn.query<any>(
              `SELECT bulk_item_id, bulk_quantity FROM modifier_options WHERE id = $1`,
              [mod.optionId],
            );
    ...
    if (completedSale && sale.customerId) {
      loyaltyAward = await calculateLoyaltyAward(
        conn,
        tenantId,
        loyaltyInputFromSale(sale),
      );
      if (
        loyaltyAward.enabled &&
        loyaltyAward.customerFound &&
        loyaltyAward.memberStatus === "active"
      ) {
    ...
    await recordAuditEvent(conn, {
      ...
      details: {
        ...
        paymentProviders: paymentProviderSummary(
          sale.payments as any[] | undefined,
        ),
...
function shouldStampWorkstationSentAt(status: unknown, item: any) {
  return (
    String(status || "").toLowerCase() === "kitchen" &&
    Boolean(item?.workstationId)
  );
}
...
export async function updateSale(
  ...
  try {
    ...
    const [existingSaleResult] = await conn.query<any>(
      `SELECT status, transaction_type, staff_id, customer_id, offline_event_id FROM sales WHERE tenant_id = $1 AND id = $2 LIMIT 1`,
      [tenantId, saleId],
    );
    ...
    if (
      ...
      existingSale?.offline_event_id === updateOfflineEventId &&
    ...
    const nextStatus =
      updates.status !== undefined ? updates.status : existingSale?.status;
    const nextTransactionType =
      (updates as any).transactionType !== undefined
        ? (updates as any).transactionType || "sale"
        : existingSale?.transaction_type || "sale";
    const completingSale =
      existingSale?.status !== "completed" &&
      nextStatus === "completed" &&
      nextTransactionType === "sale";
    let promotionValidation: PromotionValidationResult | null = null;
    if (completingSale && saleHasPromotionInput(updates)) {
      const actualCustomerId =
        updates.customerId !== undefined
          ? updates.customerId || null
          : existingSale?.customer_id || null;
      ...
      if (!Array.isArray(promotionSale.items)) {
        const [saleItemsResult] = await conn.query<any>(
          `SELECT product_id AS productId, product_name AS name, price, quantity FROM sale_items WHERE sale_id = $1`,
          [saleId],
        );
      ...
      promotionValidation = await validatePromotionForSale(
        conn,
        tenantId,
        promotionInputFromSale(promotionSale),
        {
          lock: true,
          assertClientDiscount: true,
        },
    ...
    if (updates.items !== undefined) {
      ...
      for (const rawItem of updates.items) {
        ...
        const saleItemId =
          item.id && item.productId ? item.id : generateSaleItemId();
        ...
        const shouldStampSentAt =
          !ex?.ordered_at && shouldStampWorkstationSentAt(nextStatus, item);
        ...
        const orderedAtSql = shouldStampSentAt
          ? timerNowExpression()
          : timestampExpression(finalOrderedAt);
    ...
    if (updates.payments !== undefined) {
      await conn.query(`DELETE FROM sale_payments WHERE sale_id = $1`, [
        saleId,
      ]);
    ...
    let walletSalePayment: Awaited<
      ReturnType<typeof applyWalletSalePayment>
    > | null = null;
    ...
    if (shouldDeductStock) {
      ...
      if (!itemsForStock) {
        const [saleItemsResult] = await conn.query<any>(
          `SELECT product_id, product_name, quantity FROM sale_items WHERE sale_id = $1`,
          [saleId],
        );
        itemsForStock = saleItemsResult as any[];
      }
      if (updateOfflineEventId) {
        offlineSyncConflicts = await collectOfflineStockConflicts(
          conn,
          tenantId,
          itemsForStock || [],
        );
      }
      await deductCompletedSaleProductStock(
        conn,
        tenantId,
        itemsForStock || [],
        {
      ...
      const actualCustomerIdForLoyalty =
        updates.customerId !== undefined
          ? updates.customerId || null
          : existingSale?.customer_id || null;
...
export async function updateSaleItem(
  ...
  if (updates.status !== undefined) {
    ...
    if (updates.status === "accepted") {
      fields.push(
        `accepted_at = COALESCE(accepted_at, ${timerNowExpression()})`,
      );
    } else if (updates.status === "ready") {
      fields.push(`ready_at = COALESCE(ready_at, ${timerNowExpression()})`);
    } else if (updates.status === "delivered") {
      fields.push(
        `delivered_at = COALESCE(delivered_at, ${timerNowExpression()})`,
      );
    }
  }
...
export async function updateSalePaymentProviderStatus(
  ...
  try {
    ...
    await assertSaleNotInLockedTaxPeriod(
      conn,
      tenantId,
      saleId,
      "update sale payment provider status",
    );
    const [paymentRows] = await conn.query<any>(
      `SELECT
         sp.id,
    ...
    const nextDeviceId = nullableText(
      (input as any).providerDeviceId ?? payment.providerDeviceId,
      128,
    );
    const nextReference = nullableText(
      input.providerReference ?? payment.providerReference,
      255,
    );
    const nextAuthorizationCode = nullableText(
      (input as any).authorizationCode ?? payment.authorizationCode,
      128,
    );
    ...
    if (!sale)
      throw new Error(
        "Provider status was saved but the sale could not be loaded.",
      );
    return sale;
  } catch (err) {
    await conn.rollback();
...
export async function processSaleRefund(
  ...
  try {
    ...
    if (
      ...
      original.transaction_type === "refund"
    ) {
    ...
    const originalProviderPayment =
      originalPaymentRows.find((payment) => payment.method === input.method) ||
      (input.method === "bnpl"
        ? originalPaymentRows.find((payment) => payment.method === "bnpl")
        : null);
    ...
    const originalItems = new Map(
      itemRows.map((item) => [String(item.id), item]),
    );
    ...
    for (const row of priorRows) {
      ...
      refundedByProduct.set(
        key,
        (refundedByProduct.get(key) || 0) + Number(row.quantity || 0),
      );
    }
    ...
    for (const requested of input.items) {
      ...
      if (!originalItem)
        throw new Error(
          "One of the selected items no longer exists on this sale.",
        );
      ...
      const remaining = Math.max(
        0,
        Number(originalItem.quantity || 0) - alreadyRefunded,
      );
      if (quantity <= 0 || quantity > remaining) {
        throw new Error(
          `${originalItem.name} can only refund ${remaining} more.`,
        );
      }
    ...
    const ratio =
      originalTotal > 0 ? Math.min(1, refundSubtotal / originalTotal) : 0;
    ...
    const newRefundedAmount = Number(
      (Number(original.refunded_amount || 0) + refundTotal).toFixed(2),
    );
    const refundStatus =
      newRefundedAmount >= originalTotal - 0.01 ? "full" : "partial";
    ...
    const refundPayment: any = {
      ...
      provider:
        input.method === "bnpl"
          ? input.provider || originalProviderPayment?.provider || "bnpl"
          : null,
      providerReference:
        input.method === "bnpl"
          ? input.providerReference ||
            originalProviderPayment?.providerReference ||
            null
          : null,
      providerStatus: input.method === "bnpl" ? providerRefundStatus : null,
      providerNote:
        input.method === "bnpl" ? input.providerNote || reason : null,
    };
    await conn.query(
      `INSERT INTO sales (
        id, tenant_id, customer_id, user_id, staff_id, total, subtotal, tax_amount,
    ...
    for (const item of refundItems) {
      await conn.query(
        ...
        ) VALUES ($1, $2, $3, $4, $5, $6, 'delivered', ${timerNowExpression()}, ${timerNowExpression()}, NOW(), NOW())`,
        [
          item.id,
          refundId,
          item.productId,
    ...
    if (input.method === "wallet" && original.customer_id) {
      await conn.query(
        `UPDATE customers SET wallet_balance = COALESCE(wallet_balance, 0) + $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`,
...
export async function processSaleVoid(
  ...
  try {
    ...
    if (
      ...
      sale.transaction_type === "void"
    ) {
      throw new Error("This transaction cannot be voided.");
    }
    if (sale.status === "completed") {
      throw new Error(
        "Completed sales must be handled through the refund flow.",
      );
    }
...
export async function createPayoutRequest(
  ...
  try {
    ...
    const previousBalance = Number(
      staff.walletBalance || staff.wallet_balance || 0,
    );
    if (previousBalance < amount)
      throw new Error(
        "Staff wallet balance is not enough for this payout request.",
      );
...
export async function updatePayoutRequest(
  ...
  const rows = await query(
    `SELECT * FROM payout_requests WHERE tenant_id = $1 AND id = $2`,
    [tenantId, id],
  );
  return rows[0];
}
export async function createCustomerPayoutRequest(
  ...
  try {
    ...
    if (previousBalance < amount)
      throw new Error(
        "Customer wallet balance is not enough for this payout request.",
      );
    const nextBalance = Number((previousBalance - amount).toFixed(2));
    await conn.query(
      `UPDATE customers
...
export async function updateCustomerPayoutRequest(
  ...
  const rows = await query(
    `SELECT * FROM customer_payout_requests WHERE tenant_id = $1 AND id = $2`,
    [tenantId, id],
  );
  return rows[0];
}
export async function updateAppConfig(
  ...
  const businessName =
    String(config.business?.name || tenantId).trim() || tenantId;
  await query(
...
export async function markMessageRead(
  ...
  const rows = await query<{
    read_by: any;
  }>(`SELECT read_by FROM messages WHERE tenant_id = $1 AND id = $2 LIMIT 1`, [
    tenantId,
    messageId,
  ]);
  ...
  await query(
    `UPDATE messages SET read_by = $1 WHERE tenant_id = $2 AND id = $3`,
...
export async function setupTenant(data: {
  ...
  const businessConfig = {
    ...
    packageTier:
      config.business?.packageTier ||
      process.env.JPOS_HOSTED_PACKAGE_TIER ||
      "free",
  };
  const conn = await getConnection();
  try {
    ...
    await conn.query(
      `INSERT INTO workstations (id, tenant_id, name, type, status, created_at, updated_at)
       VALUES ($1, $2, 'Kitchen', 'kitchen', 'active', NOW(), NOW())`,
      [wsId, tenantId],
    );
    await conn.commit();
    return { tenantId };
    ...
    for (const p of products) {
      ...
      if (existing.length > 0) {
        ...
        if (duplicateIds.length > 0) {
          await conn.query(
            `DELETE FROM products WHERE tenant_id = $1 AND id = ANY($2::text[])`,
...
export async function getBulkItems(tenantId: string): Promise<BulkItem[]> {
  const rows = await query(
    `SELECT
       ...
       created_at AS createdAt,
       updated_at AS updatedAt
     FROM bulk_items WHERE tenant_id = $1`,
    [tenantId],
  );
  return rows as BulkItem[];
}
export async function createBulkItem(
  tenantId: string,
  item: Partial<BulkItem>,
): Promise<BulkItem> {
...
export async function deleteBulkItem(
  ...
  await query(`DELETE FROM bulk_items WHERE tenant_id = $1 AND id = $2`, [
    tenantId,
    id,
  ]);
}
export async function updateProductRecipe(
  ...
  try {
    await conn.beginTransaction();
    await conn.query(`DELETE FROM product_recipes WHERE product_id = $1`, [
      productId,
    ]);
    for (const r of recipe) {
      await conn.query(
...
export async function getRecipeCostingReport(tenantId: string) {
  ...
  const rows = (products || []).map((product: any) => {
    ...
    const lines = rawLines
      .map((row) => serializeRecipeRow(row))
      .map((line) => ({ ...line, costed: false }));
    const costedBulkIds = new Set<string>();
    const groupRows = new Map<string, any[]>();
    for (const raw of rawLines) {
      const group = nullableText(
        raw.substituteGroup ?? raw.substitute_group,
        64,
      );
      if (!group) {
    ...
    for (const alternatives of groupRows.values()) {
      const chosen = serializeRecipeRow(
        chooseRecipeSubstitution(alternatives, 1),
      );
      if (!chosen.isOptional) costedBulkIds.add(chosen.bulkItemId);
    }
    const reportLines = lines.map((line) => ({
      ...line,
      costed: costedBulkIds.has(line.bulkItemId),
    }));
    const recipeCost = decimal(
      reportLines.reduce(
        (sum, line) => sum + (line.costed ? Number(line.lineCost || 0) : 0),
        0,
      ),
      0,
      2,
    );
    const productCost = decimal(
      product.productCost ?? product.cost_price,
      0,
      2,
    );
    ...
    const grossMarginPercent =
      sellingPrice > 0 ? decimal((grossProfit / sellingPrice) * 100, 0, 2) : 0;
    return {
  ...
  const avgGrossMarginPercent = rows.length
    ? decimal(
        rows.reduce((sum, row) => sum + row.grossMarginPercent, 0) /
          rows.length,
        0,
        2,
      )
  ...
  return {
    ...
    summary: {
      ...
      substitutionGroupCount: rows.reduce(
        (sum, row) => sum + row.substituteGroupCount,
        0,
      ),
      optionalIngredientCount: rows.reduce(
        (sum, row) => sum + row.optionalCount,
        0,
      ),
    },
    rows,
...
export async function deleteModifierGroup(modifierId: string): Promise<void> {
  ...
  try {
    await conn.beginTransaction();
    await conn.query(`DELETE FROM modifier_options WHERE modifier_id = $1`, [
      modifierId,
    ]);
    await conn.query(`DELETE FROM product_modifiers WHERE id = $1`, [
      modifierId,
    ]);
    await conn.commit();
  } catch (err) {
...
export async function getProductModifiers(
  ...
  const groups = await query(
    `SELECT id, product_id AS productId, name, type, required, min_selection AS minSelection, max_selection AS maxSelection
     FROM product_modifiers WHERE product_id = $1`,
    [productId],
  );
  for (const g of groups as ModifierGroup[]) {
    const options = await query(
      `SELECT id, modifier_id AS modifierId, name, price_extra AS priceExtra, bulk_item_id AS bulkItemId, bulk_quantity AS bulkQuantity
       FROM modifier_options WHERE modifier_id = $1`,
      [g.id],
    );
