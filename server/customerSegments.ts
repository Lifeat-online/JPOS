...
function campaignHint(segment: string) {
  if (segment === "vip") return "Invite to VIP preview, loyalty bonus, or high-value offer.";
  if (segment === "frequent") return "Send repeat-customer bundles, subscriptions, or limited-time rewards.";
  if (segment === "recent" || segment === "new") return "Send welcome-back message, review request, or second-purchase offer.";
  if (segment === "at_risk") return "Send win-back reminder before the customer lapses.";
  if (segment === "lapsed") return "Send stronger win-back campaign or update preference prompt.";
  if (segment === "no_purchase") return "Send first-purchase incentive or profile completion prompt.";
  if (segment === "account_customer") return "Send account statement, payment reminder, or account-only offer.";
  if (segment === "wallet_credit") return "Send wallet-credit reminder.";
  if (segment === "loyalty_active") return "Send points-balance or tier-progress campaign.";
  if (segment === "campaign_ready") return "Customer has contact details and marketing consent for a compliant campaign.";
  return "General customer campaign.";
}

function segmentMatches(row: CustomerCampaignRow, filter: string) {
  if (!filter || filter === "all") return true;
  if (filter === "contactable") return row.contactable;
...
export function buildCustomerCampaignRows(customers: CustomerRow[], sales: SaleRow[], now = new Date()): CustomerCampaignRow[] {
  ...
  for (const sale of sales) {
    ...
    if (!Number.isNaN(createdAt.getTime())) {
      if (!current.firstPurchaseAt || createdAt < current.firstPurchaseAt) current.firstPurchaseAt = createdAt;
      if (!current.lastPurchaseAt || createdAt > current.lastPurchaseAt) current.lastPurchaseAt = createdAt;
    }
  ...
  return customers.map(customer => {
    ...
    if (totalSpend >= 1000 || orderCount >= 10) tags.push("vip");
    if (orderCount >= 5) tags.push("frequent");
    if (daysSinceLastPurchase !== null && daysSinceLastPurchase <= 30) tags.push("recent");
    if (daysSinceLastPurchase !== null && daysSinceLastPurchase > 30 && daysSinceLastPurchase <= 90) tags.push("at_risk");
    if (daysSinceLastPurchase !== null && daysSinceLastPurchase > 90) tags.push("lapsed");
    if (orderCount === 1 && daysSinceLastPurchase !== null && daysSinceLastPurchase <= 30) tags.push("new");
    if (orderCount === 0) tags.push("no_purchase");
    if (orderCount === 0 && daysSinceCreated !== null && daysSinceCreated <= 30) tags.push("new_profile");
    if ((customer.loyaltyMemberStatus || "active") === "active" && loyaltyPoints > 0) tags.push("loyalty_active");
    if (Boolean(customer.accountEnabled) || accountBalance > 0) tags.push("account_customer");
    if (walletBalance > 0) tags.push("wallet_credit");
    if (discountPercent > 0) tags.push("discount_customer");
    if (email || phone) tags.push("contactable");
    if (campaignEligible) tags.push("campaign_ready");
...
function segmentSummary(rows: CustomerCampaignRow[]) {
  ...
  for (const row of rows) {
    for (const tag of row.segmentTags) counts.set(tag, (counts.get(tag) || 0) + 1);
  }
