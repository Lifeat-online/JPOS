...
type TaskType = "cash_variance" | "sale_exception" | "refund_request" | "void_request" | "stock_adjustment_request" | "low_stock" | "ai_recommendation" | "stock_variance" | "offline_sync";
type TaskDraft = {
    tenantId: string;
    taskType: TaskType;
    title: string;
...
async function upsertTask(draft: TaskDraft) {
    ...
    await query(`INSERT INTO manager_tasks (
...
export async function createManagerSaleApprovalRequest(tenantId: string, input: SaleApprovalRequestInput) {
    ...
    await query(`INSERT INTO manager_tasks (
     ...
     ) VALUES ($1, $2, $3, $4, $5, $6, 'open', 'approval_request', $7, $8, $9, $10, NOW(), NOW())`, [
        taskId,
        tenantId,
...
function validateStockAdjustmentInput(input: Pick<StockAdjustmentRequestInput, "productId" | "delta" | "reason">) {
    ...
    if (!productId)
        throw new Error("Product is required for a stock adjustment");
...
export async function applyStockAdjustment(tenantId: string, input: Pick<StockAdjustmentRequestInput, "productId" | "delta" | "reason" | "note">, actor: StockAdjustmentActor = {}, referenceId?: string | null) {
...
export async function createManagerStockAdjustmentRequest(tenantId: string, input: StockAdjustmentRequestInput) {
    ...
    await query(`INSERT INTO manager_tasks (
       ...
       created_at, updated_at
     ) VALUES ($1, $2, 'stock_adjustment_request', $3, $4, $5, 'open', 'approval_request', $6, $7, $8, $9, NOW(), NOW())`, [
        taskId,
        tenantId,
...
async function applySourceDecision(tenantId: string, task: any, status: TaskStatus, input: DecisionInput) {
    ...
    if (task.sourceType === "approval_request" && status === "approved") {
        ...
        if (task.taskType === "stock_adjustment_request" || details.requestedAction === "stock_adjustment") {
            ...
            return applyStockAdjustment(tenantId, {
                productId,
        ...
        if (!saleId)
            throw new Error("Approval request is missing the sale reference");
        ...
        const reviewStatus = status === "declined"
            ...
            : status === "done"
                ...
                : status === "approved"
                    ...
                    : null;
        if (reviewStatus) {
            await query(`UPDATE cash_sessions
            SET review_status = $1,
                reviewed_at = NOW(),
                reviewed_by = $2,
                manager_notes = $3,
                updated_at = NOW()
          WHERE tenant_id = $4 AND id = $5`, [reviewStatus, input.staffId || null, input.note || null, tenantId, task.sourceId]);
        }
    }
    if (task.sourceType === "ai_insight" && ["approved", "declined", "done", "dismissed"].includes(status)) {
        ...
        await query(`UPDATE ai_insights SET status = $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`, [insightStatus, tenantId, task.sourceId]);
    }
...
export async function decideManagerTask(tenantId: string, taskId: string, input: DecisionInput) {
    ...
    if (!task)
        throw new Error("Manager task not found");
    ...
    const override = terminal
        ...
        : null;
    await query(`UPDATE manager_tasks
        SET status = $1,
