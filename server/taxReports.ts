...
function buildBreakdown(invoices: ReturnType<typeof serializeInvoiceRow>[]) {
  const byRate = new Map<string, { taxRate: number; invoiceCount: number; grossSales: number; outputTax: number }>();
...
export async function lockTaxPeriod(tenantId: string, input: TaxPeriodLockInput, actor: TaxActor = {}) {
  ...
  try {
    ...
    const [overlapRows] = await conn.query<any>(
...
export async function assertSaleNotInLockedTaxPeriod(conn: any, tenantId: string, saleId: string, action: string) {
  const [rows] = await conn.query(
    `SELECT tp.id, tp.period_start AS periodStart, tp.period_end AS periodEnd
       FROM sales s
  ...
  if (locked) throw new Error(lockedPeriodError(action, locked));
}
