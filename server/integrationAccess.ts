...
function delta(value: unknown) {
    ...
    if (!Number.isFinite(parsed) || parsed === 0)
        throw new Error("A non-zero stock delta is required");
...
function normalizeEventType(value: unknown): IntegrationWebhookEventType {
    ...
    if (text === "stock.snapshot" || text === "stock.count" || text === "stock.set")
        return "stock.snapshot";
    if (text === "stock.adjustment" || text === "stock.delta" || text === "stock.change")
        return "stock.adjustment";
    if (text === "product.price.update" || text === "product.price" || text === "price.update")
        return "product.price_update";
...
export async function createIntegrationApiKey(tenantId: string, input: {
    name?: string | null;
    scopes?: unknown;
    ...
    await query(`INSERT INTO integration_api_keys (
...
export async function listIntegrationApiKeys(tenantId: string) {
    const rows = await query<any>(`SELECT id, tenant_id, name, key_prefix, scopes, status, last_used_at,
...
export async function revokeIntegrationApiKey(tenantId: string, keyId: string, actor: IntegrationActor = {}) {
    await query(`UPDATE integration_api_keys
        SET status = 'revoked',
            revoked_at = NOW(),
            revoked_by = $1,
            revoked_by_name = $2
      WHERE tenant_id = $3 AND id = $4`, [actor.staffId || null, actor.staffName || null, tenantId, keyId]);
    await recordAuditEventSafe({
        tenantId,
...
export async function authenticateIntegrationApiKey(tenantId: string, secret: string | null | undefined) {
    ...
    const rows = await query<any>(`SELECT id, tenant_id, name, key_hash, key_prefix, scopes, status, last_used_at,
            created_by, created_by_name, created_at, revoked_at, revoked_by, revoked_by_name
       FROM integration_api_keys
    ...
    if (!row)
        return null;
    await query(`UPDATE integration_api_keys SET last_used_at = NOW() WHERE tenant_id = $1 AND id = $2`, [tenantId, row.id]);
    return {
        ...serializeApiKey(row),
        keyHash: incomingHash,
    } as AuthenticatedIntegrationApiKey;
}
export async function listIntegrationWebhookEvents(tenantId: string, filters: {
    status?: string | null;
    source?: string | null;
    ...
    params.push(limit);
    const rows = await query<any>(`SELECT id, tenant_id, api_key_id, source, event_type, idempotency_key, status,
...
function normalizeWebhookPayload(payload: any) {
    ...
    const idempotencyKey = clean(raw.idempotencyKey ?? raw.idempotency_key ?? raw.eventId ?? raw.event_id ?? raw.externalEventId ?? raw.external_event_id, 160);
    if (!idempotencyKey)
        throw new Error("An idempotency key is required");
    if (!STOCK_SYNC_EVENT_TYPES.has(eventType))
        throw new Error("Unsupported integration webhook event type");
...
async function findProductForSync(tenantId: string, item: any) {
    ...
    if (productId) {
        const rows = await query<any>(`SELECT id, name, stock, min_stock, barcode
         FROM products
    ...
    if (barcode) {
        ...
        LIMIT 1`, [tenantId, barcode]);
        if (rows[0])
            return rows[0];
    }
    throw new Error(`Product not found for sync item ${productId || barcode || "unknown"}`);
}
async function upsertLocationStock(tenantId: string, productId: string, locationId: string, quantityValue: number, actor: IntegrationActor) {
    await query(`INSERT INTO product_location_stock (
     ...
     DO UPDATE SET quantity = EXCLUDED.quantity,
                   ...
                   updated_at = NOW()`, [tenantId, productId, locationId, quantityValue, actor.staffId || null, actor.staffName || null]);
}
async function readLocationQuantity(tenantId: string, productId: string, locationId: string, fallback: number) {
    try {
        const rows = await query<any>(`SELECT quantity
         FROM product_location_stock
        WHERE tenant_id = $1 AND product_id = $2 AND location_id = $3
        LIMIT 1`, [tenantId, productId, locationId]);
...
async function applyStockSyncItem(tenantId: string, eventId: string, eventType: IntegrationWebhookEventType, item: any, actor: IntegrationActor) {
    ...
    if (eventType === "product.price_update") {
        ...
        if (price === null && costPrice === null)
            throw new Error("A price or cost price is required");
        const assignments: string[] = [];
        const params: any[] = [];
        if (price !== null) {
            assignments.push("price = $1");
            params.push(price);
        }
        if (costPrice !== null) {
            assignments.push("cost_price = $1");
            params.push(costPrice);
        }
        params.push(tenantId, productId);
        await query(`UPDATE products SET ${assignments.join(", ")}, updated_at = NOW() WHERE tenant_id = $1 AND id = $2`, params);
    ...
    await query(`UPDATE products SET stock = $1, updated_at = NOW() WHERE tenant_id = $2 AND id = $3`, [nextQuantity, tenantId, productId]);
...
export async function ingestStockWebhook(tenantId: string, payload: unknown, apiKey: Pick<AuthenticatedIntegrationApiKey, "id" | "name" | "keyPrefix" | "scopes"> | null) {
    if (!apiKey || !hasScope(apiKey, "stock:write")) {
        throw new Error("Integration API key does not have stock sync access");
    }
    const normalized = normalizeWebhookPayload(payload);
    const existing = await query<any>(`SELECT id, tenant_id, api_key_id, source, event_type, idempotency_key, status,
            entity_type, entity_id, payload, result, error_message, created_at, processed_at
       FROM integration_webhook_events
      WHERE tenant_id = $1 AND source = $2 AND idempotency_key = $3
      LIMIT 1`, [tenantId, normalized.source, normalized.idempotencyKey]);
    if (existing[0]) {
    ...
    await query(`INSERT INTO integration_webhook_events (
    ...
    try {
        ...
        await query(`UPDATE integration_webhook_events
          SET status = 'applied',
              entity_type = $1,
              entity_id = $2,
              result = $3,
        ...
        await query(`UPDATE integration_webhook_events
          SET status = 'failed',
              error_message = $1,
              processed_at = NOW()
        WHERE tenant_id = $2 AND id = $3`, [message, tenantId, eventId]);
        await recordAuditEventSafe({
            tenantId,
