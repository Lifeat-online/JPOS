...
function summarizeTrend(rows: Array<{
    label: string;
    variance: number;
    source: string;
}>) {
    ...
    for (const row of rows) {
        ...
        if (row.source === "register")
            current.registerVariance = money(current.registerVariance + row.variance);
        if (row.source === "eod")
            current.closeVariance = money(current.closeVariance + row.variance);
...
export async function getOperationalAnalyticsReport(tenantId: string, filters: OperationalReportFilters = {}) {
    ...
    const completedSales = await query<any>(`SELECT
       s.id AS saleId,
       s.created_at AS createdAt,
     ...
     ORDER BY s.created_at ASC`, [tenantId, range.fromSql, range.toSql]);
    ...
    const restaurantTables = await query<any>(`SELECT id, label, status
    ...
    const openTabRowsRaw = await query<any>(`SELECT
       id,
    ...
    const refundVoidRowsRaw = await query<any>(`SELECT
       id,
       created_at AS createdAt,
       total,
     ...
     ORDER BY created_at DESC`, [tenantId, range.fromSql, range.toSql]);
    ...
    const cashSessionRows = await query<any>(`SELECT
       id,
       staff_id AS staffId,
     ...
     WHERE tenant_id = $1
       ...
       AND COALESCE(closed_at, submitted_at, created_at) <= $3
     ORDER BY COALESCE(closed_at, submitted_at, created_at) ASC`, [tenantId, range.fromSql, range.toSql]);
    const cashCloseRows = await query<any>(`SELECT
       id,
     ...
     WHERE tenant_id = $1
       AND business_date >= $2
       AND business_date <= $3
     ORDER BY business_date ASC`, [tenantId, range.fromDate, range.toDate]);
