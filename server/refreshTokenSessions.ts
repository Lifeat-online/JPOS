...
export async function verifyStoredRefreshToken(token: string) {
  ...
  const rows = await query<any>(
    `SELECT id, tenant_id AS tenantId, staff_id AS staffId, token_hash AS tokenHash,
  ...
  const row = rows[0];
  if (!row) return { valid: false, reason: "not_found", tokenHash };
  if (row.revokedAt ?? row.revoked_at) return { valid: false, reason: "revoked", tokenHash, row };
  const expiresAt = row.expiresAt ?? row.expires_at;
  if (expiresAt && new Date(expiresAt).getTime() <= Date.now()) return { valid: false, reason: "expired", tokenHash, row };

  await query(`UPDATE refresh_token_sessions SET last_used_at = NOW() WHERE token_hash = ?`, [tokenHash]);
