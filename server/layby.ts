...
function normalizePaymentMethod(method: unknown): LaybyPaymentMethod {
  ...
  if (["cash", "card", "payfast", "wallet", "account"].includes(value)) return value as LaybyPaymentMethod;
  throw new Error("Unsupported lay-by payment method.");
}

function normalizeDueDate(value: unknown) {
  const dateText = String(value || "").trim();
  if (!dateText) throw new Error("Choose a due date for the lay-by.");
  const date = new Date(dateText);
  if (Number.isNaN(date.getTime())) throw new Error("Choose a valid due date for the lay-by.");
...
async function loadLaybyOrder(
  executor: Queryable,
  tenantId: string,
  laybyId: string,
  options: { lock?: boolean } = {}
) {
  const [orderRows] = await executor.query<any>(
    `SELECT
       id,
  ...
  const [itemsRows] = await executor.query<any>(
    `SELECT
       id,
       ...
       created_at AS createdAt
     FROM layby_items
     WHERE layby_order_id = ?
     ORDER BY created_at ASC`,
    [laybyId]
  );
  const [paymentRows] = await executor.query<any>(
    `SELECT
       id,
       layby_order_id AS laybyOrderId,
       method,
...
async function insertLaybyPayment(
  conn: DbConnection,
  tenantId: string,
  laybyId: string,
  payment: LaybyPaymentInput,
  context: { saleId?: string | null; note?: string | null } = {}
) {
  const amount = money(payment.amount);
  if (amount <= 0) throw new Error("Enter a lay-by payment amount greater than zero.");
    ...
    await conn.query(
      `UPDATE cash_sessions
          SET expected_cash = COALESCE(expected_cash, 0) + ?,
              updated_at = NOW()
        WHERE tenant_id = ? AND id = ?`,
      [amount, tenantId, payment.cashSessionId]
    );
    await conn.query(
...
async function recordCashRefund(
  ...
  tenantId: string,
  laybyId: string,
  input: LaybyPaymentInput,
  amount: number
) {
  if (amount <= 0 || normalizePaymentMethod(input.method || "cash") !== "cash" || !input.cashSessionId) return;
  const movementId = makeId("cm");
  await conn.query(
    `UPDATE cash_sessions
        SET expected_cash = COALESCE(expected_cash, 0) - ?,
            updated_at = NOW()
      WHERE tenant_id = ? AND id = ?`,
    [amount, tenantId, input.cashSessionId]
  );
  await conn.query(
    `INSERT INTO cash_movements (
      id, tenant_id, cash_session_id, type, direction, amount, sale_id, payment_id,
...
async function reserveLaybyStock(conn: DbConnection, tenantId: string, laybyId: string, items: any[], actor: LaybyActor) {
  for (const item of items) {
    if (!item.productId) throw new Error(`Lay-by item "${item.productName}" is missing a product id.`);
    ...
    if (!result) throw new Error(`Product "${item.productName}" was not found for lay-by reservation.`);
...
async function releaseLaybyStock(conn: DbConnection, tenantId: string, laybyId: string, items: any[], actor: LaybyActor) {
  for (const item of items) {
    if (!item.productId || Number(item.reservedQuantity || 0) <= 0) continue;
...
export async function listLaybyOrders(tenantId: string, filters: any = {}) {
  ...
  if (search) {
    clauses.push("(LOWER(customer_name) LIKE LOWER(?) OR LOWER(id) LIKE LOWER(?))");
...
export async function createLaybyOrder(tenantId: string, data: any) {
  ...
  if (!data.customerId) throw new Error("Select a customer before creating a lay-by.");
  if (items.length === 0) throw new Error("Add at least one item before creating a lay-by.");
  ...
  if (totalAmount <= 0) throw new Error("Lay-by total must be greater than zero.");

  const dueDate = normalizeDueDate(data.dueDate);
  const payment = data.payment || {};
  const depositAmount = money(payment.amount ?? data.depositAmount);
  if (depositAmount <= 0) throw new Error("Enter a lay-by deposit greater than zero.");
  if (depositAmount > totalAmount) throw new Error("Lay-by deposit cannot be more than the total.");
  ...
  try {
    ...
    if (!customerName) {
      const [customerRows] = await conn.query<any>(
        `SELECT name FROM customers WHERE tenant_id = ? AND id = ? LIMIT 1`,
        [tenantId, data.customerId]
      );
      customerName = String((customerRows as any[])[0]?.name || "").trim();
    }
    if (!customerName) throw new Error("Selected lay-by customer was not found.");
    ...
    for (const item of items) {
      await conn.query(
        ...
        [makeId("layitem"), id, item.productId, item.productName, item.price, item.quantity, item.quantity]
      );
    }
    ...
    if (!created) throw new Error("Lay-by was created but could not be loaded.");
    return created;
  } catch (err) {
    await conn.rollback();
    throw err;
...
export async function addLaybyPayment(tenantId: string, laybyId: string, payment: LaybyPaymentInput) {
  ...
  try {
    ...
    if (!order) throw new Error("Lay-by not found.");
    if (order.status !== "active") throw new Error("Only active lay-bys can accept payments.");
    if (amount > order.balanceDue) throw new Error("Lay-by payment cannot be more than the outstanding balance.");
    ...
    await conn.query(
      `UPDATE layby_orders
          SET amount_paid = ?,
              ...
              updated_at = NOW()
        WHERE tenant_id = ? AND id = ?`,
      [nextPaid, nextBalance, tenantId, laybyId]
    );
    ...
    if (!updated) throw new Error("Lay-by payment was recorded but the lay-by could not be loaded.");
    return updated;
  } catch (err) {
    await conn.rollback();
    throw err;
...
export async function completeLaybyOrder(tenantId: string, laybyId: string, data: LaybyActor & { payment?: LaybyPaymentInput }) {
  const conn = await getConnection();
  try {
    await conn.beginTransaction();
    const order = await loadLaybyOrder(conn, tenantId, laybyId, { lock: true });
    if (!order) throw new Error("Lay-by not found.");
    if (order.status !== "active") throw new Error("Only active lay-bys can be collected.");
    ...
    if (order.balanceDue > 0) {
      ...
      if (!payment || finalAmount <= 0) throw new Error("Record the final lay-by payment before collection.");
      if (finalAmount !== order.balanceDue) throw new Error("Final lay-by payment must match the outstanding balance.");
    ...
    if (!orderForSale) throw new Error("Lay-by could not be loaded for final sale.");
    ...
    await conn.query(
      `UPDATE layby_orders
          SET status = 'completed',
              ...
              updated_at = NOW()
        WHERE tenant_id = ? AND id = ?`,
      [nextPaid, saleId, data.staffId || null, data.staffName || null, tenantId, laybyId]
    );
    ...
    await conn.commit();
    const updated = await getLaybyOrderById(tenantId, laybyId);
    if (!updated) throw new Error("Lay-by was completed but could not be loaded.");
    return updated;
  } catch (err) {
    await conn.rollback();
    throw err;
...
export async function cancelLaybyOrder(tenantId: string, laybyId: string, data: LaybyActor & {
  ...
  try {
    ...
    const order = await loadLaybyOrder(conn, tenantId, laybyId, { lock: true });
    if (!order) throw new Error("Lay-by not found.");
    if (order.status !== "active") throw new Error("Only active lay-bys can be cancelled.");

    const refundAmount = money(data.refundAmount || 0);
    if (refundAmount > order.amountPaid) throw new Error("Lay-by refund cannot be more than the amount already paid.");
    ...
    await conn.commit();
    const updated = await getLaybyOrderById(tenantId, laybyId);
    if (!updated) throw new Error("Lay-by was cancelled but could not be loaded.");
    return updated;
  } catch (err) {
    await conn.rollback();
    throw err;
