...
function normalizeMode(value: string | undefined): DemoSeedMode {
    if (value === "retail" || value === "restaurant")
        return value;
...
export async function ensureTestTenant(runQuery: QueryRunner, options: Pick<SeedTestTenantOptions, "tenantId" | "tenantName" | "mode">) {
    ...
    await runQuery(`INSERT INTO tenants (id, name, created_at, updated_at)
     VALUES ($1, $2, NOW(), NOW())
     ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, updated_at = NOW()`, [options.tenantId, options.tenantName]);
    await runQuery(`INSERT INTO app_settings (tenant_id, business, categories, setup_completed, created_at, updated_at)
     VALUES ($1, $2, $3, 1, NOW(), NOW())
     ON CONFLICT (tenant_id) DO UPDATE
       SET business = EXCLUDED.business,
           categories = EXCLUDED.categories,
           setup_completed = 1,
           updated_at = NOW()`, [options.tenantId, business, categories]);
}
...
export async function countSeededTenantRows(runQuery: QueryRunner, tenantId: string) {
    ...
    for (const table of TEST_TENANT_SUMMARY_TABLES) {
        const rows = await runQuery(`SELECT COUNT(*) AS rowCount FROM ${table} WHERE tenant_id = $1`, [tenantId]);
