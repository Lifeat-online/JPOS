...
export const REQUIRED_SCHEMA_CHECKS: SchemaRequirement[] = [
  ...
  {
    table: "sale_payments",
    columns: ["method", "amount", "provider", "provider_device_id", "provider_reference", "authorization_code", "provider_status", "provider_note", "qr_payload"],
...
export function summarizeSchemaVerification(result: SchemaVerificationResult) {
  ...
  if (result.missingColumns.length > 0) {
    parts.push(
      `missing columns: ${result.missingColumns.map((item) => `${item.table}.${item.column}`).join(", ")}`
    );
  }
