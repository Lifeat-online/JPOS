...
import { Plus, Edit, Loader2, Save, X, ShoppingBag, Calendar, AlertCircle } from 'lucide-react';
...
export function PurchaseOrdersView() {
  ...
  useEffect(() => {
    const unsubOrders = onSnapshot(query(getTenantCollection(db, tenantId, "purchaseOrders")), (snap) => setOrders(snap.docs.map(d => ({id: d.id, ...(d.data() as any)} as PurchaseOrder))));
    const unsubVendors = onSnapshot(query(getTenantCollection(db, tenantId, "vendors")), (snap) => setVendors(snap.docs.map(d => ({id: d.id, ...(d.data() as any)} as Vendor))));
    const unsubProducts = onSnapshot(query(getTenantCollection(db, tenantId, "products")), (snap) => setProducts(snap.docs.map(d => ({id: d.id, ...(d.data() as any)} as Product))));
    
    // Simulate loading finish when all load
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentOrder.vendorId) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-[24px] border border-slate-100 dark:border-slate-800/60 shadow-sm">
        ...
        <button onClick={() => { setCurrentOrder({ status: 'draft', type: 'once_off', items: [] }); setModalOpen(true); }} className="px-6 py-3 bg-primary text-white rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-primary/30 active:scale-95 transition-all">
          ...
          return (
            <div key={order.id} className="bg-white dark:bg-slate-900 rounded-[24px] border border-slate-100 dark:border-slate-800/60 p-6 shadow-sm hover:shadow-xl transition-all flex flex-col justify-between">
              <div>
                ...
                <h3 className="text-lg font-black text-slate-800 dark:text-white mb-1"><span className="text-slate-400 font-medium">To:</span> {vendor ? vendor.name : 'Unknown Vendor'}</h3>
              ...
              <div className="flex gap-2 border-t border-slate-100 dark:border-slate-800/60 pt-4 mt-4">
                 <button onClick={() => { setCurrentOrder(order); setModalOpen(true); }} className="flex-1 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                   View / Edit
                 </button>
                 {order.status === 'sent' && (
                   <button onClick={async () => updateDoc(getTenantDoc(db, tenantId, "purchaseOrders", order.id), { status: 'received' })} className="flex-1 py-2 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold hover:bg-emerald-200 transition">
                     Mark Received
                   </button>
                 )}
              </div>
              ...
              <form onSubmit={handleSave} className="space-y-6">
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select required value={currentOrder.vendorId || ''} onChange={e => setCurrentOrder({...currentOrder, vendorId: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700/60 rounded-xl focus:outline-none focus:border-primary/50 text-sm font-semibold mt-1">
                    ...
                    <div>
                      ...
                      <select value={currentOrder.status || 'draft'} onChange={e => setCurrentOrder({...currentOrder, status: e.target.value as any})} className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700/60 rounded-xl focus:outline-none focus:border-primary/50 text-sm font-semibold mt-1">
                    ...
                    <div>
                      ...
                      <select value={currentOrder.type || 'once_off'} onChange={e => setCurrentOrder({...currentOrder, type: e.target.value as any})} className="w-full px-4 py-3 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700/60 rounded-xl focus:outline-none focus:border-primary/50 text-sm font-semibold mt-1">
                 ...
                 <div className="bg-slate-50 dark:bg-[#0B1120] rounded-2xl p-4 border border-slate-100 dark:border-slate-800/60">
                   <div className="flex justify-between items-center mb-4">
                     ...
                     <button type="button" onClick={() => {
                        const items = [...(currentOrder.items || []), { productId: '', productName: '', quantity: 1, expectedPrice: 0 }];
                   ...
                   <div className="space-y-3">
                      {(currentOrder.items || []).map((item, idx) => (
                         <div key={idx} className="flex gap-2">
                           <select 
                             className="flex-1 px-3 py-2 bg-white dark:bg-slate-900 rounded-lg text-sm font-medium border border-slate-200 dark:border-slate-700"
                             value={item.productId}
                             onChange={e => {
                               const p = products.find(p => p.id === e.target.value);
                               const newItems = [...(currentOrder.items || [])];
                               if (p) {
                                  newItems[idx] = { ...item, productId: p.id, productName: p.name, expectedPrice: p.costPrice || p.price };
                               }
                           ...
                           <input type="number" min="1" placeholder="Qty" value={item.quantity} onChange={e => {
                             ...
                             setCurrentOrder({...currentOrder, items: newItems});
                           }} className="w-20 px-3 py-2 bg-white dark:bg-slate-900 rounded-lg text-sm text-center border border-slate-200 dark:border-slate-700" />
                           <input type="number" step="0.01" placeholder="Price" value={item.expectedPrice} onChange={e => {
                             ...
                             setCurrentOrder({...currentOrder, items: newItems});
                           }} className="w-24 px-3 py-2 bg-white dark:bg-slate-900 rounded-lg text-sm border border-slate-200 dark:border-slate-700" />
                           <button type="button" onClick={() => {
                             const newItems = [...(currentOrder.items || [])];
                      ...
                      {(!currentOrder.items || currentOrder.items.length === 0) && <p className="text-xs text-center text-slate-400 py-4">No items added to this order.</p>}
                   </div>
                 </div>
                 ...
                 <div className="pt-4 flex gap-3">
                  ...
                  <button type="submit" disabled={isProcessing || !currentOrder.items?.length} className="flex-1 py-3.5 bg-primary text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-primary/20 flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-50 disabled:active:scale-100">
