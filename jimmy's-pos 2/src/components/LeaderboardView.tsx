...
export function LeaderboardView({ staff }: LeaderboardViewProps) {
  const rankedStaff = [...staff].sort((a, b) => {
    // Basic sorting: who has most tips, or best prep time
    const scoreA = (a.metrics?.totalTipsRounded || 0) - (a.metrics?.avgPrepTimeMs || 9999999);
    const scoreB = (b.metrics?.totalTipsRounded || 0) - (b.metrics?.avgPrepTimeMs || 9999999);
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rankedStaff.map((member, index) => {
            ...
            return (
              <div key={member.id} className="bg-white dark:bg-slate-900 rounded-[28px] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col relative group">
                <div className={`h-24 ${index === 0 ? 'bg-amber-500' : index === 1 ? 'bg-slate-400' : index === 2 ? 'bg-amber-700' : 'bg-primary'} relative overflow-hidden`}>
                  <div className="absolute top-4 right-4 text-white font-black text-2xl opacity-50">
                    #{index + 1}
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <span className="font-black text-slate-700 dark:text-slate-200">
                        {formatTime(member.metrics?.avgPrepTimeMs || 0)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                      <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-sm font-bold">
                      ...
                      <span className="font-black text-slate-700 dark:text-slate-200">
                        R{member.metrics?.totalTipsRounded || 0}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="mt-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {member.badges.map(badge => (
                          <div key={badge} className="px-3 py-1.5 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800/50 rounded-lg text-yellow-700 dark:text-yellow-500 text-xs font-bold flex items-center gap-1.5 cursor-help" title={badge}>
